...
public class AccActOnLoadBgFacadeControllerBean extends AbstractAccActOnLoadBgFacadeControllerBean
{
    ...
    protected RptParams _createTempTable(Context ctx, RptParams params)    throws BOSException, EASBizException
	{
	    ...
	    header.setLabels(new Object[][]{ 
	    		{
	    			"id","业务日期","付款申请单编码","申请金额","已付金额","实际付款日期","占用预算金额","状态","所属组织","工程项目","收款单位","制单人","制单日期","审批人","审批日期"
	    		}
	    },true);
    ...
    protected String getSql(Context ctx,RptParams params){
    	...
    	sb.append(" select t.* from(select bill.fid id,bill.fbookedDate bizDate,bill.fnumber number,sum(entry.frequestAmount) amount,sum(isnull(entry.factPayAmount,0)) payAmount,bill.fpaydate payDate,(case when (bill.fhasClosed=1 or bill.fstate not in('3AUDITTING','4AUDITTED')) then 0 else sum(entry.frequestAmount-isnull(entry.factPayAmount,0)) end) actOnLoadAmount,bill.fstate state,orgUnit.fname_l2 orgUnit,curProject.fname_l2 curProject,supplier.fname_l2 supplier,creator.fname_l2 creator,bill.fcreateTime createTime,auditor.fname_l2 auditor,bill.fauditTime auditTime ");
    	...
    	sb.append(" left join T_BD_Supplier supplier on supplier.fid=bill.fsupplierid");
    	...
    	sb.append(" group by bill.fid,bill.fbookedDate,bill.fnumber,bill.fpaydate,bill.fstate,bill.fhasClosed,orgUnit.fname_l2,curProject.fname_l2,supplier.fname_l2,creator.fname_l2,bill.fcreateTime,auditor.fname_l2,bill.fauditTime ");
    	sb.append(" )t order by t.bizDate,t.number ");
    	return sb.toString();
    }
}