...
////		execQuery();
	...
	private void fillTable() throws Exception{
		...
		// }
		String measureStageID = getFilterValue("measureStage.id");
		if (measureStageID != null) {
			sql.append(" and \"TEMPLATEMEASURECOST\".FMeasureStageID='" + measureStageID + "'");
		}
...
// view.getSelector().add("costCenterOU.partFI.unit.id");
	...
	public Object getData() {
		...
		if(this.tabPane.getSelectedIndex()==0){
			...
			if(isPrice.booleanValue()){
				...
				return value==null?null:new Object[]{FDCHelper.toBigDecimal(value, 2)};
			}
		}
	}
	...
	private void loadDBpanel(KDPanel panel) throws Exception {
...
//		super.actionQuery_actionPerformed(e);fprojectperiod
	...
	protected void tabPane_stateChanged(ChangeEvent e) throws Exception {
		if(tabPane.getSelectedIndex()==1&&!hasLoadDBPanel){
			loadDBpanel(this.pnlDB);
		}
		Boolean isPrice=(Boolean)getUIContext().get("isPrice");
		if(isPrice!=null&&isPrice.booleanValue()){
