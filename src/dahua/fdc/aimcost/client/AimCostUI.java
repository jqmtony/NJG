...
//					this.tblMain.setUserObject("delteRow");
	...
	public void actionUnAudit_actionPerformed(ActionEvent e) throws Exception {
		...
		if (node.getUserObject() instanceof CurProjectInfo) {// 相对工程项目而言
			CurProjectInfo project = (CurProjectInfo) node.getUserObject();
			projectId = project.getId().toString();
			if (isExitAimAimCostAdjust(projectId)) {
				FDCMsgBox.showInfo("该工程项目下存在\"已提交\"状态的目标成本调整单,不允许反审批！");
				SysUtil.abort();
			}
			if (isGenByAimAimCostAdjust(projectId)) {
				FDCMsgBox.showInfo("当前版本目标成本是由目标成本调整单生成，不允许反审批");
				SysUtil.abort();
			}
		}
		verifySaved(null);
	...
	/**
	 * 当前工程项目是否存在状态为“已提交”的目标成本调整单
	 * 
	 * @return
	 */
	private boolean isExitAimAimCostAdjust(String projectId) {
		EntityViewInfo view = new EntityViewInfo();
		...
		view.setFilter(filter);
		try {
			AimAimCostAdjustCollection aacaCol = AimAimCostAdjustFactory.getRemoteInstance().getAimAimCostAdjustCollection(view);
			if (aacaCol.size() != 0) {
				return true;
			}
	...
	/**
	 * 当前版本目标成本是否是目标成本调整单生成
	 * 
	 * @return
	 */
	private boolean isGenByAimAimCostAdjust(String projectId) {
		boolean flag = false;
		String versionNumber = txtVersionNumber.getText();
		EntityViewInfo view = new EntityViewInfo();
		view.getSelector().add("genByAdjust");
		FilterInfo filter = new FilterInfo();
		...
		view.setFilter(filter);
		AimCostCollection acCol;
		try {
			acCol = AimCostFactory.getRemoteInstance().getAimCostCollection(view);
			if (acCol != null && acCol.size() == 1) {
				AimCostInfo aimCostInfo = acCol.get(0);
				if (aimCostInfo.getGenByAdjust() != null) {
					flag = true;
				}
			}
		} catch (BOSException e) {
		...
		if (node.getUserObject() instanceof CurProjectInfo) {
			...
			if (info.isIsLeaf()) {
				if (!info.getProjectStatus().getId().toString().equals(
					...
					info.setProjectStatus(ProjectStatusFactory.getRemoteInstance().getProjectStatusInfo(
							new ObjectUuidPK(BOSUuid.read(ProjectStatusInfo.proceedingID))));
