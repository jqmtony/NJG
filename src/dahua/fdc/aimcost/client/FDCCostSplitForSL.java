 ...
 * Date 2006-12-7
 */
public class FDCCostSplitForSL
{
    private static final Logger logger = CoreUIObject.getLogger(FDCCostSplitForSL.class);
    private Context ctx=null;
    
    public FDCCostSplitForSL(Context ctx){
    	this.ctx=ctx;
    }
	 ...
	 * 创建时间：2006-10-19 <p>
	 */    
    public void apptAmount(IObjectCollection allEntrys,ForecastChangeVisSplitEntryInfo curEntry){
    	...
    	if(costSplitType==null || costSplitType.equals(CostSplitTypeEnum.MANUALSPLIT)){
			...
			if(amount==null || amount.compareTo(FDCHelper.ZERO)==0){
				...
				for(int i=curIndex+1; i<allEntrys.size(); i++){
					ForecastChangeVisSplitEntryInfo entry = (ForecastChangeVisSplitEntryInfo)allEntrys.getObject(i);
    ...
    private void apptAmountBotUp(IObjectCollection allEntrys,int curIndex){
		...
		ForecastChangeVisSplitEntryInfo curEntry=(ForecastChangeVisSplitEntryInfo)allEntrys.getObject(curIndex);		
	 ...
	 * 创建时间：2006-12-26 <p>
	 */
    private void adjustAmount(IObjectCollection allEntrys,ForecastChangeVisSplitEntryInfo curEntry){
    	ForecastChangeVisSplitEntryInfo entry=curEntry;    	
		...
		for(int i=index+1; i<allEntrys.size(); i++){				
			ForecastChangeVisSplitEntryInfo tempEntry=(ForecastChangeVisSplitEntryInfo)allEntrys.getObject(i);
    	...
    	//对修正后的数据进行产品拆分
    	for(int i=index+1;i<allEntrys.size();i++){
    		ForecastChangeVisSplitEntryInfo topEntry=(ForecastChangeVisSplitEntryInfo) allEntrys.getObject(i);
...
//        			apptAmountAddlAcct(allEntrys, i);	    			
    ...
    private void adjustAmountProject(IObjectCollection allEntrys, int index){
		ForecastChangeVisSplitEntryInfo curEntry =(ForecastChangeVisSplitEntryInfo)allEntrys.getObject(index);		
		...
		for(int i=index+1; i<allEntrys.size(); i++){		
			entry=(ForecastChangeVisSplitEntryInfo)allEntrys.getObject(i);
		...
		if(idx>0 && curAmount!=null&&curAmount.compareTo(amountTotal)!=0){	
			entry=(ForecastChangeVisSplitEntryInfo)allEntrys.getObject(idx);
	...
	private void adjustBotupAmt(IObjectCollection allEntrys,
			ForecastChangeVisSplitEntryInfo curEntry, int index, int level) {
		//末级拆分修正
		BigDecimal total=FDCHelper.ZERO;
		BigDecimal maxAmount=FDCHelper.ZERO;
		int max=index;
		for(int i=index+1; i<allEntrys.size(); i++){		
			ForecastChangeVisSplitEntryInfo myEntry=(ForecastChangeVisSplitEntryInfo)allEntrys.getObject(i);
		...
		if(max>index){
			ForecastChangeVisSplitEntryInfo myEntry=(ForecastChangeVisSplitEntryInfo)allEntrys.getObject(max);
			BigDecimal amt=FDCHelper.toBigDecimal(myEntry.getAmount());
			amt=FDCHelper.toBigDecimal(curEntry.getAmount()).subtract(total).add(amt);
			myEntry.setAmount(amt);
			//汇总到非明细
			for(int i=index+1; i<allEntrys.size(); i++){		
				ForecastChangeVisSplitEntryInfo myEntry1=(ForecastChangeVisSplitEntryInfo)allEntrys.getObject(i);
				...
				if(!myEntry1.getCostAccount().getCurProject().isIsLeaf()){
					...
					for(int j=i+1;j<allEntrys.size();j++){
						ForecastChangeVisSplitEntryInfo myEntry2=(ForecastChangeVisSplitEntryInfo)allEntrys.getObject(j);
    ...
    private void apptAmountBotUp(IObjectCollection allEntrys,int curIndex, BigDecimal ratio, BigDecimal parApportionTotal, 
		...
		ForecastChangeVisSplitEntryInfo topEntry =(ForecastChangeVisSplitEntryInfo)allEntrys.getObject(curIndex);
...
//		amountTotal=amountTotal.setScale(2, BigDecimal.ROUND_HALF_UP);
	 ...
	 * 创建时间：2006-11-07 <p>
	 */
    public void totApptValue(IObjectCollection allEntrys,ForecastChangeVisSplitEntryInfo curEntry){
    	int curIndex=allEntrys.indexOf(curEntry);
    	if(curIndex==-1){
    		return;
    	}
	 ...
	 * 创建时间：2006-11-30 <p>
	 */
	public static boolean isProdSplitLeaf(ForecastChangeVisSplitEntryInfo entry){
	 ...
	 * 创建时间：2006-11-30 <p>
	 */
    public boolean isProdSplitParent(ForecastChangeVisSplitEntryInfo entry){		
		boolean isTrue=false;
	 ...
	 * 创建时间：2006-12-28 <p>
	 */
    public static boolean isChildProjSameAcct(ForecastChangeVisSplitEntryInfo entry, ForecastChangeVisSplitEntryInfo parent){		
		boolean isTrue=false;
    ...
    private void totApptValueTopDown(IObjectCollection allEntrys,int topIndex){
    	ForecastChangeVisSplitEntryInfo topEntry =(ForecastChangeVisSplitEntryInfo)allEntrys.getObject(topIndex);  
