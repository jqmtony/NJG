...
//		happenGetter=fullDynamicCostMap.getHappenDataGetter();
    ...
    protected void fillTable()  throws Exception {
		...
		// 如果选择显示级次，加入可拆分过滤条件
		FilterItemInfo itemInfo = new FilterItemInfo("isSplit", new Integer(1));
		if (isShowSplitAcct) {
			acctFilter.getFilterItems().add(itemInfo);
		}	
		...
		// 保证所有的科目都在缓存中
		acctFilter.getFilterItems().remove(itemInfo);
    ...
    protected void fillNode(DefaultMutableTreeNode node, KDTable table) throws BOSException, SQLException, EASBizException {
		...
		// 子集科目集合
		CostAccountCollection coin = FDCAimcostClientHelper.getCostAccountCollection(costAcct,
				curProject.getId().toString(), acctChidrenMap);
		
		if (node.isLeaf()) {
			        loadRowDatas(node, table, costAcct, conSplits, conBills,
					aimCosts, noTextSplits,
					changeSplits, changeBills, completePrjAmts, paySplits, changeReasons, acctId,
					row, coin);
		} else {
		...
		for (int i = 1; i < dymicCols + 1; i++) {
			String year_month = year_from
					+ getMonth(String.valueOf((new Integer(month_from).intValue() + i - 1)));

			for (int j = 1; j < 6; j++) {
				if (new Integer(month_from).intValue() + i - 1 > 12 * j) {
					int newStartYear = new Integer(year_from).intValue() + 1 * j;
					year_month = String.valueOf(newStartYear)
							+ getMonth(String.valueOf(new Integer(month_from).intValue()
									+ i - 1 - 12 * j));
				}
			}
			// 无文本
			if(costAcct.isIsLeaf()){
				...
				row.getCell(year_month + "txt").setValue(
					FDCHelper.toBigDecimal(noTextSplits.get(acctId + year_month)));
	 ...
	 * @see
	 */
	private void loadRowDatas(DefaultMutableTreeNode node, KDTable table,
			CostAccountInfo costAcct, Map conSplits, Map conBills, Map aimCosts, Map noTextSplits,
			Map changeSplits, Map changeBills, Map completePrjAmts, Map paySplits,
			Map changeReasons, String acctId, IRow row, CostAccountCollection coin) {
		BigDecimal conSplitAmts = FDCHelper.ZERO;
		String costId = null;
		ContractBillCollection contractCion = null;
		// 科目下合同金额
...
//				dataRow.getCell("constrAmt").setValue(FDCHelper.divide(info.getAmount(), totalConstructArea,2,BigDecimal.ROUND_HALF_UP));
				...
				for (int i = 1; i < dymicCols + 1; i++) {
					...
					for (int k = 0; k < changeTypes.size(); k++) {
						...
						Set changeCounnts = (Set) changeBills.get(acctId + contractId + year_month
								+ typeId);
						if (changeCounnts != null) {
							dataRow.getCell(year_month + "num" + typeId).setValue(
									new Integer(changeCounnts.size()));
						
						}
						BigDecimal changeAmt = (BigDecimal) changeSplits.get(acctId + contractId
								+ year_month + typeId);
						if (FDCHelper.toBigDecimal(changeAmt).compareTo(FDCHelper.ZERO) != 0) {
							...
							BigDecimal prop = FDCHelper.divide(FDCHelper.multiply(changeAmt,
									FDCHelper.ONE_HUNDRED), info.getAmount(), 2,
						...
						if (i > 1) {
							...
							// 合计
							Set changeTotalCounnts = (Set) changeBills.get(acctId + contractId
									+ typeId);
							
							if (changeTotalCounnts != null) {
								dataRow.getCell(totalKey + typeId).setValue(
										new Integer(changeTotalCounnts.size()));
							}
							BigDecimal totalChangeAmt = (BigDecimal) changeSplits.get(acctId
									+ contractId + typeId);
							
							if (FDCHelper.toBigDecimal(totalChangeAmt).compareTo(FDCHelper.ZERO) != 0) {
								dataRow.getCell(totalKey + "changeAmt" + typeId).setValue(
										totalChangeAmt);
								BigDecimal prop = FDCHelper.divide(FDCHelper.multiply(
										totalChangeAmt, FDCHelper.ONE_HUNDRED), info.getAmount(),
					...
					if (i > 1) {
						...
						if (conBills.containsKey(acctId + contractId + totalKey)) {
							...
							dataRow.getCell(totalKey + "con_num").setValue(
									new Integer(conCounts.size()));
						}
						if (conSplits.containsKey(acctId + contractId + totalKey)) {
							BigDecimal conSplit = FDCHelper.toBigDecimal(conSplits.get(acctId
									+ contractId + totalKey));
						...
						if (conBills.containsKey(acctId + contractId + year_month)) {
							...
							dataRow.getCell(year_month + "con_num").setValue(
									new Integer(conCounts.size()));
						}
						if (conSplits.containsKey(acctId + contractId + year_month)) {
							BigDecimal conSplit = FDCHelper.toBigDecimal(conSplits.get(acctId
									+ contractId + year_month));
							dataRow.getCell(year_month + "con_amt").setValue(conSplit);
						}
					}
					// 应付
					if (completePrjAmts.containsKey(contractId + year_month)) {
						BigDecimal completePrjAmt = FDCHelper.toBigDecimal(completePrjAmts
								.get(contractId + year_month));
						dataRow.getCell(year_month + "pay_amt").setValue(completePrjAmt);
						
					}
					// 实付
					if (paySplits.containsKey(acctId + contractId + year_month)) {
						BigDecimal paySplit = FDCHelper.toBigDecimal(paySplits.get(acctId
								+ contractId + year_month));
						dataRow.getCell(year_month + "pay_pay").setValue(paySplit);
						BigDecimal prop = FDCHelper.divide(FDCHelper.multiply(paySplit,
								FDCHelper.ONE_HUNDRED), completePrjAmts
								.get(contractId + year_month), 2, BigDecimal.ROUND_HALF_UP);
						dataRow.getCell(year_month + "pay_scale").setValue(prop);
					}
				}
				
				if (completePrjAmts.containsKey(contractId + totalKey)) {
					BigDecimal total = FDCHelper.toBigDecimal(completePrjAmts.get(contractId
							+ totalKey));
					// 累计动态成本-应付款
				...
				if (dymicCols > 1) {
					...
					if (paySplits.containsKey(acctId + contractId)) {
						BigDecimal total = FDCHelper.toBigDecimal(paySplits
								.get(acctId + contractId));
						dataRow.getCell(totalKey + "pay_pay").setValue(total);
						BigDecimal prop = FDCHelper.divide(FDCHelper.multiply(total,
								FDCHelper.ONE_HUNDRED), completePrjAmts.get(contractId), 2,
								BigDecimal.ROUND_HALF_UP);
						dataRow.getCell(totalKey + "pay_scale").setValue(prop);
					}
				}
				// 累计动态成本-实付款
				BigDecimal dynCostTotal = FDCHelper.add(paySplits.get(acctId + contractId),
						paySplits.get(acctId + contractId + getBegainPeriodNumber()));
				if (FDCHelper.toBigDecimal(dynCostTotal).compareTo(FDCHelper.ZERO) != 0) {
					...
					BigDecimal prop = FDCHelper.divide(FDCHelper.multiply(dynCostTotal,
							FDCHelper.ONE_HUNDRED), completePrjAmts.get(contractId), 2,
							BigDecimal.ROUND_HALF_UP);
				...
				if (isDebug) {
					for (int i = 0; i < tblMain.getColumnCount() - 1; i++) {
						...
						System.out.println(i + "科目明细" + tblMain.getHeadRow(4).getCell(i).getValue()
								+ dataRow.getCell(i).getValue());
					}
				}
			}
		}
		...
		//获得当前成本科目所有子集目标成本
		BigDecimal childrenaimCost = FDCHelper.ZERO;
		for (int i = 0, l = coin.size(); i < l; i++) {
			costId = coin.get(i).getId().toString();
			childrenaimCost = FDCHelper
					.toBigDecimal(aimCosts.get(costId + "aimCost"));
			aimCost= FDCHelper.add(aimCost, childrenaimCost);
		}
		BigDecimal aimCost4 = FDCHelper.divide(aimCost, FDCHelper.TEN_THOUSAND, 4,
				BigDecimal.ROUND_HALF_UP);
		if (aimCost.compareTo(FDCHelper.ZERO) != 0) {
			row.getCell("aimCost").setValue(
					FDCHelper.divide(aimCost, FDCHelper.TEN_THOUSAND, 4, BigDecimal.ROUND_HALF_UP));
			row.getCell("constrAmt").setValue(
					FDCHelper.divide(aimCost, totalConstructArea, 2, BigDecimal.ROUND_HALF_UP));
		}
		if (dymicCols > 1) {
			// 合计
       ...
       //获得成本科目已发生成本
		...
		BigDecimal childrenhasHappenAmount = FDCHelper.ZERO;
//		hasHappenAmount = (BigDecimal) aimCosts.get(acctId + "soFarHasAmt");
		// 已发生
		if (coin.size() > 0) {
			for (int i = 0, l = coin.size(); i < l; i++) {
				costId = coin.get(i).getId().toString();
				if (coin.get(i).isIsLeaf()) {
					childrenhasHappenAmount = FDCHelper
					.toBigDecimal(aimCosts.get(costId + "soFarHasAmt"));
					if (childrenhasHappenAmount.compareTo(FDCHelper.ZERO) != 0) {
						hasHappenAmount = FDCHelper.add(childrenhasHappenAmount, hasHappenAmount);
					}
				}
			}
		} else {
		...
		BigDecimal hasHappen = FDCHelper.divide(hasHappenAmount, FDCHelper.TEN_THOUSAND, 4,
				BigDecimal.ROUND_HALF_UP);
		if (FDCHelper.toBigDecimal(hasHappen).compareTo(FDCHelper.ZERO) != 0) {
			row.getCell(totalDynCostKey).setValue(hasHappen);
		}
		// 平米造价
		BigDecimal price = FDCHelper.divide(hasHappenAmount, totalConstructArea, 2,
				BigDecimal.ROUND_HALF_UP);
		...
		// 与目标成本差比
		BigDecimal difProp = FDCHelper.divide(FDCHelper.multiply(aimCostDifAmt,
				FDCHelper.TEN_THOUSAND), totalConstructArea, 2, BigDecimal.ROUND_HALF_UP);
		if (FDCHelper.toBigDecimal(difProp).compareTo(FDCHelper.ZERO) != 0) {
			row.getCell(aimCostDifKey + priceKey).setValue(difProp);
		}
		// 已发生成本占目标成本百分比
		BigDecimal hasHappenProp = FDCHelper.divide(FDCHelper.multiply(hasHappenAmount,
				FDCHelper.ONE_HUNDRED), aimCost, 2, BigDecimal.ROUND_HALF_UP);
		...
		if (isDebug) {
			for (int i = 0; i < tblMain.getColumnCount() - 1; i++) {
				...
				System.out.println(i + "明细科目" + tblMain.getHeadRow(4).getCell(i).getValue()
...
//			columns.add(year_month+"pay_pay");
    ...
    //处理明细科目下的明细行数据汇总
	private void setDynUnionData(KDTable table) throws BOSException{
...
//			amountColumns.add(totalKey+typeId);
		...
		for (int i = 0; i < table.getRowCount(); i++) {
			...
			if (row.getUserObject() == null) {
				// 设置汇总行
				CostAccountInfo accountinfo =(CostAccountInfo)row.getCell("acctNumber").getUserObject();
		...
		for (int i = 1; i < dymicCols + 1; i++) {
			...
			for (int k = 0; k < changeTypes.size(); k++) {
				ChangeTypeInfo typeInfo = changeTypes.get(k);
				String typeId = typeInfo.getId().toString();
                amountTotalColumns.add(year_month+"changeAmt"+typeId);  
			}
		}
	
		//填充合计行，加上拆分金额列
		amountTotalColumns.add("conSplit");
	...
	protected void setUnionData(KDTable table) {
		...
		//移除目标成本列，跳过汇总
		amountColumns.remove("aimCost");
		//移除与目标成本差异列，跳过汇总
		amountColumns.remove(aimCostDifKey + amtKey);	
		//移除已发生成本列，跳过汇总
		amountColumns.remove(totalDynCostKey);	
		//移除已发生成本占目标成本比例列，跳过汇总
		amountColumns.remove(hasHappenPropKey);
		//移除无文本合同列，跳过汇总
		amountColumns.remove("totalTxt");
		for (int i = 0; i < table.getRowCount(); i++) {
			IRow row = table.getRow(i);
			if (row.getUserObject() == null) {
				// 设置汇总行
		...
		for (int i = 1; i < dymicCols + 1; i++) {
			String year_month = year_from + getMonth(String.valueOf((new Integer(month_from).intValue() + i - 1)));
			CostAccountCollection collection = null;
			BigDecimal noText1 = FDCHelper.ZERO;
			BigDecimal childrennoText1 = FDCHelper.ZERO;
			String acctId =null;
			...
			for (int z = 0; z < table.getRowCount(); z++){
				IRow row = this.tblMain.getRow(z);
				CostAccountInfo	info = (CostAccountInfo)row.getCell("acctNumber").getUserObject();
				if(info!=null&&!info.isIsLeaf()){
					try {
						acctId= info.getId().toString();
						collection =FDCAimcostClientHelper.getCostAccountCollection(info,
								curProject.getId().toString(), acctChidrenMap);
						for(int k =0;k<collection.size();k++){	
							CostAccountInfo infoss =collection.get(k);	
							if(infoss.isIsLeaf()){			
								String costId = infoss.getId().toString();
								childrennoText1= FDCHelper.toBigDecimal(noTextSplits.get(costId + year_month));
								if(childrennoText1.compareTo(FDCHelper.ZERO)!=0){
									noText1 =FDCHelper.add(noText1, childrennoText1);
								}
							}
						}
						if(noText1.compareTo(FDCHelper.ZERO)!=0){
							row.getCell(year_month+"txt").setValue(noText1);
						}
						noText1 = FDCHelper.ZERO;
		...
		//汇总合计无文本列
		for(int z= 0; z<tblMain.getRowCount();z++){
			IRow row = tblMain.getRow(z);
			BigDecimal noText =FDCHelper.ZERO;
			BigDecimal childrennoText = FDCHelper.ZERO;
			if(row.getCell("acctNumber").getUserObject()!=null){
			for (int i = 1; i < dymicCols + 1; i++) {
				String year_month = year_from + getMonth(String.valueOf((new Integer(month_from).intValue() + i - 1)));
				...
				childrennoText = (BigDecimal)row.getCell(year_month+"txt").getValue();
				if(childrennoText!=null){
					noText = FDCHelper.add(noText, childrennoText);
				}	
		 }
			if(dymicCols>1){
				row.getCell("totalTxt").setValue(noText);	
		}
	  }
	}
		...
		//填充合计行，加上与目标成本差列 
		amountColumns.add(aimCostDifKey + amtKey);
		//填充合计行，加上已发生成本列
		amountColumns.add(totalDynCostKey);
		//填充合计行，加上无文本合同列
		amountColumns.add("totalTxt");
...
//		footRow.getCell(totalDynCostKey + "pay_scale").setValue(scale);
    ...
    protected void initAcct(FilterInfo acctFilter) throws BOSException {
		...
		/**
	 * @description 科目级次显示事件
	 * @author 陈伟
	 * @createDate 2011-8-31
	 * @param e
	 * @throws Exception
	 * @version EAS7.0
	 * @see com.kingdee.eas.fdc.aimcost.client.AbstractAimCostDynInfoUI#actionSplit_actionPerformed(java.awt.event.ActionEvent)
	 */
...
//    	super.tblMain_tableClicked(e);
     ...
     * @throws BOSException
     */
    protected void setData(KDTable table) throws BOSException{
    	...
    	CostAccountInfo cioninfo =null;
    	ContractBillCollection infos =null;
    	ContractBillInfo info =null;
    	BigDecimal changeAmt = FDCHelper.ZERO;
    	BigDecimal conSplitAmt= FDCHelper.ZERO;
		for(int z =0;z<table.getRowCount();z++){
			IRow row = table.getRow(z);		
			if(row.getUserObject()==null){
               cioninfo = (CostAccountInfo)row.getCell("acctNumber").getUserObject();
			   acctId = cioninfo.getId().toString();
			   cion = FDCAimcostClientHelper.getCostAccountCollection(cioninfo ,
						curProject.getId().toString(), acctChidrenMap);
			   //汇总变更数据
				for (int i = 1; i < dymicCols + 1; i++) {
					String year_month = year_from
					...
					for (int k = 0; k < changeTypes.size(); k++) {
                        ...
                        infos =(ContractBillCollection)retValue.get(acctId);
                        if(infos!=null){
            
                           for(int kk=0;kk<infos.size();kk++){
                        	   contractbillId = infos.get(kk).getId().toString();
                        	   BigDecimal childrenchangeAmt = (BigDecimal) changeSplits.get(acctId +contractbillId 
  								     + year_month + typeId);
                        	   if(FDCHelper.toBigDecimal(childrenchangeAmt).compareTo(FDCHelper.ZERO) != 0){
                        		   
                        		   changeAmt = FDCHelper.add(changeAmt,childrenchangeAmt);	
                        	   }
                        	 	   
                           }
                         }
						if(cion!=null){ 
							for(int j=0;j<cion.size();j++){
							    costId = cion.get(j).getId().toString();
							    infos = (ContractBillCollection)retValue.get(costId);
							    if(infos!=null){
							      for(int jj=0;jj<infos.size();jj++){					    
							        contractbillId = infos.get(jj).getId().toString();
							        BigDecimal childrenchangeAmt = (BigDecimal) changeSplits.get(costId +contractbillId 
								     + year_month + typeId);
						            if(FDCHelper.toBigDecimal(childrenchangeAmt).compareTo(FDCHelper.ZERO) != 0){
						              changeAmt = FDCHelper.add(changeAmt,childrenchangeAmt);	
						               }
						...
						if (FDCHelper.toBigDecimal( changeAmt).compareTo(FDCHelper.ZERO) != 0) {
							row.getCell(year_month + "changeAmt" + typeId).setValue( changeAmt );
						  }	
						 changeAmt = FDCHelper.ZERO;
					}
		    	}
				//汇总拆分金额
			    	  ...
			    	  for(int i=0;i<infos.size();i++){
			    		 contractbillId = infos.get(i).getId().toString();
			    		 BigDecimal childrenconSplitAmt = FDCHelper.toBigDecimal(conSplits.get(acctId + contractbillId ));
			    		   if(FDCHelper.toBigDecimal(childrenconSplitAmt).compareTo(FDCHelper.ZERO)!=0){
					        	conSplitAmt = FDCHelper.add(conSplitAmt, childrenconSplitAmt);		        	   
					      }
			    	  } 
			       }
				...
				if(cion!=null){
					for(int j=0;j<cion.size();j++){
						String costId = cion.get(j).getId().toString();
						String conbillId = null;
					    infos = (ContractBillCollection)retValue.get(costId);
					    if(infos!=null){
					       for(int jj=0;jj<infos.size();jj++){
					    	 conbillId  = infos.get(jj).getId().toString();
					    	 BigDecimal childrenconSplitAmt = FDCHelper.toBigDecimal(conSplits.get(costId +conbillId));
					         if(FDCHelper.toBigDecimal(childrenconSplitAmt).compareTo(FDCHelper.ZERO)!=0){
					        	conSplitAmt = FDCHelper.add(conSplitAmt, childrenconSplitAmt);		        	
					        }
					     }
                ...
                if (FDCHelper.toBigDecimal( conSplitAmt).compareTo(FDCHelper.ZERO) != 0){
				    row.getCell("conSplit").setValue(conSplitAmt);
                }
				//下一次统计清0;
				conSplitAmt= FDCHelper.ZERO;
