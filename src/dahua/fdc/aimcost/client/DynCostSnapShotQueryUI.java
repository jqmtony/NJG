...
import com.kingdee.eas.fdc.aimcost.AimDynCostGetFacadeFactory;
...
//		}
	...
	private void gatherDatas() {
		...
		for (int j = 2; j < table.getColumnCount(); j++) {
			...
			amountColumns.add(HAS_HAPPEN + SALABLEAMT);
			amountColumns.add(HAS_HAPPEN + RATE);
			if (tblMain.getColumn(j).getKey().equals(HAS_HAPPEN)
					|| tblMain.getColumn(j).getKey().equals(
							HAS_HAPPEN + SALABLEAMT)
					|| tblMain.getColumn(j).getKey().equals(RATE)) {
				continue;
			}
...
//		Date lastDate = new Date();
		...
		Map map = AimDynCostGetFacadeFactory.getRemoteInstance().getAimDynCost(project.getId().toString());
		aimGetter = (Map) map.get("aimGetter");
		dyGetter = (Map) map.get("dyGetter");

	}
	private Map aimGetter = new HashMap();
	private Map dyGetter = new HashMap();
	...
	private void loadHistoryDate() throws Exception {
		...
		if (al.size() != 0) {
			...
			historyCacheMap = DynCostSnapShotFactory.getRemoteInstance().getHistoryByMonth(transMap);
		}
	}
	...
	private void loadDatas() {
		...
		// acctFilter.getFilterItems().add(new FilterItemInfo("isEnabled", new
		// Integer(1)));
		...
		try {
			...
			FilterInfo prjFilter = new FilterInfo();
			if (new CurProjectInfo().getBOSType().equals(bosType)) {
				prjFilter.getFilterItems().add(new FilterItemInfo("curProject.id", project.getId().toString()));
			}
			AcctAccreditHelper.handAcctAccreditFilter(null, project.getId().toString(), prjFilter);
			//			acctFilter.getFilterItems().remove(itemInfo);
			/*
			 * 注释掉上面一行acctFilter.getFilterItems().remove(itemInfo)，因为如果不注释在执行initAcct(acctFilter)会报错
			 * 因为acctFilter的size原来为4，即存在#3的过滤条件，remove(itemInfo)后，#3就没有了
			 * Added by Owen_wen 2012-09-24
			 */
			this.initAcct(prjFilter);
	...
	private Object[] getHistoryDyncostValue(CostAccountInfo costAcct, Map temp) throws BOSException {
		BigDecimal saleAmt = FDCHelper.ZERO;
		BigDecimal dyncost = FDCHelper.ZERO;
		CostAccountInfo info = null;
		Object[] obj = new Object[2];
		// 子集科目集合
		CostAccountCollection cain = FDCAimcostClientHelper.getCostAccountCollection(costAcct, project.getId().toString(), acctChidrenMap);
		for (int i = 0, length = cain.size(); i < length; i++) {
			info = cain.get(i);
			DynCostSnapShotInfo dcssiTemp = (DynCostSnapShotInfo) temp.get(info.getId().toString());
			dyncost = FDCHelper.add(dyncost, dcssiTemp.getDynCostAmt());
			saleAmt = FDCHelper.add(saleAmt, dcssiTemp.getSalableAmt());
	...
	private Object[] getAimcostValue(CostAccountInfo costAcct, BigDecimal aimCostAmtChildren,
			BigDecimal dynCostAmtChildren, BigDecimal salableAmtChildren,
			BigDecimal soFarHasAmtChildren)
			throws BOSException {

		DynCostSnapShotInfo dcssiChildren = new DynCostSnapShotInfo();
		CostAccountInfo info = null;
		Object[] obj = new Object[4];
		// 子集科目集合
		CostAccountCollection cain = FDCAimcostClientHelper.getCostAccountCollection(costAcct,
				project.getId().toString(), acctChidrenMap);
		for (int i = 0, length = cain.size(); i < length; i++) {
		    info = cain.get(i);
			dcssiChildren = (DynCostSnapShotInfo) cacheMap.get(info.getId().toString());
			if (dcssiChildren != null) {
				aimCostAmtChildren = FDCHelper.add(aimCostAmtChildren, dcssiChildren
						.getAimCostAmt());
				dynCostAmtChildren = FDCHelper.add(dynCostAmtChildren, dcssiChildren
						.getDynCostAmt());
				salableAmtChildren = FDCHelper.add(salableAmtChildren, dcssiChildren
						.getSalableAmt());
				soFarHasAmtChildren = FDCHelper.add(soFarHasAmtChildren, dcssiChildren
						.getSoFarHasAmt());

			}

		}
		obj[0] = aimCostAmtChildren;
		obj[1] = dynCostAmtChildren;
		obj[2] = salableAmtChildren;
		obj[3] = soFarHasAmtChildren;
		return obj;
	}
	/*
	 * 填充各科目节点数据
	...
	private void fillNode(DefaultMutableTreeNode node) throws BOSException, SQLException, EASBizException {
		...
		//成本科目多级改造
		row.getCell("costAccount").setUserObject(costAcct);
		row.getCell("costAccountName").setValue(costAcct.getName());
		// 子集目标成本集合
		BigDecimal aimCostAmtChildren = FDCHelper.ZERO;
		// 子集动态成本集合
		BigDecimal dynCostAmtChildren = FDCHelper.ZERO;
		// 子集动态成本集合
		BigDecimal salableAmtChildren = FDCHelper.ZERO;

		// 子集已发生成本
		BigDecimal soFarHasAmtChildren = FDCHelper.ZERO;

		// 目标成本
		BigDecimal aimCostAmt = FDCHelper.ZERO;
		// 动态成本
		BigDecimal dynCostAmt = FDCHelper.ZERO;
		// 动态成本可售单方
		BigDecimal salableAmt = FDCHelper.ZERO;
		// 已发生成本
		BigDecimal soFarHasAmt = FDCHelper.ZERO;
		
		if (node.isLeaf()) {
//			// ////////////////////////////
//			// 填充最新数据信息
			DynCostSnapShotInfo dcssi = new DynCostSnapShotInfo();
			// 当前显示的是可拆分科目，同时节点是叶子但是科目本身是根的科目，即为需要将下级汇总给本级的科目
			if (this.isShowSplitAcct && !costAcct.isIsLeaf()) {
                 this.children = true;
			} else {
				this.children = false;
			}
			...
			if (this.children) {
				Object[] obj = getAimcostValue(costAcct, aimCostAmtChildren, dynCostAmtChildren,
						salableAmtChildren, soFarHasAmtChildren);
				aimCostAmtChildren = (BigDecimal) obj[0];
				dynCostAmtChildren = (BigDecimal) obj[1];
				salableAmtChildren = (BigDecimal) obj[2];
				soFarHasAmtChildren = (BigDecimal) obj[3];

			}
			dcssi = (DynCostSnapShotInfo) cacheMap.get(acctId);

			if (dcssi != null && dcssi.getAimCostAmt() != null) {

				salableAmt = FDCHelper.add(dcssi.getSalableAmt(), salableAmt);

				soFarHasAmt = FDCHelper.add(dcssi.getSoFarHasAmt(), soFarHasAmt);					

			}

			if (aimGetter != null) {
				aimCostAmt = FDCHelper.toBigDecimal(aimGetter.get(costAcct.getLongNumber()));
			}
			if (dyGetter != null) {
				dynCostAmt = FDCHelper.toBigDecimal(dyGetter.get(costAcct.getLongNumber()));
			}
		
			
			salableAmt = FDCHelper.add(salableAmt, salableAmtChildren);

			 soFarHasAmt = FDCHelper.add(soFarHasAmt, soFarHasAmtChildren);

			if (aimCostAmt != null) {
		
				
				if (aimCostAmt != null) {
					row.getCell(AIM_COST).setValue(FDCHelper.divide(aimCostAmt, transBigDecimal));// 目标成本总成本
				}
				if (aimCostAmt != null && this.aimSaleArea != null) {
					row.getCell(AIM_COST + SALABLEAMT).setValue(
							FDCHelper.divide(aimCostAmt, this.aimSaleArea, 2,
									BigDecimal.ROUND_HALF_UP));//目标成本可售单方--------
																// --目标成本/可售面积
				}
				if (dynCostAmt != null) {
					row.getCell(LAST_DYNAMIC_COST).setValue(
							FDCHelper.divide(dynCostAmt, transBigDecimal, 2,
									BigDecimal.ROUND_HALF_UP));// 最新动态成本总成本
				}
				
				row.getCell(LAST_DYNAMIC_COST + SALABLEAMT).setValue(salableAmt);// 最新动态成本可售单方
				if (soFarHasAmt != null) {
					row.getCell(HAS_HAPPEN).setValue(
							FDCHelper.divide(soFarHasAmt, transBigDecimal, 2,
									BigDecimal.ROUND_HALF_UP));// 已发生成本金额
				}
				if (soFarHasAmt != null && this.dySaleArea != null) {
					row.getCell(HAS_HAPPEN + SALABLEAMT).setValue(
							FDCHelper.divide(soFarHasAmt, this.dySaleArea, 2,
									BigDecimal.ROUND_HALF_UP));//已发生成本可售单方------
																// --金额初以可售面积
				}
				if (soFarHasAmt != null && dynCostAmt != null) {
					BigDecimal tmp = soFarHasAmt.multiply(FDCHelper.ONE_HUNDRED);
					if (FDCHelper.toBigDecimal(dynCostAmt).signum() != 0) {
						row.getCell(HAS_HAPPEN + RATE).setValue(
								FDCHelper.divide(tmp, dynCostAmt, 2, BigDecimal.ROUND_HALF_UP));// 已发生成本比率
					}
				}
				if (salableAmt != null) {
					if (row.getCell(AIM_COST + SALABLEAMT).getValue() != null) {
						row.getCell(DIFF_AIMCOST).setValue(
								salableAmt.subtract((BigDecimal) (row
										.getCell(AIM_COST + SALABLEAMT).getValue())));// 单方差异_与目标成本差异
					}else{
						row.getCell(DIFF_AIMCOST).setValue(salableAmt);
					}
					...
					for (int i = 0; i < this.al.size(); i++) {
						...
						if (temp!=null&&temp.size() != 0 && temp.get(acctId) != null) {
							...
							if (dcssiTemp.getDynCostAmt() != null) {
								row.getCell(al.get(i).toString()).setValue(
										FDCHelper.divide(dcssiTemp.getDynCostAmt(), transBigDecimal, 2, BigDecimal.ROUND_HALF_UP));// 动态成本总成本
							}
							row.getCell(al.get(i).toString() + SALABLEAMT).setValue(dcssiTemp.getSalableAmt());// 动态成本可售单方
						}
						if (i == al.size() - 1) {
							if (dcssiTemp.getSalableAmt() != null){
								row.getCell(DIFF).setValue(
										salableAmt.subtract(dcssiTemp.getSalableAmt()));// 单方差异_与上月成本差异
																						// --
																						// --
																						// --
																						// --
								...
								row.getCell(DIFF).setValue(salableAmt);// 当上月可售单方为空的时候
																		// ，
																		// 最新不为空的时候
																		// ，
																		// 与上月差异应该有数据
...
//			// 区分叶子节点
			...
			if (dcssc != null) {
				DynCostSnapShotInfo dcssi = (DynCostSnapShotInfo) cacheMap.get(acctId);
				if (dcssi != null && dcssi.getAimCostAmt() != null) {
			...
			// modified by zhaoqin for R130709-0062 on 2013/07 start
			// soFarHasAmt = FDCHelper.add(soFarHasAmt, soFarHasAmtChildren);
			// modified by zhaoqin for R130709-0062 on 2013/07 end
			
			if (aimCostAmt != null) {

				row.getCell(AIM_COST).setValue(FDCHelper.divide(aimCostAmt, transBigDecimal));// 目标成本总成本
				if (FDCHelper.toBigDecimal(aimCostAmt).compareTo(FDCHelper.ZERO) != 0)
...
//					Map temp = new HashMap();
					...
					for (int i = 0; i < this.al.size(); i++) {
						...
						if (temp!=null&&temp.size() != 0 && temp.get(acctId) != null) {
							dcssiTemp = (DynCostSnapShotInfo) temp.get(acctId);
							Object[] value = getHistoryDyncostValue(costAcct, temp);
							row.getCell(al.get(i).toString()).setValue(
									FDCHelper.divide(value[0], transBigDecimal, 2, BigDecimal.ROUND_HALF_UP));// 动态成本总成本
							row.getCell(al.get(i).toString() + SALABLEAMT).setValue(value[1]);// 动态成本可售单方
					
						if (i == al.size() - 1) {
							if (dcssiTemp.getSalableAmt() != null){
								row.getCell(DIFF).setValue(salableAmt.subtract(dcssiTemp.getSalableAmt()));// 单方差异_与上月成本差异
							}
						}
						} else {
							row.getCell(DIFF).setValue(salableAmt);// 当上月可售单方为空的时候
					...
					if(item.getPropertyName().equals("snapShotDate")){
						if(item.getCompareType()==CompareType.GREATER_EQUALS){
							if (item.getCompareValue() instanceof Timestamp) {
									this.begindate = new Date(((Timestamp) item.getCompareValue())
											.getTime());
...
//		super.getRowCountFromDB();
	 ...
	 * @throws Exception
	 * @version EAS7.0
	 * @see com.kingdee.eas.fdc.aimcost.client.AbstractDynCostSnapShotQueryUI#actionSplit_actionPerformed(java.awt.event.ActionEvent)
	 */
