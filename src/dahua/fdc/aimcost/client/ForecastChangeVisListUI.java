 ...
 * output class name
 */
public class ForecastChangeVisListUI extends AbstractForecastChangeVisListUI
{
    private static final Logger logger = CoreUIObject.getLogger(ForecastChangeVisListUI.class);
  //获取有权限的组织
    ...
    private IForecastChangeVis IForecastChangeVis = ForecastChangeVisFactory.getRemoteInstance();
     ...
     */
    public ForecastChangeVisListUI() throws Exception
    {
        super();
    }
	...
	private void setSaveActionStatus(ForecastChangeVisInfo info) {
		try {
			refresh(null);
	...
	private ForecastChangeVisInfo getSelectInfo() throws EASBizException, BOSException{
    	checkSelected();
	...
	protected void checkBeforeEditOrRemove(ForecastChangeVisInfo info,String warning) throws BOSException {
	...
	public void checkBeforeAuditOrUnAudit(ForecastChangeVisInfo info,FDCBillStateEnum state, String warning) throws Exception{
		//检查单据是否在工作流中
		FDCClientUtils.checkBillInWorkflow(this,info.getId().toString());
    ...
    protected void prepareUIContext(UIContext uiContext, ActionEvent e) {//提前过滤界面
		...
		if (act.equals(actionAddNew)) {
			...
			try {
				if(ForecastChangeVisFactory.getRemoteInstance().exists("select id where contractNumber.id='"+orderId+"'"))
				{
					MsgBox.showWarning("已存在预估变更签证单，不允许新增！");SysUtil.abort();
				}
				
				uiContext.put("contractInfo", IContractBill.getContractBillInfo(new ObjectUuidPK(orderId)));
