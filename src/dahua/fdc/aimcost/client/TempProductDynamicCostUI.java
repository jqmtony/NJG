 ...
 * output class name
 */
public class TempProductDynamicCostUI extends AbstractProductDynamicCostUI {
	...
	private void fillAimData(IRow row) throws BOSException, SQLException {
		...
		DefaultKingdeeTreeNode proNode = (DefaultKingdeeTreeNode) treeMain
				.getLastSelectedPathComponent();
		if (this.productTypeGetter.getProductTypeSize() > 0 && proNode.isLeaf()) {
	 ...
	 */
	public TempProductDynamicCostUI() throws Exception {
		super();
	}
	...
	private void submitData() throws BOSException, EASBizException {
		for (int i = 0; i < addDynamicCostColl.size(); i++) {
			DynamicCostFactory.getRemoteInstance().submit(
					addDynamicCostColl.get(i));
		}
		addDynamicCostColl.clear();
		for (int i = 0; i < this.tblMain.getRowCount(); i++) {
			IRow row = this.tblMain.getRow(i);
			if (row.getUserObject() != null) {
				...
				if (isEdited) {
					...
					for (int j = 0; j < rowSetting.size(); j++) {
						DynamicCostProductSplitEntryInfo info = rowSetting
								.get(j);
						info.setDynamicCostId(dynamic.getId().toString());
						info.setSplitAmount((BigDecimal) calData.get(info
								.getProduct().getId().toString()));
						if (info.getApportionType() != null) {
							DynamicCostProductSplitEntryFactory
									.getRemoteInstance().addnew(info);
						}
					}
