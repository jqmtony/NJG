...
import com.kingdee.eas.fdc.aimcost.report.companydynamic.CompanyDynamicReportFacadeFactory;
 ...
 * output class name
 */
public class CompanyDynamicReportUI extends AbstractCompanyDynamicReportUI
{
    private static final Logger logger = CoreUIObject.getLogger(CompanyDynamicReportUI.class);
    boolean isQuery = false;
    /**
     * output class constructor
     */
    public CompanyDynamicReportUI() throws Exception
    {
        super();
	...
	protected CommRptBaseConditionUI getQueryDialogUserPanel() throws Exception {
		return new CompanyDynamicReportFilterUI();
	}

	@Override
	protected ICommRptBase getRemoteInstance() throws BOSException {
		return CompanyDynamicReportFacadeFactory.getRemoteInstance();
	}
			...
			public void afterExec(Object result) throws Exception {
				...
				for(Iterator it = set.iterator(); it.hasNext();) {
					...
					if(detailMap != null) {
						...
						addRow.getCell("projectName").setValue(detailMap.get("project"));
						...
						if(curDetailMap != null) {
							IRow addRow = kDTable1.addRow();
							addRow.getCell("projectName").setValue(curDetailMap.get("project"));
							addRow.getCell("saimCost").setValue(curDetailMap.get("aimCost"));
							addRow.getCell("sdynamicCost").setValue(curDetailMap.get("dynamicCost"));
							addRow.getCell("sdiffAmount").setValue(curDetailMap.get("diffAmount"));
							addRow.getCell("sdiffRate").setValue(curDetailMap.get("diffRate"));
	...
	private void addCurrentMonthData(Map map) {
		...
		if(kDTable1.getRowCount() < 1) {
			...
			for(int i = 0; i < kDTable1.getRowCount(); i++) {
				...
				String project = row.getCell("projectName").getValue().toString();
				
				Map detailMap = (Map) map.get(project);
