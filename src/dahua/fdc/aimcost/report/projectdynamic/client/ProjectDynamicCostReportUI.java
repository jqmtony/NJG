...
import com.kingdee.eas.fdc.aimcost.report.projectdynamic.ProjectDynamicCostFacadeFactory;
...
public class ProjectDynamicCostReportUI extends AbstractProjectDynamicCostReportUI
{
	...
	protected CommRptBaseConditionUI getQueryDialogUserPanel() throws Exception {
		return new ProjectDynamicCostFilertUI();
	}
	...
	protected void query() {
		...
		kDTable1.removeRows();
//		ReportUtils.mergerTable(kDTable1, 
//				new String[]{"curProject"}, 
//				new String[]{"curProject"}
//		);
	}
	...
	public void tableDataRequest(KDTDataRequestEvent arg0) {
		...
		dialog.setLongTimeTask(new ILongTimeTask() {
			...
			public void afterExec(Object result) throws Exception {
				...
				Map preMonthMap = (Map) ((RptParams) result).getObject("preMonthMap");
				Map curMonthMap = (Map) ((RptParams) result).getObject("curMonthMap");
				...
				for(Iterator it = set.iterator(); it.hasNext();) {
					...
					if(detailMap != null) {
						...
						addRow.getCell("costAccountName").setValue(detailMap.get("costAccName"));
						addRow.getCell("costAccountNumber").setValue(detailMap.get("costAccNum"));
						...
						if(curDetailMap != null) {
							IRow addRow = kDTable1.addRow();
							addRow.getCell("costAccountName").setValue(curDetailMap.get("costAccName"));
							addRow.getCell("costAccountNumber").setValue(curDetailMap.get("costAccNum"));
							addRow.getCell("saimCost").setValue(curDetailMap.get("aimCost"));
							addRow.getCell("sdynamicCost").setValue(curDetailMap.get("dynamicCost"));
							addRow.getCell("sdiffAmount").setValue(curDetailMap.get("diffAmount"));
							addRow.getCell("sdiffRate").setValue(curDetailMap.get("diffRate"));
...
//				addPreMOnthData(preMonthMap);
//				addCurrentMonthData(curMonthMap);
			}
		});
		dialog.show();
		isQuery = false;
	...
	private void addPreMOnthData(Map map) {
		Set keySet = map.keySet();
		for(Iterator it = keySet.iterator(); it.hasNext();) {
			Map detailMap = (Map) map.get(it.next().toString());
			IRow addRow = kDTable1.addRow();
			addRow.getCell("costAccountName").setValue(detailMap.get("costAccName"));
			addRow.getCell("costAccountNumber").setValue(detailMap.get("costAccNum"));
			addRow.getCell("faimCost").setValue(detailMap.get("aimCost"));
	...
	private void addCurrentMonthData(Map map) {
		Set keySet = map.keySet();
		if(kDTable1.getRowCount() < 1) {
			for(Iterator it = keySet.iterator(); it.hasNext();) {
				...
				addRow.getCell("costAccountName").setValue(detailMap.get("costAccName"));
				addRow.getCell("costAccountNumber").setValue(detailMap.get("costAccNum"));
				addRow.getCell("saimCost").setValue(detailMap.get("aimCost"));
				addRow.getCell("sdynamicCost").setValue(detailMap.get("dynamicCost"));
				addRow.getCell("sdiffAmount").setValue(detailMap.get("diffAmount"));
				addRow.getCell("sdiffRate").setValue(detailMap.get("diffRate"));
			}
		} else {
			for(int i = 0; i < kDTable1.getRowCount(); i++) {
				IRow row = kDTable1.getRow(i);
				String accNumer = row.getCell("costAccountNumber").getValue().toString();
				
				Map detailMap = (Map) map.get(accNumer);
				row.getCell("saimCost").setValue(detailMap.get("aimCost"));
				row.getCell("sdynamicCost").setValue(detailMap.get("dynamicCost"));
				row.getCell("sdiffAmount").setValue(detailMap.get("diffAmount"));
				row.getCell("sdiffRate").setValue(detailMap.get("diffRate"));
			}
		}
	}
}