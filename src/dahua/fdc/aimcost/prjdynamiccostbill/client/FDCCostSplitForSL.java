...
public class FDCCostSplitForSL
{
	 ...
	 * 创建时间：2006-10-19 <p>
	 */    
    public void apptAmount(IObjectCollection allEntrys,OtherSplitNewEntryInfo curEntry){
    	...
    	if(costSplitType==null || costSplitType.equals(CostSplitTypeEnum.MANUALSPLIT)){
			...
			if(amount==null || amount.compareTo(FDCHelper.ZERO)==0){
				...
				for(int i=curIndex+1; i<allEntrys.size(); i++){
					OtherSplitNewEntryInfo entry = (OtherSplitNewEntryInfo)allEntrys.getObject(i);
    ...
    private void apptAmountBotUp(IObjectCollection allEntrys,int curIndex){
		...
		OtherSplitNewEntryInfo curEntry=(OtherSplitNewEntryInfo)allEntrys.getObject(curIndex);		
	 ...
	 * 创建时间：2006-12-26 <p>
	 */
    private void adjustAmount(IObjectCollection allEntrys,OtherSplitNewEntryInfo curEntry){
    	OtherSplitNewEntryInfo entry=curEntry;    	
		...
		for(int i=index+1; i<allEntrys.size(); i++){				
			OtherSplitNewEntryInfo tempEntry=(OtherSplitNewEntryInfo)allEntrys.getObject(i);
    	...
    	//对修正后的数据进行产品拆分
    	for(int i=index+1;i<allEntrys.size();i++){
    		OtherSplitNewEntryInfo topEntry=(OtherSplitNewEntryInfo) allEntrys.getObject(i);
...
//        			apptAmountAddlAcct(allEntrys, i);	    			
    ...
    private void adjustAmountProject(IObjectCollection allEntrys, int index){
		OtherSplitNewEntryInfo curEntry =(OtherSplitNewEntryInfo)allEntrys.getObject(index);		
		...
		for(int i=index+1; i<allEntrys.size(); i++){		
			entry=(OtherSplitNewEntryInfo)allEntrys.getObject(i);
		...
		if(idx>0 && curAmount!=null&&curAmount.compareTo(amountTotal)!=0){	
			entry=(OtherSplitNewEntryInfo)allEntrys.getObject(idx);
	...
	private void adjustBotupAmt(IObjectCollection allEntrys,
			OtherSplitNewEntryInfo curEntry, int index, int level) {
		//末级拆分修正
		BigDecimal total=FDCHelper.ZERO;
		BigDecimal maxAmount=FDCHelper.ZERO;
		int max=index;
		for(int i=index+1; i<allEntrys.size(); i++){		
			OtherSplitNewEntryInfo myEntry=(OtherSplitNewEntryInfo)allEntrys.getObject(i);
		...
		if(max>index){
			OtherSplitNewEntryInfo myEntry=(OtherSplitNewEntryInfo)allEntrys.getObject(max);
			BigDecimal amt=FDCHelper.toBigDecimal(myEntry.getAmount());
			amt=FDCHelper.toBigDecimal(curEntry.getAmount()).subtract(total).add(amt);
			myEntry.setAmount(amt);
			//汇总到非明细
			for(int i=index+1; i<allEntrys.size(); i++){		
				OtherSplitNewEntryInfo myEntry1=(OtherSplitNewEntryInfo)allEntrys.getObject(i);
				...
				if(!myEntry1.getCostAccount().getCurProject().isIsLeaf()){
					...
					for(int j=i+1;j<allEntrys.size();j++){
						OtherSplitNewEntryInfo myEntry2=(OtherSplitNewEntryInfo)allEntrys.getObject(j);
    ...
    private void apptAmountBotUp(IObjectCollection allEntrys,int curIndex, BigDecimal ratio, BigDecimal parApportionTotal, 
		...
		OtherSplitNewEntryInfo topEntry =(OtherSplitNewEntryInfo)allEntrys.getObject(curIndex);
...
//		amountTotal=amountTotal.setScale(2, BigDecimal.ROUND_HALF_UP);
	 ...
	 * 创建时间：2006-11-07 <p>
	 */
    public void totApptValue(IObjectCollection allEntrys,OtherSplitNewEntryInfo curEntry){
    	int curIndex=allEntrys.indexOf(curEntry);
    	if(curIndex==-1){
    		return;
    	}
	 ...
	 * 创建时间：2006-11-30 <p>
	 */
	public static boolean isProdSplitLeaf(OtherSplitNewEntryInfo entry){
	 ...
	 * 创建时间：2006-11-30 <p>
	 */
    public boolean isProdSplitParent(OtherSplitNewEntryInfo entry){		
		boolean isTrue=false;
	 ...
	 * 创建时间：2006-12-28 <p>
	 */
    public static boolean isChildProjSameAcct(OtherSplitNewEntryInfo entry, OtherSplitNewEntryInfo parent){		
		boolean isTrue=false;
    ...
    private void totApptValueTopDown(IObjectCollection allEntrys,int topIndex){
    	OtherSplitNewEntryInfo topEntry =(OtherSplitNewEntryInfo)allEntrys.getObject(topIndex);  
