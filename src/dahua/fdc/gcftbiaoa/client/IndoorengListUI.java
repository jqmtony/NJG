 ...
 * output class name
 */
public class IndoorengListUI extends AbstractIndoorengListUI
{
	private static final long serialVersionUID = 1L;
    private static final Logger logger = CoreUIObject.getLogger(IndoorengListUI.class);
	...
	public void actioneRevise_actionPerformed(ActionEvent e) throws Exception {
		...
		com.kingdee.eas.fdc.gcftbiaoa.IIndooreng remoteInstance = com.kingdee.eas.fdc.gcftbiaoa.IndoorengFactory.getRemoteInstance();
		IndoorengInfo Indoorenginfo = remoteInstance.getIndoorengInfo(new ObjectUuidPK(id),getSelectors());   //getSelectors 修订后，不点保存，不显示F7字段的值
		if(!Indoorenginfo.isLasted()){
		...
		uiContext.put("IndoorengInfo", Indoorenginfo);
...
//	}
	 ...
	 private void checkBeforeAuditOrUnAudit(IndoorengInfo Indoorenginfo , FDCBillStateEnum aduit,String Waring ) {
			//检查单据是否在审核中
			FDCClientUtils.checkBillInWorkflow(this, Indoorenginfo.getId().toString());
			boolean b = Indoorenginfo!=null&&Indoorenginfo.getState()!=null&&Indoorenginfo.getState().equals(aduit);
			if(!b){
	 ...
	 public void actionAduit_actionPerformed(ActionEvent e) throws Exception {
		 ...
		 IndoorengInfo Indoorenginfo = remoteInstance.getIndoorengInfo(new ObjectUuidPK(id));
//		 			if(!Indoorenginfo.isLasted()){
//		 				MsgBox.showWarning("不是最新版，不能审核");
//		 				abort();
//		 			}
		 if(Indoorenginfo.getState().equals(FDCBillStateEnum.SAVED)){
			 MsgBox.showWarning("请先提交，再审核");
			 abort();
		 }
		 checkBeforeAuditOrUnAudit(Indoorenginfo, FDCBillStateEnum.SUBMITTED, CANTAUDIT);
//		 super.actionAduit_actionPerformed(e);
		 IndoorengFactory.getRemoteInstance().actionAudit(Indoorenginfo);
	...
	public void actionUnAduit_actionPerformed(ActionEvent e) throws Exception {
		checkSelected();//判断是否选中
		 ...
		 IIndooreng remoteInstance = IndoorengFactory.getRemoteInstance();
		 IndoorengInfo Indoorenginfo = remoteInstance.getIndoorengInfo(new ObjectUuidPK(id));
		 checkBeforeAuditOrUnAudit(Indoorenginfo, FDCBillStateEnum.AUDITTED, CANTUNAUDIT);
//		 super.actionUnAduit_actionPerformed(e);
		 IndoorengFactory.getRemoteInstance().actionUnAudit(Indoorenginfo);
		 refresh(null);
	}
