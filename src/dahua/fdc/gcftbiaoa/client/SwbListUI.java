 ...
 * output class name
 */
public class SwbListUI extends AbstractSwbListUI
{
	private static final long serialVersionUID = 1L;
    private static final Logger logger = CoreUIObject.getLogger(SwbListUI.class);
     ...
     */
    public SwbListUI() throws Exception
    {
        super();
	...
	public void actionRevise_actionPerformed(ActionEvent e) throws Exception {
		...
		com.kingdee.eas.fdc.gcftbiaoa.ISwb remoteInstance = com.kingdee.eas.fdc.gcftbiaoa.SwbFactory.getRemoteInstance();
		SwbInfo Swbinfo = remoteInstance.getSwbInfo(new ObjectUuidPK(id),getSelectors());   //getSelectors 修订后，不点保存，不显示F7字段的值
		if(!Swbinfo.isLasted()){
	 ...
	 private void checkBeforeAuditOrUnAudit(SwbInfo Swbinfo , FDCBillStateEnum aduit,String Waring ) {
			//检查单据是否在审核中
			FDCClientUtils.checkBillInWorkflow(this, Swbinfo.getId().toString());
			boolean b = Swbinfo!=null&&Swbinfo.getState()!=null&&Swbinfo.getState().equals(aduit);
			if(!b){
...
//		 				abort();
//		 			}
		 if(Swbinfo.getState().equals(FDCBillStateEnum.SAVED)){
			 MsgBox.showWarning("请先提交，再审核");
			 abort();
		 }
		 checkBeforeAuditOrUnAudit(Swbinfo, FDCBillStateEnum.SUBMITTED, CANTAUDIT);
	 ...
	 public void actionUnAudit_actionPerformed(ActionEvent e) throws Exception {
		 ...
		 ISwb remoteInstance = SwbFactory.getRemoteInstance();
		 SwbInfo Swbinfo = remoteInstance.getSwbInfo(new ObjectUuidPK(id));
		 checkBeforeAuditOrUnAudit(Swbinfo, FDCBillStateEnum.AUDITTED, CANTUNAUDIT);
