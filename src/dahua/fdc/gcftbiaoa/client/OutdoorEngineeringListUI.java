...
import com.kingdee.eas.fdc.gcftbiaoa.IOutdoorEngineering;
 ...
 * output class name
 */
public class OutdoorEngineeringListUI extends AbstractOutdoorEngineeringListUI
{
	private static final long serialVersionUID = 1L;
    private static final Logger logger = CoreUIObject.getLogger(OutdoorEngineeringListUI.class);
     ...
     */
    public OutdoorEngineeringListUI() throws Exception
    {
        super();
    }
	...
	public void actionRevise_actionPerformed(ActionEvent e) throws Exception {
		...
		com.kingdee.eas.fdc.gcftbiaoa.IOutdoorEngineering remoteInstance = com.kingdee.eas.fdc.gcftbiaoa.OutdoorEngineeringFactory.getRemoteInstance();
		OutdoorEngineeringInfo OutdoorEngineeringinfo = remoteInstance.getOutdoorEngineeringInfo(new ObjectUuidPK(id),getSelectors());   //getSelectors 修订后，不点保存，不显示F7字段的值
		if(!OutdoorEngineeringinfo.isLasted()){
		...
		uiContext.put("OutdoorEngineeringInfo", OutdoorEngineeringinfo);
	 ...
	 private void checkBeforeAuditOrUnAudit(OutdoorEngineeringInfo OutdoorEngineeringinfo , FDCBillStateEnum aduit,String Waring ) {
			//检查单据是否在审核中
			FDCClientUtils.checkBillInWorkflow(this, OutdoorEngineeringinfo.getId().toString());
			boolean b = OutdoorEngineeringinfo!=null&&OutdoorEngineeringinfo.getState()!=null&&OutdoorEngineeringinfo.getState().equals(aduit);
			if(!b){
	 ...
	 public void actionAduit_actionPerformed(ActionEvent e) throws Exception {
		 checkSelected();//判断是否选中
		 String id = getSelectedKeyValue();
		 IOutdoorEngineering remoteInstance = OutdoorEngineeringFactory.getRemoteInstance();
		 OutdoorEngineeringInfo OutdoorEngineeringinfo = remoteInstance.getOutdoorEngineeringInfo(new ObjectUuidPK(id));
//		 			if(!OutdoorEngineeringinfo.isLasted()){
//		 				MsgBox.showWarning("不是最新版，不能审核");
//		 				abort();
//		 			}
		 if(OutdoorEngineeringinfo.getState().equals(FDCBillStateEnum.SAVED)){
			 MsgBox.showWarning("请先提交，再审核");
			 abort();
		 }
		 checkBeforeAuditOrUnAudit(OutdoorEngineeringinfo, FDCBillStateEnum.SUBMITTED, CANTAUDIT);
	 ...
	 public void actionUnAudit_actionPerformed(ActionEvent e) throws Exception {
		 ...
		 IOutdoorEngineering remoteInstance = OutdoorEngineeringFactory.getRemoteInstance();
		 OutdoorEngineeringInfo OutdoorEngineeringinfo = remoteInstance.getOutdoorEngineeringInfo(new ObjectUuidPK(id));
		 checkBeforeAuditOrUnAudit(OutdoorEngineeringinfo, FDCBillStateEnum.AUDITTED, CANTUNAUDIT);
