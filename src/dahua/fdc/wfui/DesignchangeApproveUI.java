...
import com.kingdee.eas.fdc.contract.client.ChangeAuditRequestEditUI;
...
//    	kDTable1.getRow(kDTable1.getRowCount()-1).setHeight(150);
    	...
    	sb.append(" group by  ChangeAB.FCurProjectName ,ChangeAB.FNumber ,ChangeAB.Fname ,BaseU.Fname_l2,ChangeAB.Freadesc  ,entry.FChangeContent,u.Fname_l2,u.Fname_l2,"); 
    	sb.append(" to_char(ChangeAB.FCreateTime,'yyyy-mm-dd'),entry.FIsBack,ChangeAB.CFQuality,ChangeAB.CFTimeLi,ChangeAB.CFSale,ChangeAB.CFCost,ChangeAB.CFSFEJJD,ChangeAB.CFXSCN ,ChangeAB.CFBJZB "); 
    	...
    	while(rowset.next()){
    		...
    		if(quality.equals("提高"))
    			this.kDTable1.getCell(6, 3).setValue(Boolean.TRUE);
    		if(quality.equals("降低"))
    			this.kDTable1.getCell(6, 5).setValue(Boolean.TRUE);
    		if(quality.equals("无影响"))
    			this.kDTable1.getCell(6, 7).setValue(Boolean.TRUE);
    		//填充工期
    		String TimeLi = rowset.getString("工期")!=null?rowset.getString("工期"):"";
    		if(TimeLi.equals("缩短"))
    			this.kDTable1.getCell(7, 3).setValue(Boolean.TRUE);
    		if(TimeLi.equals("延长"))
    			this.kDTable1.getCell(7, 5).setValue(Boolean.TRUE);
...
//    	}
	...
	protected void yd_actionPerformed(ActionEvent e) throws Exception {
		...
		IUIWindow uiWindow = UIFactory.createUIFactory(UIFactoryName.MODEL).create(ChangeAuditRequestEditUI.class.getName(),uiContext,null,OprtState.VIEW);
		uiWindow.show();
	}
