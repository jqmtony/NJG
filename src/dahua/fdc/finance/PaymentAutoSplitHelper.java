	...
	public static void autoSplitAmt(
		...
		for (Iterator iter = entrys.iterator(); iter.hasNext();) {
			...
			if (isSettle) {
				...
				BigDecimal amount = FDCHelper.ZERO;
				if (FDCHelper.compareTo(item.getSplitedCostAmt(), FDCHelper.ZERO) == 0) {
					amount = FDCHelper.toBigDecimal(item.getShouldPayAmt()).add(FDCHelper.toBigDecimal(item.getAmount())).subtract(
							FDCHelper.toBigDecimal(item.getSplitedPayedAmt()));
				} else {
					amount = FDCHelper.toBigDecimal(item.getSplitedCostAmt()).add(FDCHelper.toBigDecimal(item.getAmount())).subtract(
							FDCHelper.toBigDecimal(item.getSplitedPayedAmt()));
				}
				item.put("distributeAmt", amount);
			}
...
//					}
					...
					if(currentProductFloorEntry!=null){
						...
						//最后一项拆分时
						if(judgeLastProductSplit.size()!=0 && item!=null&&judgeLastProductSplit.get(judgeLastProductSplit.size()-1).equals(new Integer(item.getSeq()))){
