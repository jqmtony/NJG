...
//		deductAccountMap = (Map)param.get("deductAccountMap");
	...
	protected void initSplitBillInfos(Map dataMap, Set paymentBillIds) throws BOSException {
		...
		builder
				.appendSql(" and split.fcreatetime < (select fcreatetime from t_fnc_paymentsplit where fpaymentbillid=? and fstate!='9INVALID') \n");
		...
		builder.appendSql(" and split.fisinvalid=0 \n");
		builder
				.appendSql(" and split.fcreatetime < (select fcreatetime from t_fnc_paymentNoCostSplit where fpaymentbillid=? and fstate!='9INVALID') \n");//根据制单日期 
...
//				colls.removeObject(info);
	...
	private void initSplitInfos(Map dataMap, Set paymentBillIds) throws BOSException {
		if(isWorkLoadVoucher(dataMap)){
			...
			view.getSelector().add("paymentBill.amount");
			view.getSelector().add("paymentBill.localAmt");
			...
			view.getSelector().add("paymentBill.fdcPayType.name");
