...
//					}
//				}
				if(FDCUtils.isContractBill(null, paymentSplitInfo.getPaymentBill().getContractBillId())&& paymentSplitInfo.getPaymentBill()!=null && paymentSplitInfo.getPaymentBill().getFdcPayType()!=null){
					if ("预付款".equals(paymentSplitInfo.getPaymentBill().getFdcPayType().getName())
							&& FDCHelper.compareTo(paymentSplitInfo.getPaymentBill().getAmount(), FDCHelper.ZERO) > 0) {
						coll.addObject(createAdvanceCostEntry(paymentSplitInfo.getPaymentBill()));
					}
				}
			}
			...
			if(paymentNoCostSplitInfo!=null&&paymentNoCostSplitInfo.getPaymentBill()!=null&&paymentNoCostSplitInfo.getPaymentBill().getContractBillId()!=null){
				...
				//					}
				//				}
				if (FDCUtils.isContractBill(null, paymentNoCostSplitInfo.getPaymentBill().getContractBillId())
						&& paymentNoCostSplitInfo.getPaymentBill() != null
						&& paymentNoCostSplitInfo.getPaymentBill().getFdcPayType() != null) {
					if ("预付款".equals(paymentNoCostSplitInfo.getPaymentBill().getFdcPayType().getName())
							&& FDCHelper.toBigDecimal(paymentNoCostSplitInfo.getPaymentBill().getAmount()).signum() > 0) {
						coll.addObject(createAdvanceCostEntry(paymentNoCostSplitInfo.getPaymentBill()));
					}
				}
			}
...
//		}
		voucherEntryInfo.setAmount(paymentBillInfo.getAmount());
		voucherEntryInfo.setLocateAmount(paymentBillInfo.getLocalAmt());
