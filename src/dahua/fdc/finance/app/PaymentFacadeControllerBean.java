...
//	在fetchPayData()里需要用到合同ID集合，为了不至于再循环遍历一次 contractBills 抽取为全局变量 by Cassiel_peng 
    ...
    protected Map _fetchInitData(Context ctx, Map paramMap)throws BOSException, EASBizException
    {
		...
		existFilter.getFilterItems().add(new FilterItemInfo("fdcPayReqID", fdcPayReqID));
		if (paramMap.get("createTime") != null) {
			existFilter.getFilterItems().add(new FilterItemInfo("createTime", paramMap.get("createTime"), CompareType.LESS));
		} else if (billInfo != null && billInfo.getCreateTime() != null) {
			existFilter.getFilterItems().add(new FilterItemInfo("createTime", billInfo.getCreateTime(), CompareType.LESS));
		}
...
//		selector.add("curProject.fullOrgUnit.name");
		...
		selector.add("contractType.id");//加入所需要的合同类型值
		selector.add("contractType.isLeaf");
		selector.add("contractType.level");
		selector.add("contractType.number");
		selector.add("contractType.longnumber");
		selector.add("contractType.name");
		selector.add("contractType.isRefProgram");
...
//		selector.add("partB.*");
		...
		// 上次累计申请
		// 应该用CompareType.LESS，而不是CompareType.LESS_EQUALS Added by Owen_wen 2011-09-13
		// by tim_gao 这里改回到CompareType.LESS_EQUALS 因为如果不用等于，在paymentbill
		// 的updateDynamicValue方法
		// 中取不到 本合同对应原付款申请单中累计申请额最大的
		view = new EntityViewInfo();
		filter = new FilterInfo();
		FilterItemCollection items = filter.getFilterItems();
		items.add(new FilterItemInfo("createTime", info.getCreateTime(),
		...
		//			// 包含最后这笔
		//			filter.getFilterItems().add(new FilterItemInfo("createTime", lastBillInfo.getCreateTime(), CompareType.LESS_EQUALS));
		//		} else {
...
//		filter.getFilterItems().add(new FilterItemInfo("createTime", info.getCreateTime(),CompareType.LESS));
		...
		filter.getFilterItems().add(new FilterItemInfo("CreateTime", info.getCreateTime(), CompareType.LESS_EQUALS));
		view.setFilter(filter);
		PayRequestBillCollection reqCols = PayRequestBillFactory.getLocalInstance(ctx).getPayRequestBillCollection(view);
		HashSet idSet = new HashSet();
		HashMap reqInvoiceMap = new HashMap();
		//by tim_gao加入原币的
		HashMap reqInvoiceOriMap = new HashMap();
		HashMap payInvoiceMap = new HashMap();
		HashMap payInvoiceOriMap = new HashMap();
		
		BigDecimal allInvoiceAmt = FDCHelper.ZERO;
		//by tim_gao 加入原币
		BigDecimal allInvoiceOriAmt = FDCHelper.ZERO;
		
		//by tim_gao 不加如本次付款申请单的发票本币金额
		BigDecimal allInvoiceAmtWithOutThis = FDCHelper.ZERO;
		//by tim_gao 不加如本次付款申请单的发票原币金额
		BigDecimal allInvoiceOriAmtWithOutThis = FDCHelper.ZERO;
		boolean isadd = false;
		//by tim_gao 本付款单ID
		if(null==id){
			if(null!=billInfo&&null!=billInfo.getId()){
				id = billInfo.getId().toString();
			}else{
				if(null!=lastBillInfo&&null!=lastBillInfo.getId()){
					//后面要用ID 为null 校验
					id = lastBillInfo.getId().toString();
					isadd = true;
				}
			}
		}
		if (reqCols != null && reqCols.size() >0) {
			for (Iterator it = reqCols.iterator(); it.hasNext();) {
				PayRequestBillInfo reqInfo = (PayRequestBillInfo)it.next();
				idSet.add(reqInfo.getId().toString());
				reqInvoiceMap.put(reqInfo.getId().toString(), reqInfo.getInvoiceAmt());
				//原币
				reqInvoiceOriMap.put(reqInfo.getId().toString(), reqInfo.getInvoiceOriAmt());
			}
		}
		...
		if (idSet.size() > 0) {
			...
			view.getSelector().add("paymentbill.fdcPayReqID");
			filter.getFilterItems().add(new FilterItemInfo("paymentbill.fdcPayReqID", idSet, CompareType.INCLUDE));
			view.setFilter(filter);
			FDCPaymentBillCollection fdcPayCols = FDCPaymentBillFactory.getLocalInstance(ctx).getFDCPaymentBillCollection(view);
			if (fdcPayCols != null && fdcPayCols.size() > 0) {
				for (Iterator it = fdcPayCols.iterator(); it.hasNext();) {
					FDCPaymentBillInfo fdcPayInfo = (FDCPaymentBillInfo) it.next();
					if (fdcPayInfo.getInvoiceAmt() != null) {
						
					if (!payInvoiceMap.containsKey(fdcPayInfo.getPaymentBill().getFdcPayReqID())) {
						payInvoiceMap.put(fdcPayInfo.getPaymentBill().getFdcPayReqID(), fdcPayInfo.getInvoiceAmt());
						
					} else {
						BigDecimal invoiceAmt = (BigDecimal)payInvoiceMap.get(fdcPayInfo.getPaymentBill().getFdcPayReqID());
						
						if (invoiceAmt == null) {
							invoiceAmt = FDCHelper.ZERO;
						}
						invoiceAmt = FDCHelper.add(invoiceAmt, fdcPayInfo.getInvoiceAmt());
						payInvoiceMap.put(fdcPayInfo.getPaymentBill().getFdcPayReqID(), invoiceAmt);
					}
					//by tim_gao 本币总和和不加本次的
					allInvoiceAmt = FDCHelper.add(allInvoiceAmt, fdcPayInfo.getInvoiceAmt());
					if(null!=id ){
						if(isadd||!(id.equals(fdcPayInfo.getPaymentBill().getId().toString()))){
							allInvoiceAmtWithOutThis = FDCHelper.add(allInvoiceAmtWithOutThis, fdcPayInfo.getInvoiceAmt());
						}
					}else{
						//当id 没有只能考虑为付款单与付款申请一一对应
						if(!fdcPayReqID.equals(fdcPayInfo.getPaymentBill().getFdcPayReqID())){
							allInvoiceAmtWithOutThis = FDCHelper.add(allInvoiceAmtWithOutThis, fdcPayInfo.getInvoiceAmt());
						}
					}
					...
					if (fdcPayInfo.getInvoiceOriAmt() != null) {
						
						if (!payInvoiceOriMap.containsKey(fdcPayInfo.getPaymentBill().getFdcPayReqID())) {
							payInvoiceOriMap.put(fdcPayInfo.getPaymentBill().getFdcPayReqID(), fdcPayInfo.getInvoiceOriAmt());
						} else {
							BigDecimal invoiceOriAmt = (BigDecimal)payInvoiceOriMap.get(fdcPayInfo.getPaymentBill().getFdcPayReqID());
							
							if (invoiceOriAmt == null) {
								invoiceOriAmt = FDCHelper.ZERO;
							}
							invoiceOriAmt = FDCHelper.add(invoiceOriAmt, fdcPayInfo.getInvoiceOriAmt());
							payInvoiceOriMap.put(fdcPayInfo.getPaymentBill().getFdcPayReqID(), invoiceOriAmt);
						}
						//by tim_gao 原币总和和不加本次的
						allInvoiceOriAmt = FDCHelper.add(allInvoiceOriAmt, fdcPayInfo.getInvoiceOriAmt());
						
						if(null!=id ){
							if(isadd||!(id.equals(fdcPayInfo.getPaymentBill().getId().toString()))){
								allInvoiceOriAmtWithOutThis = FDCHelper.add(allInvoiceOriAmtWithOutThis, fdcPayInfo.getInvoiceOriAmt());
							}
						}else{
							//当id 没有只能考虑为付款单与付款申请一一对应
							if(!fdcPayReqID.equals(fdcPayInfo.getPaymentBill().getFdcPayReqID())){
		...
		//如果中间表没有,那就是没有发票,所以去掉了
		
//		if(null==allInvoiceAmt||FDCHelper.ZERO.equals(allInvoiceAmt)){
//			for (Iterator it = reqInvoiceMap.keySet().iterator(); it.hasNext();) {
//				String key = (String) it.next();
//				if (payInvoiceMap.containsKey(key)) {
//					allInvoiceAmt = FDCHelper.add(allInvoiceAmt, payInvoiceMap.get(key));
//				} else {
//					allInvoiceAmt = FDCHelper.add(allInvoiceAmt, reqInvoiceMap.get(key));
//				}
//			}
//		}
//		// by tim_gao 原币
//		if(null==allInvoiceOriAmt||FDCHelper.ZERO.equals(allInvoiceOriAmt)){
//			for (Iterator it = reqInvoiceOriMap.keySet().iterator(); it.hasNext();) {
//				String key = (String) it.next();
//				if (payInvoiceOriMap.containsKey(key)) {
//					allInvoiceOriAmt = FDCHelper.add(allInvoiceOriAmt, payInvoiceOriMap.get(key));
//				} else {
//					allInvoiceOriAmt = FDCHelper.add(allInvoiceOriAmt, reqInvoiceOriMap.get(key));
//				}
//			}
//		}
		
		initMap.put("FDCPaymentAllInvoicAmt", allInvoiceAmt);
		//by tim_gao 原币
		initMap.put("FDCPaymentAllInvoicOriAmt", allInvoiceOriAmt);
		//by tim_gao 没有本次的
		initMap.put("FDCPayAllInvoicOriWithoutThis", allInvoiceOriAmtWithOutThis);
		initMap.put("FDCPayAllInvoicWithoutThis", allInvoiceAmtWithOutThis);
//		FDCSQLBuilder builder=new FDCSQLBuilder(ctx);
//		builder.appendSql(" select SUM(ISNULL(FPaymentInvoiceAmt, FInvoiceAmt)) FAllInvoiceAmt from (  \n");
//		builder.appendSql(" select payreq.fid, payreq.FInvoiceAmt, sum(fdcPay.FInvoiceAmt) FPaymentInvoiceAmt from T_CON_PayRequestBill payreq  \n");
//		builder.appendSql(" left join T_CAS_PaymentBill payment on payreq.fid = payment.FFdcPayReqID  \n");
//		builder.appendSql(" left join T_FNC_FDCPaymentBill fdcPay on fdcPay.FPaymentBillId = payment.FID  \n");
//		builder.appendSql("	where payreq.FID in ? \n");
//		builder.appendSql("	group by payreq.fid, payreq.FInvoiceAmt \n");
//		builder.appendSql("	) T \n");
...
//			BigDecimal paidAmt = rs.getBigDecimal("FPayedAmt");
	...
	/**
	 * 在工作流中通过付款单ID获得合同类型 - R140611-0055
	 * 
	 ...
	 * @date 2015/03/24
	 */
	protected IObjectValue _getContractType(Context ctx, String contractId)
			throws BOSException, EASBizException {
		if(StringUtils.isEmpty(contractId))
			return null;
		BOSUuid billId = BOSUuid.read(contractId);
		IObjectPK pk = new ObjectUuidPK(billId);
		SelectorItemCollection selector = new SelectorItemCollection();
		selector.add("contractType.*");
		if ((new ContractBillInfo().getBOSType()).equals(billId.getType())) {
			ContractBillInfo info = 
				ContractBillFactory.getLocalInstance(ctx).getContractBillInfo(pk, selector);
			return null == info ? null : info.getContractType();
		} else if((new ContractWithoutTextInfo().getBOSType()).equals(billId.getType())) {
			ContractWithoutTextInfo info = 
				ContractWithoutTextFactory.getLocalInstance(ctx).getContractWithoutTextInfo(pk, selector);
			return null == info ? null : info.getContractType();
		}
		return null;
	}
