    	...
    	if(fdcPeriod.getId()==null){
	    	...
	    	try {
				if (rowSet.size() > 0) {
					...
					// 不存在时，新增一期间，否则中断by hpw
					FDCBudgetPeriodInfo budgetPeriod = FDCBudgetPeriodInfo.getPeriod(fdcPeriod.getYear(), fdcPeriod.getMonth(), false);
					fdcPeriod.setId(BOSUuid.create(budgetPeriod.getBOSType()));
...
//		filter.setMaskString("#0 and #1 and #2 and ((#3 and #4) or ((#5 or #6) and #7))) and #8");	-by neo
	...
	protected void _updateMonthBudget(Context ctx, BOSUuid billId)
		...
		logger.info("头" + depMonBudgetAcctInfo.getNumber());
		for(Iterator iter=depMonBudgetAcctInfo.getEntrys().iterator();iter.hasNext();){
			...
			logger.info(depMonEntry.getCost());
			logger.info(depMonEntry.getAmount());
