...
public abstract class AbstractPaymentSplitFacadeControllerBean extends AbstractBizControllerBean implements PaymentSplitFacadeController
{
    ...
    public IObjectCollection getPaymentSplitEntryByParam(Context ctx, Map enumConIdsMap, boolean isAdd, IObjectCollection selector, Map costIdandConIdMap, Map transSelector, IObjectValue spiltClass) throws BOSException, EASBizException
    {
        try {
            ServiceContext svcCtx = createServiceContext(new MetaDataPK("d774cf95-fb70-47c0-80a3-9aaac8646e40"), new Object[]{ctx, enumConIdsMap, new Boolean(isAdd), selector, costIdandConIdMap, transSelector, spiltClass});
            invokeServiceBefore(svcCtx);
            IObjectCollection retValue = (IObjectCollection)_getPaymentSplitEntryByParam(ctx, enumConIdsMap, isAdd, selector, costIdandConIdMap, transSelector, spiltClass);
            svcCtx.setMethodReturnValue(retValue);
            invokeServiceAfter(svcCtx);
            return retValue;
        } catch (BOSException ex) {
    ...
    protected abstract IObjectCollection _getPaymentSplitEntryByParam(Context ctx, Map enumConIdsMap, boolean isAdd, IObjectCollection selector, Map costIdandConIdMap, Map transSelector, IObjectValue spiltClass) throws BOSException, EASBizException;
