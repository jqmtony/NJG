...
//		objectValue.setCreateTime(new Timestamp(new Date().getTime()));
	...
	private void updateTabHeader(boolean selected) {
		...
		int contractNumberIdx = kdtEntrys.getColumn(CONTRACT_NUMBER)
				.getColumnIndex();
		int deductItemIdx = kdtEntrys.getColumn(DEDUCT_ITEM).getColumnIndex();
...
//			view.getSelector().add("contractBill.partB.name");
	...
	private void initEntryTable() {
		...
		kdtEntrys.getColumn(CONTRACT_NUMBER).getStyleAttributes().setBackground(FDCColorConstants.requiredColor);
		kdtEntrys.getColumn("contractName").getStyleAttributes().setBackground(FDCColorConstants.cantEditColor);
		kdtEntrys.getColumn("deductUnit").getStyleAttributes().setBackground(FDCColorConstants.requiredColor);
		kdtEntrys.getColumn("deductType").getStyleAttributes().setBackground(FDCColorConstants.requiredColor);
		kdtEntrys.getColumn(DEDUCT_ITEM).getStyleAttributes().setBackground(FDCColorConstants.requiredColor);
		kdtEntrys.getColumn(DEDUCT_AMT).getStyleAttributes().setBackground(FDCHelper.KDTABLE_SUBTOTAL_BG_COLOR);
//		kdtEntrys.getColumn("deductDate").getStyleAttributes().setBackground(FDCColorConstants.cantEditColor);
//		kdtEntrys.getColumn("deductDate").getStyleAttributes().setLocked(true);
		kdtEntrys.getColumn("deductDate").setRequired(true);
		kdtEntrys.getColumn("deductDate").setEditor(FdcEditorHelper.getDateEditor());
		kdtEntrys.getColumn("deductDate").setWidth(100);
		kdtEntrys.getColumn("currency").getStyleAttributes().setBackground(FDCColorConstants.cantEditColor);
