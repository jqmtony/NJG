...
//				 }
	 ...
	 */
	public static boolean checkIsPaymentSplit(String id) throws Exception {
		if (id == null)
			throw new IllegalArgumentException("invalid id argument");
		
		FilterInfo filter = new FilterInfo();
		filter.getFilterItems().add(new FilterItemInfo("paymentBill", id));
		//如果存在付款拆分

		return PaymentSplitFactory.getRemoteInstance().exists(filter) || PaymentNoCostSplitFactory.getRemoteInstance().exists(filter);
	}
}