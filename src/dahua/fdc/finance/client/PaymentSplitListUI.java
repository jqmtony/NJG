...
//			tblMain.getColumn("fiVouchered").getStyleAttributes().setHided(true);
	...
	public void actionCostSplit_actionPerformed(ActionEvent e) throws Exception {
		...
		if (!isConWithoutTxt(getMainTable())) {
			...
			FDCSplitClientHelper.isBillSplitedByContractBillId(this.getContractBillId(), this);
		}
...
//				this.setCursorOfDefault();
	 ...
	 */
	protected void tblMain_tableSelectChanged(com.kingdee.bos.ctrl.kdf.table.event.KDTSelectEvent e)
			throws Exception {
		checkSelected();
		// 财务组织时不另取
...
//			String longName = action.getClass().getName();
	...
	public IMetaDataPK getMetaDataPK() {
		// TODO 候鹏伟在解决BT322149问题，不知道为何要改变MetaDataPK，导致“无文本合同付款拆分”菜单不受控，LicenseSerivce.getUIClassName()返回的是PaymentSplitWithoutTxtConEditUI，所以不受控
		// 查看了所有房地产代码，没有引用此方法，所以暂时注释掉。Added by Owen_wen
		/*final Object obj = getUIContext().get(UIContext.UICLASSPARAM);
		if (obj != null && obj.equals("isContract=\"false\"")) {// 满足菜单界面定义的参数条件
	...
	protected void checkBillState(String state, String res) throws Exception {
		...
		for (Iterator iter = coll.iterator(); iter.hasNext();) {
			...
			if (FDCBillStateEnum.SUBMITTED_VALUE.equals(state)) {
				if (FDCBillStateEnum.AUDITTED_VALUE.equals(element.getString(getBillStatePropertyName()))) {
					MsgBox.showWarning(this, "当前记录中有记录已经审批通过!");
					abort();
				}
			} else if(FDCBillStateEnum.AUDITTED_VALUE.equals(state)){
				if (!FDCBillStateEnum.AUDITTED_VALUE.equals(element.getString(getBillStatePropertyName()))) {
					MsgBox.showWarning(this, "当前记录中有记录未审批通过!");
					abort();
				}
			}
		}
