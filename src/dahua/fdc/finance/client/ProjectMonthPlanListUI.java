...
import com.kingdee.eas.fdc.finance.IProjectMonthPlan;
 ...
 * output class name
 */
public class ProjectMonthPlanListUI extends AbstractProjectMonthPlanListUI
{
    private static final Logger logger = CoreUIObject.getLogger(ProjectMonthPlanListUI.class);
    //获取有权限的组织
	protected Set authorizedOrgs = null;
	protected static final String CANTEDIT = "cantEdit";
	protected static final String CANTREMOVE = "cantRemove";
    public ProjectMonthPlanListUI() throws Exception
    {
        super();
    }
    ...
    protected String getEditUIName(){
        return ProjectMonthPlanEditUI.class.getName();
    }
    ...
    protected void checkBeforeEditOrRemove(String warning,String id) throws EASBizException, BOSException, Exception {
		...
		sels.add("state");
		sels.add("creator.id");
		ProjectMonthPlanInfo info=(ProjectMonthPlanInfo)getBizInterface().getValue(new ObjectUuidPK(id),sels);
		if(!info.getCreator().getId().toString().equals(SysContext.getSysContext().getCurrentUserInfo().getId().toString())){
	...
	public void actionAudit_actionPerformed(ActionEvent e) throws Exception {
		...
		for(int i = 0; i < id.size(); i++){
			...
			sels.add("state");
			sels.add("creator.id");
			ProjectMonthPlanInfo info=(ProjectMonthPlanInfo)getBizInterface().getValue(new ObjectUuidPK(id.get(i).toString()),sels);
			if(!info.getCreator().getId().toString().equals(SysContext.getSysContext().getCurrentUserInfo().getId().toString())){
				FDCMsgBox.showWarning("编制人非本人，禁止操作！");
				SysUtil.abort();
			}
			...
			((IProjectMonthPlan)getBizInterface()).audit(BOSUuid.read(id.get(i).toString()));
		}
	...
	public void actionUnAudit_actionPerformed(ActionEvent e) throws Exception {
		...
		for(int i = 0; i < id.size(); i++){
			...
			((IProjectMonthPlan)getBizInterface()).unAudit(BOSUuid.read(id.get(i).toString()));
		}
		FDCClientUtils.showOprtOK(this);
		this.refresh(null);
	}
	public void actionModify_actionPerformed(ActionEvent e) throws Exception {
		ProjectMonthPlanInfo info=getSelectedInfo();
		if(!info.getCreator().getId().toString().equals(SysContext.getSysContext().getCurrentUserInfo().getId().toString())){
			FDCMsgBox.showWarning("编制人非本人，禁止操作！");
			SysUtil.abort();
		}
	...
	private void checkAudited(ProjectMonthPlanInfo info) throws BOSException, EASBizException {
		if (!FDCUtils.isBillAudited(info)) {
			MsgBox.showWarning(this, "非审批单据不能修订！");
	        SysUtil.abort();
		}
	}
	private void checkLastVersion(ProjectMonthPlanInfo info) throws BOSException, EASBizException {
		if(!info.isIsLatest()){
			MsgBox.showWarning(this, "非最新版本不能修订！");
	        SysUtil.abort();
		}
	}
	private ProjectMonthPlanInfo getSelectedInfo() throws BOSException, EASBizException {
		checkSelected();
		...
		return ProjectMonthPlanFactory.getRemoteInstance().getProjectMonthPlanInfo(new ObjectUuidPK(getSelectedKeyValue()),sel);
	}
	protected IQueryExecutor getQueryExecutor(IMetaDataPK queryPK, EntityViewInfo viewInfo) {
		...
		if(viewInfo.getSorter()!=null&&viewInfo.getSorter().size()<2){
			...
			viewInfo.getSorter().add(new SorterItemInfo("creator.number"));
