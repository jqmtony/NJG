...
//		builder.appendSql("where acctPay.fprojectId=? and payReq.fcurProjectId=? and payReq.fstate=? and ((period.fyear=? and period.fmonth<=?) or (period.fyear<?)) ");
		builder.appendSql("where acctPay.fprojectId=? and payReq.fstate=? and ((period.fyear=? and period.fmonth<=?) or (period.fyear<?)) ");
...
//		builder.appendSql("where acctPay.fprojectId=? and payReq.fcurProjectId=? and payReq.fstate=? and period.fyear=? and period.fmonth<=? ");
		builder.appendSql("where acctPay.fprojectId=? and payReq.fstate=? and period.fyear=? and period.fmonth<=? ");
		builder.appendSql("group by fcostaccountid,acctPay.fcontractid");
		builder.addParam(prjId);
//		builder.addParam(prjId);
		builder.addParam(FDCBillStateEnum.AUDITTED_VALUE);
...
//		builder.appendSql("where acctPay.fprojectId=? and payReq.fcurProjectId=? and payReq.fstate=? and period.fyear<?");
		builder.appendSql("where acctPay.fprojectId=? and payReq.fstate=? and period.fyear<?");
		builder.appendSql("group by fcostaccountid,acctPay.fcontractid");
		builder.addParam(prjId);
//		builder.addParam(prjId);
		builder.addParam(FDCBillStateEnum.AUDITTED_VALUE);
...
//		builder.addParam(prjId);
	...
	/**
	 * 合同下所有变更签证确认拆到此科目的合计金额 by Cassiel_peng  2008-9-2
	 */
	...
	public static Map getPayedAmtWithSettlement(Context ctx,String prjId) throws BOSException,SQLException{
		...
		builder.appendSql("inner join T_CON_ContractSettlementBill settle on head.FCostBillID=settle.FID  \n");
		//		builder.appendSql("where FCostBillType='SETTLEMENTSPLIT' and head.fcontrolUnitId in (select fcontrolUnitId from T_FDC_CurProject where fid=entry.fobjectid) and head.FIsInvalid=0 And  entry.fobjectid=? and  ((settle.FIsSettled=1 and settle.FIsFinalSettle=1) or (settle.FIsSettled=0 and settle.FIsFinalSettle=0))  \n");
		//builder.appendSql("where FCostBillType='SETTLEMENTSPLIT' and head.FIsInvalid=0 And  entry.fobjectid=? and  ((settle.FIsSettled=1 and settle.FIsFinalSettle=1) or (settle.FIsSettled=0 and settle.FIsFinalSettle=0))  \n");
		builder.appendSql("where FCostBillType='SETTLEMENTSPLIT' and head.FIsInvalid=0 And  entry.fobjectid=?  \n");
	 ...
	 * 已实现成本
	 */
	public static Map getPayedAmtWithCost(Context ctx,String prjId,boolean flag) throws BOSException,SQLException{
		...
		if(flag){ //去发票值做已实现成本时,值没有同步到成本拆分上,所以还是要在付款拆分取
			builder.appendSql("select split.fcontractbillid as fcontractbillid,splitEntry.Fcostaccountid,sum(splitEntry.FInvoiceAmt) as famount ");
			builder.appendSql("from T_AIM_CostSplit costsplit ");
			builder.appendSql("inner join T_FNC_PaymentSplit split on split.fid= costsplit.fsplitbillid");
			builder.appendSql(" inner join T_FNC_PaymentSplitEntry splitEntry on splitEntry.FParentID = split.fid ");
			builder.appendSql("inner join T_AIM_CostSplitEntry costSplitEntry on costSplitEntry.fparentid=costsplit.fid");
			builder.appendSql("	and splitEntry.fcostAccountId = costSplitEntry .FCostAccountID ");
		...
		if(flag){
			builder.appendSql("group by split.fcontractbillid,splitEntry.Fcostaccountid \n");
		}else{
	...
	public static Map getPayedAmt(Context ctx,String prjId) throws BOSException, SQLException{
		...
		builder.appendSql("costsplit.fcostbilltype=?  \n  ");
