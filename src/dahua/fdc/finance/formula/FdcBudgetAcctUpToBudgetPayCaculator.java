 ...
 * @author pengwei_hou
 * @date 2009-06-11
 * @work 若录入了截止期间，获取该年度年初到截止期间之间的月度预算表计划付款累计。
 * 若未录入截止期间，则获取预算当前期间年度年初到预算系统当前期间之间的月度预算表计划付款累计。
 * @method fdc_budget_acct_upToBudgetPay 预算系统年初截至截止期间计划付款取数函数
 */
public class FdcBudgetAcctUpToBudgetPayCaculator implements ICalculator, IMethodBatchQuery 
{
	...
	{
//		获得从界面传递过来的相关参数
		SortedParameterArray params = (SortedParameterArray) methods.get("fdc_budget_acct_upToBudgetPay");
		...
		if (params != null) {
			for (int i = 0; i < params.size(); i++) {
				...
				try {
					// 通过计算公式计算，返回结果
					BigDecimal amount = this.fdc_budget_acct_upToBudgetPay(
	...
	/**
	 * 若录入了截止期间，获取该年度年初到截止期间之间的月度预算表计划付款累计。
	 * 若未录入截止期间，则获取预算当前期间年度年初到预算系统当前期间之间的月度预算表计划付款累计。
	 * 
	 ...
	 */
	public BigDecimal fdc_budget_acct_upToBudgetPay(String costCenterNumber,
...
//		startDate = FDCDateHelper.getLastDayOfMonth2(FDCDateHelper.getFirstYearDate(endDate));
		...
		if(curProjectID.size() == 1)
		{
			...
			{
				result = FDCBudgetAcctCaculatorHelper.getUpToBudgetPay(ServerCtx, costAccountID, startDate, period.getEndDate());
			}
			if(result == null)
			{
