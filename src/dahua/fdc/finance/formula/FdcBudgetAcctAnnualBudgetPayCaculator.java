 ...
 * @author pengwei_hou
 * @date 2009-06-09
 * @work 预算系统年度预算预算付款取数函数
 * @method fdc_budget_acct_annualBudgetPay 项目年度预算表的该预算期间对应月份预算付款数据
 */
public class FdcBudgetAcctAnnualBudgetPayCaculator implements IMethodBatchQuery,
		ICalculator {
	...
	public boolean batchQuery(Map methods) {
		//获得从界面传递过来的相关参数
		SortedParameterArray params = (SortedParameterArray) methods.get("fdc_budget_acct_annualBudgetPay");
		...
		if (params != null) {
			for (int i = 0; i < params.size(); i++) {
				...
				try {
					// 通过计算公式计算，返回结果
					BigDecimal amount = this.fdc_budget_acct_annualBudgetPay(
	 ...
	 */
	public BigDecimal fdc_budget_acct_annualBudgetPay(String costCenterNumber,
...
//		transformDate = FDCDateHelper.getLastDayOfMonth2(transformDate);
		...
		if (curProjectID.size() == 1)
		{
			...
			result = FDCBudgetAcctCaculatorHelper.getAnnualBudgetPay(ServerCtx,costAccountID, period);	
			if(result == null)
			{
