...
//		selector.add("curProject.number");
	...
	public IObjectValue importSplit() throws BOSException ,EASBizException{
		...
		if(!isHasSettled()||noSettleSplit){
			...
			// 判断是否存在合同变更及是否拆分 add by 何鹏
			if (isExistChangeCostSplit()) {
					throw new EASBizException(new NumericExceptionSubItem("100",
							"存在未拆分的变更签证确认，不允许进行工程量拆分！"));
			}
			int index=1;
	 ...
	 * @see
	 */
	public boolean isExistChangeCostSplit() throws BOSException {
		boolean changeCostSplit = false;
		FDCSQLBuilder builder = new FDCSQLBuilder(ctx);
		builder.appendSql("select * from T_Con_ContractChangeBill");
		builder.appendSql(" where ");
		builder.appendSql(" fcontractbillid= ?");
		builder.appendSql(" and (fsplitstate is null or fsplitstate='"
				+ CostSplitStateEnum.NOSPLIT_VALUE + "')");
		builder.addParam(getContractBillId());
		IRowSet rowSet = builder.executeQuery();
		if (rowSet.size() > 0) {
			changeCostSplit = true;
		}
		return changeCostSplit;
	}
}
