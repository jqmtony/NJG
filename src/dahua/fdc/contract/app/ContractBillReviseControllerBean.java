...
public class ContractBillReviseControllerBean extends AbstractContractBillReviseControllerBean
{
    ...
    protected boolean isCanModifyNumberAndName(Context ctx) throws EASBizException, BOSException {
    	boolean isOk = false;
		// 客户需求改变，在启用FDCConstants.FDC_PARAM_CANMODIFYCONTRACTNUMBER这个参数后，需要一直保持合同秀订单的编码及名称
		//可修改状态，但是不提示其名称重复，故在此屏蔽其是否启用状态检测。考虑到后续如果继续使用参数来控制，故没有在参数中删除该参数
    	// Added by Adward_huang 2012-8-29
		//    	try {
    ...
    */
    ...
    protected void _audit(Context ctx, BOSUuid billId) throws BOSException, EASBizException {
					...
					if (oldCostSplit) {
						...
						try {
							if(rowSet.next()){
								...
								if(isSplitBaseOnProp){
									ContractCostSplitFactory.getLocalInstance(ctx).autoSplit4(BOSUuid.read(conId));
									builder.clear();	// modified by zhaoqin for R131216-0522 on 2013/12/17
									// 审核时，如果为自动拆分，那么就将状态改为”全部拆分“
									builder.appendSql("update T_CON_ContractBill set fsplitState=? where fid=?");
									builder.addParam(CostSplitStateEnum.ALLSPLIT_VALUE);
									builder.addParam(conId.toString());
									builder.getSql();
									builder.execute();
									builder.clear();
		...
		ContractBillInfo contractBillInfo = ContractBillFactory.getLocalInstance(ctx).getContractBillInfo(
				"select amount where id='" + conId + "'");
		BigDecimal latestPrice = FDCUtils.getContractLastAmt(ctx, conId);
		// 更新未审批的付款申请单以及付款单的合同造价为修订后的金额 by sxhong 2008－12－19
		FDCSQLBuilder builder = new FDCSQLBuilder(ctx);
		builder.appendSql("update T_Con_PayRequestBill set FContractPrice=?,FLatestPrice=? where fcontractid=? and ");
		builder.addParam(contractBillInfo.getAmount());
		builder.addParam(latestPrice);
		builder.addParam(conId);
		builder.appendParam("fstate", new String[] { FDCBillStateEnum.SAVED_VALUE, FDCBillStateEnum.SUBMITTED_VALUE ,FDCBillStateEnum.AUDITTING_VALUE});
		builder.execute();
		...
		// 同步附件到合同
		ContractUtil.synAttachmentToContract(ctx, info);
	}
...
//			}	
	...
	protected IObjectPK _submit(Context ctx, IObjectValue model) throws BOSException, EASBizException {
		...
		if (info.getName() != null) {
			...
			info.setName(string.trim());
		}
		if (info.getNumber() != null) {
			String string = info.getNumber();
			info.setNumber(string.trim());
		}
		checkContractN(ctx, info);
		saveBail(ctx, info);
	 ...
	 * 校验合同名称编码是否重复
	 */
	private void checkContractN(Context ctx, ContractBillReviseInfo info) throws ContractException, EASBizException, BOSException {
		...
		if (isOk) {
			...
			filter.getFilterItems().add(new FilterItemInfo("curProject.id", info.getCurProject().getId()));
			filter.getFilterItems().add(new FilterItemInfo("id", info.getContractBill().getId().toString(), CompareType.NOTEQUALS));
...
//		ContractPayItemCollection ContractBailInfo
    ...
    private void synToContractBill(Context ctx, ContractBillReviseInfo info)  throws BOSException, EASBizException {
    	...
    	if(isOk){
    		...
    		newConInfo.setCodingNumber(info.getNumber());	// modified by zhaoqin for R130913-0068 on 2013/9/27
//    		newConInfo.setName(info.getMainContractNumber());
		...
		if (contractBillInfo.getBail() != null) {
			BOSUuid bailID = contractBillInfo.getBail().getId();
			/* modified by zhaoqin for R20150206-0082,R20150114-0073 on 2015/03/16 */
			//ContractBailFactory.getLocalInstance(ctx).delete(new ObjectUuidPK(bailID));
			if (bailInfo != null) {
				bailInfo.setId(bailID);
			}
		}
		if (bailInfo != null) {
			ContractBailFactory.getLocalInstance(ctx).save(bailInfo);
		}
...
//    	}
    	...
    	/* modified by zhaoqin for R20150206-0082,R20150114-0073 on 2015/03/16 */
    	if(null == bailInfo && null != contractBillInfo.getBail()) {
    		BOSUuid bailID = contractBillInfo.getBail().getId();
    		ContractBailFactory.getLocalInstance(ctx).delete(new ObjectUuidPK(bailID));
    	}
    }
...
//					BigDecimal localAmount = rs1.getBigDecimal("mainLocalAmount");
		    	...
		    	DbUtil.execute(ctx, updateSql, new Object[] { oriAmount, mainAmount, newGrtAmount, newStampAmt, mainId });
...
//					viewInfo.getSelector().add("splitState");
	...
	protected IObjectPK _addnew(Context ctx, IObjectValue model) throws BOSException, EASBizException {
		...
		ContractBillReviseInfo info = (ContractBillReviseInfo) model;
		// 从合同同步附件
		ContractUtil.synAttachmentFromContract(ctx, info);
