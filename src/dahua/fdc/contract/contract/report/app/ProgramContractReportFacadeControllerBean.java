...
public class ProgramContractReportFacadeControllerBean extends AbstractProgramContractReportFacadeControllerBean
{
    ...
    protected RptParams _createTempTable(Context ctx, RptParams params) throws BOSException, EASBizException {
	    ...
	    // real date
	    initColoum(header,col,"sgtRealDate",100,false);
	    initColoum(header,col,"sgtOverdue",80,false);
	    initColoum(header,col,"csRealDate",100,false);
	    initColoum(header,col,"csOverdue",80,false);
	    initColoum(header,col,"startRealDate",80,false);
	    initColoum(header,col,"startOverdue",80,false);
	    initColoum(header,col,"endRealDate",80,false);
	    initColoum(header,col,"endOverdue",80,false);
	    initColoum(header,col,"csendRealDate",110,false);
	    initColoum(header,col,"csendOverdue",80,false);
	    header.setLabels(new Object[][]{ 
	    		{
	    			...
	    			"id","level","longNumber","headNumber","规划合同名称","合约类型","规划金额","预留变更率","采购控制金额","设计部","成本管理部/组","工程部","工程部","材料管理公司",
	    			"设计部","设计部","成本管理部/组","成本管理部/组","工程部","工程部","工程部","工程部","材料管理公司","材料管理公司"
	    		},{
	    			"id","level","longNumber","headNumber","规划合同名称","合约类型","规划金额","预留变更率","采购控制金额","施工图交接时间","合同签订时间","开工时间","竣工时间","合同签订完成时间",
	    			"施工图交接时间","是否逾期","合同签订时间","是否逾期","开工时间","是否逾期","竣工时间","是否逾期","合同签订完成时间","是否逾期"
	    		}
	    },true);
    ...
    protected String getSql(Context ctx,RptParams params){
    	...
    	sb.append("pcont.CFStartDate,pcont.CFEndDate,pcont.CFCsendDate from T_CON_ProgrammingContract pcont left join T_CON_Programming program on pcont.FPROGRAMMINGID=program.fid ");
