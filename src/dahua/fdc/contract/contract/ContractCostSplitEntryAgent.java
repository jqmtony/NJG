...
public class ContractCostSplitEntryAgent extends ContractCostSplitEntryInfo implements IObjectValueAgent{
    public static final BOSObjectType bosType = new BOSObjectType("4027B6D2");
    private final AgentManager manager;

    private ContractCostSplitEntryAgent ()
    {
        manager = new AgentManager();
    }
    
    public static ContractCostSplitEntryAgent copyOvToAgent(IObjectValue vo){
        ContractCostSplitEntryAgent agent = new ContractCostSplitEntryAgent();
        return (ContractCostSplitEntryAgent)agent.manager.copyOvToAgent(agent,vo);
    }

    public static ContractCostSplitEntryAgent copyOvAsNewAgent(IObjectValue vo){
        ContractCostSplitEntryAgent agent = new ContractCostSplitEntryAgent();
        return (ContractCostSplitEntryAgent)agent.manager.copyOvToAgentAsNew(agent,vo);
    }

    public static ContractCostSplitEntryAgent find(IObjectPK id) throws BOSException
    {
		ContractCostSplitEntryAgent agent = new ContractCostSplitEntryAgent();
        return (ContractCostSplitEntryAgent)agent.manager.find(agent, id);
    }
    
    public static ContractCostSplitEntryAgent create() throws BOSException
    {
        return (ContractCostSplitEntryAgent)AgentManager.create(new ContractCostSplitEntryAgent());
    }
