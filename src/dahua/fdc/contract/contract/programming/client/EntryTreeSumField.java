    ...
    protected void appendProFootRow(KDTable table,String columnKey)
    {
        ...
        Iterator lt=tableToSumIndex.entrySet().iterator();
         while (lt.hasNext()){
             ...
             if (columnMap.size()>0)
             {
                 IRow footRow = FDCTableHelper.generateFootRow(tableSum);
  ...
  //金额类字段在修改时自动向上汇总
	public void caclTotalAmount(KDTable kdtEntries) {
	  ...
	  private String[] getProSumFields(){
	    	return new String[] { "amount", "signUpAmount", "changeAmount", "settleAmount", "balance", "controlBalance", "buildPerSquare",
				"soldPerSquare" };
	    }
}
