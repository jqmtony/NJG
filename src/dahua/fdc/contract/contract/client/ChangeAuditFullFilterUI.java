 ...
 * output class name
 */
public class ChangeAuditFullFilterUI extends AbstractChangeAuditFullFilterUI {
	 ...
	 */
	public ChangeAuditFullFilterUI(ListUI listUI, ItemAction actionListOnLoad)
			throws Exception {
		super();
		this.listUI = listUI;
	...
	protected void getCompanyFilter(FDCCustomerParams para, FilterInfo filter) {
		...
		if (companyIds != null) {
			filter.getFilterItems().add(new FilterItemInfo("orgUnit.id", FDCHelper.getSetByArray(companyIds), CompareType.INCLUDE));
		} else {
	...
	protected void getProjectFilter(FDCCustomerParams para, FilterInfo filter) {
		if (para.isNotNull(KEY_PROJECTIDS)) {
			filter.getFilterItems().add(
					new FilterItemInfo("curProject.id", FDCHelper.getSetByArray(para.getStringArray(KEY_PROJECTIDS)), CompareType.INCLUDE));
		} else {
	...
	public FilterInfo getFilterInfo() {
		...
		getCompanyFilter(para, filter);
		getProjectFilter(para, filter);
		
		if (para.isNotNull(KEY_CHANGETYPEIDS)) {
			filter.getFilterItems().add(
					new FilterItemInfo("auditType.id", FDCHelper
							.getSetByArray(para.getStringArray(KEY_CHANGETYPEIDS)),
							CompareType.INCLUDE));
		}
		else {
			filter.getFilterItems().add(
					new FilterItemInfo("auditType.isEnabled", Boolean.TRUE));
		}
...
//		}
		...
		if(para.isNotNull(KEY_STATE)) {
			...
			if (para.getInt(KEY_STATE) == 0) {
				state = ChangeBillStateEnum.SAVED_VALUE;
			} else if (para.getInt(KEY_STATE) == 1) {
				state = ChangeBillStateEnum.SUBMIT_VALUE;
			} else if (para.getInt(KEY_STATE) == 2) {
				state = ChangeBillStateEnum.AUDITTING_VALUE;
			} else if (para.getInt(KEY_STATE) == 3) {
				state = ChangeBillStateEnum.AUDIT_VALUE;
			} else if (para.getInt(KEY_STATE) == 5) {
				state = ChangeBillStateEnum.ANNOUNCE_VALUE;
			} else if (para.getInt(KEY_STATE) == 6) {
				state = ChangeBillStateEnum.VISA_VALUE;
			} else if (para.getInt(KEY_STATE) == 7) {
				state = ChangeBillStateEnum.AHEADDISPATCH_VALUE;
			}
			 else if (para.getInt(KEY_STATE) == 8) {
				state = ChangeBillStateEnum.REGISTER_VALUE;
			}
			
			if (state != null) {
				filter.getFilterItems().add(new FilterItemInfo("changeState", state));
			}
		}
		return filter;
	}
...
//	private final String KEY_PARTBID = "partBId";
...
//		} 
		...
		} else if (this.radioDisPatch.isSelected()) {
			param.add(KEY_STATE, 5);
		} else if (this.radioVisa.isSelected()) {
			param.add(KEY_STATE, 6);
		}
		else if (this.radioAhead.isSelected()) {
			param.add(KEY_STATE, 7);
		}
		else if (this.radioReg.isSelected()) {
			param.add(KEY_STATE, 8);
		}
...
//			} 
				...
				this.radioDisPatch.setSelected(true);
			} else if (state == 6) {
				this.radioVisa.setSelected(true);
			} 
			else if (state == 7) {
				this.radioAhead.setSelected(true);
			} 
			else if (state ==8) {
				this.radioReg.setSelected(true);
			} 
		}
