	...
	public void actionClearSplit_actionPerformed(ActionEvent e)
		...
		if (isCostSplit(ids)
			...
			if(isAdjustVourcherModel()){
				...
				l	合同拆分被后续拆分引用时，提示：该合同拆分被后续拆分引用，作废合同拆分时将作废后续拆分，且该合同进入待处理流程，确认是否操作？
				*/

				if (isHasOtherSplit(selectedContractBillID)) {
					msg = "该合同拆分被后续拆分引用，作废合同拆分时将作废后续拆分，且该合同进入待处理流程，确认是否操作？";
					//					msg = "作废合同拆分将把合同相关拆分作废，若对应的合同存在被付款单引用生成凭证的付款拆分或者生成凭证的调整单，则此合同进入待处理流程，需要财务进行后续凭证处理，确认是否操作？";
				} else {
					
//					msg = "该合同拆分没有被后续拆分引用，将只作废本合同拆分！确认是否操作？";
					FDCMsgBox.showWarning("该合同拆分没有被后续拆分引用，请点击“拆分”按钮直接修改！");
					abort();
				}
			}
			if (MsgBox
	...
	/**
	 * 描述：合同拆分是否被后续拆分引用（注：后续拆分--工程量拆分、结算拆分、合同付款拆分）
	 * @throws BOSException 
	 * @throws EASBizException 
	 * @Author：keyan_zhao
	 * @CreateTime：2012-9-5
	 */
	private boolean isHasOtherSplit(String contractId) throws EASBizException, BOSException {
		...
		paySplitFilter.getFilterItems().add(new FilterItemInfo("state", FDCBillStateEnum.INVALID_VALUE, CompareType.NOTEQUALS));
		isExist = PaymentSplitFactory.getRemoteInstance().exists(paySplitFilter);
		if (isExist) {
			return true;
		}
		FilterInfo settSplitFilter = new FilterInfo();
		settSplitFilter.getFilterItems().add(new FilterItemInfo("contractBill.id", contractId));
		settSplitFilter.getFilterItems().add(new FilterItemInfo("state", FDCBillStateEnum.INVALID_VALUE, CompareType.NOTEQUALS));
		isExist = SettlementCostSplitFactory.getRemoteInstance().exists(settSplitFilter);
		if (isExist) {
			return true;
		}
		return false;
	...
	public void actionViewInvalid_actionPerformed(ActionEvent e)
		...
		try {
			...
			handUIExceptionAndAbort(exe);// OprtState.EDIT);
		}
	}
	...
	protected void refresh(ActionEvent e) throws Exception {
		...
		/**
		 * 当点击查询时，此时ListUI界面传过来的e==null,如果不执行刷新，则查询不到数据
		 */
		if (e == null) {
			super.refresh(e);
		}
		...
		super.filterByBillState(ev);
		 if(ev!=null&&ev.getFilter()!=null){
			 try {
				...
				f.getFilterItems().add(new FilterItemInfo("contractBill.state", FDCBillStateEnum.BACK_VALUE, CompareType.NOTEQUALS));
