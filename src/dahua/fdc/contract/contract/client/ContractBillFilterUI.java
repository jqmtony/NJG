...
public class ContractBillFilterUI extends ContractFullFilterUI {
	...
	public void onLoad() throws Exception {
		...
		//过滤查询条件的合同签约开始日期置空   add by jian_cao
	...
	protected void getProjectFilter(FilterInfo filter, FDCCustomerParams para) {
		try {
			...
			if (project != null) {
				...
				newfilter.getFilterItems().add(new FilterItemInfo("curProject.id", idSet, CompareType.INCLUDE));
			} else {
				newfilter.getFilterItems().add(new FilterItemInfo("curProject.isEnabled", Boolean.TRUE));
			}
