...
public final class PayReqWebTableHelper {
	...
	private PayRequestBillWebEditUI editUI = null;

	private HashMap bindCellMap = null;
	private KDTable table=null;
	public PayReqWebTableHelper(PayRequestBillWebEditUI editUI) {
		this.editUI = editUI;
...
*/
		 ...
		 * 甲供材款累计
		 */
		row = table.getRow(lastRowIdx - 4);
		row.getCell(4).getStyleAttributes().setLocked(false);
		row.getCell(6).setExpressions("=D" + (lastRowIdx - 3) + "+F" + (lastRowIdx - 3));
		row.getCell(7).setExpressions("=(C" + (lastRowIdx - 3) + "+F" + (lastRowIdx - 3) + ")");

		/*实付款
		 * 因为要初化完后才能定位应扣款项小计的位置,故放到这里来计算. 实付款 实付款＝进度款小计－应扣款项小计-应扣甲供材
		 * 调整为:实付款＝合同内工程款+预付款+奖励-违约金-应扣款项小计-应扣甲供材 by sxhong 2007/09/28
		 */
		int paidRowIdx = lastRowIdx - 3;
...
//		table.getCell(paidRowIdx + 1, 6).setExpressions("=G2-H5");
		/*
		 * 本次申请%=本期发生实付款/最新造价
		 */
