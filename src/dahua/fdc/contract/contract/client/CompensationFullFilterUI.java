 ...
 * output class name
 */
public class CompensationFullFilterUI extends AbstractCompensationFullFilterUI
{
    private static final Logger logger = CoreUIObject.getLogger(CompensationFullFilterUI.class);
    
	private static final String CONTRACT_STATE = "contractState";

	private static final String COMPENSATION_TYPE = "compensationType";
	...
	public final static String resourcePath = "com.kingdee.eas.fdc.contract.client.ContractFullResource";
	
	public final static String ALL = EASResource.getString(resourcePath, "ALL");
     ...
     */
    public CompensationFullFilterUI() throws Exception
    {
        super();
    }

    public CompensationFullFilterUI(CompensationFullListUI listUI, ItemAction load)throws Exception {
		super();
		this.listUI = listUI;
	...
	public FilterInfo getFilterInfo() {
		...
		if (para.isNotNull(COMPENSATION_TYPE)) {
			filter.getFilterItems().add(
					new FilterItemInfo("compensationType.id", para.getString(COMPENSATION_TYPE)));
		}
	...
	public CustomerParams getCustomerParams() {
		...
		if (comboCompensationType.getSelectedItem() != ALL) {
			 param.add(COMPENSATION_TYPE, 
					 ((CounterclaimTypeInfo)comboCompensationType.getSelectedItem()).getId().toString());
		}
...
//		}
	...
	public void onLoad() throws Exception {
		...
		comboCompensationType.addItem(ALL);
        CounterclaimTypeCollection col = CounterclaimTypeFactory.getRemoteInstance().getCounterclaimTypeCollection();
        comboCompensationType.addItems(col.toArray());
	}
	...
	public void setCustomerParams(CustomerParams cp) {
		...
		if (para.isNotNull(COMPENSATION_TYPE)) {
			String id = para.getString(COMPENSATION_TYPE);
			FDCCustomerParams.setSelectedItem(comboCompensationType,BOSUuid.read(id));
		}
		...
		if (object != null) {
			...
			for (int i = 0; i < contractTypeList.size(); i++) {
				if (contractTypeList.get(i) instanceof ContractTypeInfo) {
					...
					//if (contractType != null && contractType.isIsLeaf()) { update by renliang
