...
public class FDCUtils {
	...
	/**
	 * 
	 * 描述：批量取参数方法，按<b>需要</b>取数，这样不引发性能问题，用于替换有性能问题的getDefaultFDCParam()方法<p>
	 * 使用示例：<br>
	 * 	<code> HashMap hmParamIn = new HashMap();
	    hmParamIn.put(FDCConstants.FDC_PARAM_STARTMG, comPK);
	    hmParamIn.put(FDCConstants.FDC_PARAM_SELECTPERSON, null);
	    getParamHashMapBatch(Context ctx, HashMap hmParamIn);</code>
	    
	 * @param paramIn 是HashMap，key是参数number，value是组织或公司ID （IObjectPK 类型，集团级参数传入nulll）
	 * @return 参数值HashMap，key是参数number，value是参数值
	 * @Author：owen_wen
	 * @CreateTime：2013-4-3
	 */
    public static HashMap getParamHashMapBatch(Context ctx, HashMap hmParamIn) throws BOSException, EASBizException {
		IParamControl pc;
		if (ctx != null)
	...
	/**
	 * 获得房地产系统参数
	 * @deprecated 该方法不再建议使用，因为取的参数值过多，会影响性能，在750下尤其明显 by Owen_wen 2013-02-21
	 * @see FDCUtils其它取参数方法, 如getParamHashMapBatch(),getBooleanValue4FDCParamByKey()
	 */
    public static HashMap getDefaultFDCParam(Context ctx, String companyID) throws BOSException, EASBizException {
        ...
        hmParamIn.put(FDCConstants.FDC_PARAM_MORESETTER_ALLNOTPAID, null);  // Added by Owen_wen 2010-07-28
        //集团参数：再次归档时更新合同编号
...
//        hmParamIn.put(FDCConstants.FDC_PARAM_BASEONPAYMENTBILL, null);
        ...
        //付款单生成凭证时检查付款单是否已拆分
        hmParamIn.put(FDCConstants.FDC_PARAM_CHECKPAYMENTSPLIT, null);
        //付款单提交时检查付款单已经拆分完成
...
//		进度管理完工工程量的确认是否严格控制
		...
		// ///////////////////////////////////////////////////////////////////////////
		/** 合同/财务 FDC30 **/
		hmParamIn.put(FDCConstants.FDC351_IS_PAY_SPLIT_MERGE, comPK);
    ...
    public static boolean getDefaultFDCParamByKey(Context ctx, String companyID,String fdcParamKey) throws BOSException, EASBizException {
    	return getBooleanValue4FDCParamByKey(ctx, companyID, fdcParamKey);
    }
    ...
    public static String getFDCParamByKey(Context ctx, String companyID,String paramNumber) throws EASBizException, BOSException{
		...
		if (ctx != null) {
			...
			fdcParamValue = ParamControlFactory.getRemoteInstance().getParamValue(comPK, paramNumber);
		}
    ...
    public static String getStringValue4FDCParamByKey(Context ctx, String companyID, String paramNumber) throws EASBizException,
			BOSException {
		return FDCHelper.getStringValue4FDCParamByKey(ctx, companyID, paramNumber);
    }
    ...
    public static AccountViewInfo getDefaultFDCParamAccount(Context ctx, String companyID) throws BOSException, EASBizException {
    	...
    	String paramValue = getFDCParamByKey(ctx, companyID, FDCConstants.FDC_PARAM_ACCOUNTVIEW);
		if(paramValue!=null){
			String accountId = paramValue.toString();
			// accountId取到的值可能为"true"，也要排除 Added by Owen_wen 2012-4-5
			if (!StringUtils.isEmpty(accountId) && !"none".equals(accountId) && !"true".equals(accountId)) {
				if (ctx == null) {
					account = AccountViewFactory.getRemoteInstance().getAccountViewInfo(new ObjectUuidPK(BOSUuid.read(accountId)));
				} else {
					account = AccountViewFactory.getLocalInstance(ctx).getAccountViewInfo(new ObjectUuidPK(BOSUuid.read(accountId)));
				}
			}
		}
        return account;
    ...
    public static VoucherTypeInfo getDefaultFDCParamVoucherType(Context ctx, String companyID) throws BOSException, EASBizException {
		...
		String paramValue = getFDCParamByKey(ctx, companyID, FDCConstants.FDC_PARAM_VOUCHERTYPE);
		if (!FDCHelper.isEmpty(paramValue) && !paramValue.equals("none")) {
			if (ctx == null) {
				voucherType = VoucherTypeFactory.getRemoteInstance().getVoucherTypeInfo(new ObjectUuidPK(BOSUuid.read(paramValue)));
			} else {
				voucherType = VoucherTypeFactory.getLocalInstance(ctx).getVoucherTypeInfo(new ObjectUuidPK(BOSUuid.read(paramValue)));
			}
		}
		return voucherType;
    }
    ...
    public static boolean IsInCorporation(Context ctx, String companyID) throws BOSException, EASBizException {
		return getBooleanValue4FDCParamByKey(ctx, companyID, FDCConstants.FDC_PARAM_INCORPORATION);
	}
    ...
    public static boolean IsFinacial(Context ctx, String companyID) throws BOSException, EASBizException {
    	return getBooleanValue4FDCParamByKey(ctx, companyID, FDCConstants.FDC_PARAM_FINACIAL);
    }
    /**
    ...
    public static boolean IsSettlementCostSplitVoucher(Context ctx, String companyID) throws BOSException, EASBizException {
        return getBooleanValue4FDCParamByKey(ctx, null, FDCConstants.FDC_PARAM_SETTLEMENTCOSTSPLIT);
    }
    ...
    public static boolean IsUseCostOrFinance(Context ctx, String companyID) throws BOSException, EASBizException {
        return getBooleanValue4FDCParamByKey(ctx, companyID, FDCConstants.FDC_PARAM_USECOSTORFINANCE);
    }
...
//		return  changeAmount.add(guerdonAmt).subtract(compenseAmt).subtract(deductAmt);
	...
	// 合同+变更
	public static BigDecimal getConAndChange(Context ctx, String contractId) throws BOSException, EASBizException {
		Map map = (Map) FDCUtils.getConAndChange(ctx, new String[] { contractId });
		BigDecimal conAndChange = (BigDecimal) map.get(contractId);
		return conAndChange;
	}
	
	public static boolean isIncludeChangeAudit(Context ctx) throws EASBizException, BOSException {
	...
	/**
	 * 合同+变更 与getLastAmt_Batch的区别是，这里不取合同造价只取合同+变更（不是变更结算）
	 */
	public static Map getConAndChange(Context ctx, String[] contractIdList) throws BOSException, EASBizException {
		if (contractIdList == null || contractIdList.length == 0) {
			return new HashMap();
		}
		BigDecimal bdZero = FDCNumberHelper.ZERO;
		...
		try {
			...
			try {
				...
				/*
				 * 万科最新要求,未结算的合同最新造价=合同金额+变更金额+奖励-索赔,扣款(已被申请单关联的) by sxhong
				 * 2007/09/28
				 */
				if (noSettleContractIdList != null) {
					String[] noSettleContractIdArray = FDCHelper.stringToStrArray(noSettleContractIdList);
	...
	public static Map getLastOriginalAmt_Batch(Context ctx,String[] contractIdList) throws Exception {
		...
		try {
			builder
					.appendSql("select fcontractbillid,FTotalOriginalAmount as FTotalOriginalAmount from t_con_contractsettlementbill where fstate='4AUDITTED' and FIsFinalSettle=1 and ");
...
//					ContractException.CANNOTFINDCOSTACCOUNTWITHACCOUNT);
   ...
   public static boolean IsSimpleFinacial(Context ctx, String companyID) throws BOSException, EASBizException {
       return getBooleanValue4FDCParamByKey(ctx, companyID, FDCConstants.FDC_PARAM_SIMPLEFINACIAL);
   }
   ...
   public static boolean IsSimpleFinacialExtend(Context ctx, String companyID) throws BOSException, EASBizException {
       return getBooleanValue4FDCParamByKey(ctx, companyID, FDCConstants.FDC_PARAM_SIMPLEFINACIALEXTEND);
   }
   ...
   public static boolean IsUnInCorporation(Context ctx, String companyID) throws BOSException, EASBizException {
	   return getBooleanValue4FDCParamByKey(ctx, companyID, FDCConstants.FDC_PARAM_UNINCORPORATION);
   }
