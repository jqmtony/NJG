	...
	public CoreBaseInfo transmit(Hashtable htable, Context ctx)
		...
		*/
		...
		if(this.isLength(getResource(ctx, "yusuanyuanbijine"), FOriginalAmount, "Double", true, -1)){//预算原币金额
			BigDecimal oriAmount = strToBigDecimal(FOriginalAmount);
			info.setOriginalAmount(oriAmount);
			info.setAmount(oriAmount.multiply(cbinfo.getExRate()));
		}
		if(this.isLength(getResource(ctx, "jiesuanyuanbijine"), FOriBalanceAmount, "Double", false, -1)){//结算金额原币
			BigDecimal oriBlanceAmt = strToBigDecimal(FOriBalanceAmount);
			info.setOriBalanceAmount(oriBlanceAmt);
			info.setBalanceAmount(oriBlanceAmt.multiply(cbinfo.getExRate()));
		}
		...
		if(this.isLength(getResource(ctx, "yuanxilianxidanhao"), FOriginalContactNum, "String", false, 50)){	//原始联系单号
			//与变更签证申请中的原始联系单号保持一致，否则，取变更签证申请中的原始联系单号
			//先通过合同cbinfo的id 在变更签证申请分录ChangeSupplierEntryInfo 中拿到分录的信息    然后拿到联系单号
