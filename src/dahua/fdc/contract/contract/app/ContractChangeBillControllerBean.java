...
public class ContractChangeBillControllerBean extends
	...
	protected Map _fetchInitData(Context ctx, Map paramMap) throws BOSException, EASBizException {
		...
		if(contractBillId!=null && initMap.get(FDCConstants.FDC_INIT_CONTRACT)==null){
			...
			selectors.add("curProject.costCenter.id");	// modified by zhaoqin for R130711-0070 on 2013/12/3
	...
	protected void _audit(Context ctx, BOSUuid billId) throws BOSException, EASBizException {
		...
		selectors.add("ContractBill.isCoseSplit");
		...
		boolean isImportConSplit = FDCUtils.getDefaultFDCParamByKey(ctx, info.getCurProject().getCostCenter().getId().toString(),
				FDCConstants.FDC_PARAM_IMPORTCONSPLIT);
		if (info != null && info.isIsCostSplit()&&info.getCurProject() != null && info.getCurProject().getCostCenter() != null && info.getCurProject().getCostCenter().getId() != null) {
			...
			// 是否自动拆分
			//boolean isAutoSplit = FDCUtils.getBooleanValue4FDCParamByKey(ctx, info.getCurProject().getCostCenter().getId().toString(),
			 ...
			 *  2: 不自动拆分
			 */
			String autoSplit = FDCUtils.getFDCParamByKey(ctx, info.getCurProject().getCostCenter().getId().toString(), 
					FDCConstants.FDC_PARAM_ISAUTOSPLIT);
			if (isImportConSplit && null != autoSplit && "1".equals(autoSplit)) {
		...
		try {
			synContractProgAmt(ctx,info,true);
		...
		// 通过指令单id查询 审批单下发单位分录
		BigDecimal costAmount = info.getAmount();
		if (costAmount != null && costAmount.compareTo(FDCHelper.ZERO) == 0 && !isImportConSplit) {
			if (info.getContractBill().isIsCoseSplit()) { // 判断是否成本拆分
				// 生成变更成本拆分数据
				ConChangeSplitInfo costSplitInfo = createConChangeSplitInfo(ctx, billId.toString());
				ConChangeSplitFactory.getLocalInstance(ctx).save(costSplitInfo);
			} else {
				// 生成变更非成本拆分数据		
				ConChangeNoCostSplitInfo noCostSplitInfo = createConChangeNoSplitInfo(ctx, billId.toString());
	...
	protected ConChangeNoCostSplitInfo createConChangeNoSplitInfo(Context ctx, String contractChange) throws BOSException, EASBizException {
		...
		if (contractChange != null) {
			contractChangeInfo = ContractChangeBillFactory.getLocalInstance(ctx).getContractChangeBillInfo(
					new ObjectUuidPK(BOSUuid.read(contractChange)), selectors);
	...
	protected void _unAudit(Context ctx, BOSUuid billId) throws BOSException, EASBizException {
		...
		try {
			synContractProgAmt(ctx,info,false);
		} catch (SQLException e) {
			logger.error(e);
			throw new BOSException(e);
		}
      ...
      * @throws SQLException 
      */
    private void synContractProgAmt(Context ctx,ContractChangeBillInfo model,boolean flag) throws EASBizException, BOSException, SQLException{
		...
		if(flag){
			...
			pcInfo.setControlBalance(FDCHelper.subtract(controlBalanceAmt, changeAmount));
			...
			pcInfo.setControlBalance(FDCHelper.add(controlBalanceAmt, changeAmount));
	...
	protected boolean _visa(Context ctx, Set idSet, IObjectCollection cols) throws BOSException, EASBizException {
		...
		while (itr.hasNext()) {
			...
			_update(ctx, new ObjectUuidPK(billInfo.getId()), billInfo);
		}
	...
	protected void _visa(Context ctx, IObjectPK[] pks) throws BOSException, EASBizException {
		for (int i=0;i<pks.length;i++) {
			...
			ContractChangeBillInfo bill = (ContractChangeBillInfo) super._getValue(ctx, pk, selector);
			if(bill.getChangeAudit()!=null){
				ChangeAuditBillInfo info = ChangeAuditBillFactory.getLocalInstance(ctx).
				getChangeAuditBillInfo(new ObjectUuidPK(bill.getChangeAudit().getId()));
			...
			BigDecimal costAmount = bill.getAmount();
			if (costAmount != null && costAmount.compareTo(FDCHelper.ZERO) == 0) {
				if (bill.getContractBill().isIsCoseSplit()) {
					// 查出变更拆分数据并更新拆分状态
					ConChangeSplitCollection ccsColl = ConChangeSplitFactory.getLocalInstance(ctx).getConChangeSplitCollection(
							"where contractChange.id = '" + pk.toString() + "'");
					if (ccsColl != null && ccsColl.size() > 0) {
						ConChangeSplitInfo info = ccsColl.get(0);
						String sql = "update T_CON_ConChangeSplit set fsplitstate = '3ALLSPLIT' where fid = ? and fstate <> '9INVALID'";
						FDCSQLBuilder builder = new FDCSQLBuilder(ctx);
					...
					// 查出变更拆分数据并更新拆分状态
					ConChangeNoCostSplitCollection ccsColl = ConChangeNoCostSplitFactory.getLocalInstance(ctx)
							.getConChangeNoCostSplitCollection("where contractChange.id = '" + pk.toString() + "'");
					if (ccsColl != null && ccsColl.size() > 0) {
						ConChangeNoCostSplitInfo info = ccsColl.get(0);
						String sql = "update T_CON_ConChangeNoCostSplit set fsplitstate = '3ALLSPLIT' where fid = ? and fstate <> '9INVALID'";
						FDCSQLBuilder builder = new FDCSQLBuilder(ctx);
						builder.appendSql(sql);
						builder.addParam(info.getId().toString());
						builder.execute();
	...
	/**
	 * 在工作流中设置变更签证确认的状态为"结算未审批"   by Cassiel_peng 2009-8-19 
	 */
	...
	/**
	 * 在工作流中设置变更签证确认的状态为"结算已审批"   by Cassiel_peng 2009-8-19
	 */
	protected void _setSettAuditedForWF(Context ctx, BOSUuid pk)
		...
		//在工作流第一个审批节点触发的时候变更签证确认的原币金额就已经存到FSettAuditAmt字段中,待审批全都通过之后才反写到 FOrgBlanceAmount字段中
...
//		ContractChangeBillFactory.getLocalInstance(ctx).updatePartial(billInfo, selector);
	...
	/**
	 * 在工作流中设置变更签证确认的状态为"结算审批中"   by Cassiel_peng 2009-8-19
	 */
