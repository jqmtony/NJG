 ...
 * 
 */
package com.kingdee.eas.fdc.contract.contractsplit.util;
 ...
 *
 */
public class ProjectTreeBuilderForSL {
	...
	public FilterInfo getProjectFilterByCurOrg() throws BOSException,
       ...
       // 得到一个项目实体
		...
		if (projectIdSet != null && projectIdSet.size() > 0) {
			projFilter.getFilterItems().add(
					new FilterItemInfo("id", projectIdSet, CompareType.INCLUDE));
		}
		projFilter.getFilterItems().add(
...
//		}
	...
	public ProjectTreeBuilderForSL() {
	 ...
	 *               <p>
	 */
	public ProjectTreeBuilderForSL(boolean isIncludeDisabled) {
	 ...
	 *               <p>
	 */
	public ProjectTreeBuilderForSL(boolean isIncludeDisabled, boolean noOrgIsolation) {
		this.isIncludeDisabled = isIncludeDisabled;
	 ...
	 *               <p>
	 */
	public ProjectTreeBuilderForSL(boolean isIncludeDisabled, boolean noOrgIsolation, Set set) {
		this.isIncludeDisabled = isIncludeDisabled;
		this.noOrgIsolation = noOrgIsolation;
		...
		//判断合同拆分和变更拆分的项目树合集是否为空
		 if (projectIdSet != null && projectIdSet.size() > 0) {
			filter.getFilterItems()
					.add(new FilterItemInfo("id", projectIdSet, CompareType.INCLUDE));
		}
		if (isDevPrjFilter) {
			if (isContainDevPrj) {
				filter.getFilterItems().add(
	...
	private void buildProjTree(FilterInfo filter) throws Exception {
		...
		FilterInfo newfilInfo = new FilterInfo();
		newfilInfo.getFilterItems().add(new FilterItemInfo("isWholeAgeStage",1));
		
		filter.mergeFilter(newfilInfo, "and");
