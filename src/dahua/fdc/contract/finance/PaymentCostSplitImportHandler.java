...
public class PaymentCostSplitImportHandler implements ISplitImportHandler{
	 ...
	 * @param info
	 */
	public void handleSplitedAmt(PaymentSplitInfo info) throws BOSException {
		...
		PaymentSplitCollection paymentSplitCollection = iPaymentSplit
...
/*				if(info.isIsProgress()){
	 ...
	 * @param info
	 */
	public Map handleSplitedCostAmt(PaymentSplitInfo info) throws BOSException {
		String contractId = info.getContractBill().getId().toString();
		// 以前的付款拆分分录
		Map paySplitEntryMap = new HashMap();
		// 已拆分的保修款付款拆分分录
		...
		PaymentSplitCollection paymentSplitCollection = iPaymentSplit
				.getPaymentSplitCollection(view);
		for (int i = 0; i < paymentSplitCollection.size(); i++) {
			for (Iterator iter = paymentSplitCollection.get(i).getEntrys().iterator(); iter
					.hasNext();) {
				PaymentSplitEntryInfo entry = (PaymentSplitEntryInfo) iter.next();
				if (entry.getCostAccount() == null) {
					continue;
				...
				if (entry.getParent().getPaymentBill() != null
						...
						&& entry.getParent().getPaymentBill().getFdcPayType().getPayType() != null
						&& entry.getParent().getPaymentBill().getFdcPayType().getPayType().getId()
								.equals(PaymentTypeInfo.keepID)) {
					mapEntry.setSplitQualityAmt(FDCHelper.add(mapEntry.getSplitQualityAmt(), entry
							.getPayedAmt()));
				}
