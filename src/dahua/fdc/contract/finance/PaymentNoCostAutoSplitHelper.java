				...
				if(!item.isIsLeaf()){//是非明细节点
					...
					if (startIndex < sortList.size() - 1) {
						for (int i = startIndex + 1; i < sortList.size(); i++) {
							PaymentNoCostSplitEntryInfo splitEntry = (PaymentNoCostSplitEntryInfo) sortList.get(i);
							if (item.getAccountView().getId().toString().equals(splitEntry.getAccountView().getId().toString())
									&& CostSplitTypeEnum.PRODSPLIT.equals(splitEntry.getSplitType()) && splitEntry.isIsLeaf()) {
								judgeLastProductSplit.add(new Integer(splitEntry.getSeq()));
							} else {
								break;
							}
