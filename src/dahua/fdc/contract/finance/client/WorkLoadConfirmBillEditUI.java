 ...
 protected boolean isBaseContract() throws EASBizException, BOSException
{
    ...
    private void fillPrjTable(java.util.List entryList, Map accMap)
    {
        ...
        IRow footRow = FDCTableHelper.generateFootRow(tblDetail);
...
//    	WorkLoadPrjBillEntryCollection prjFillEntryCol = editData.getPrjFillBillEntry();
	...
	public void onLoad() throws Exception {
		...
		initCtrl();
		handleOldData();
		btnContractFullInfo.setIcon(EASResource.getIcon("imgTbtn_execute"));
		mentItemContractInfo.setIcon(EASResource.getIcon("imgTbtn_execute"));
		
		/* modified by zhaoqin for R131204-0537 on 2014/01/14 */
		setActionVoucher();
	}
...
//      确认单未审批，提示单据状态不适合。
        if(!FDCBillStateEnum.AUDITTED.
        	...
        	IUIWindow splitUIwindow = null;
			UIContext uiContext = new UIContext(this);
			uiContext.put("contractBillId", editData.getContractBill().getId().toString());
			String uiName = "com.kingdee.eas.fdc.finance.client.WorkLoadSplitEditUI";
	        EntityViewInfo view = new EntityViewInfo();
...
//		FDCClientHelper.setn
	...
	protected void verifyInputForSubmint() throws Exception {
		...
		builder.appendSql("(  \n");
		builder.appendSql(" (select isnull(head.FProjectPriceInContract,0) as famount from T_Con_PayRequestBill head \n");
		builder.appendSql("   left outer join T_FDC_PaymentType type on type.fid=head.FPaymentType \n");
		builder.appendSql("   where head.FContractId=? and head.FHasClosed=0 and type.fname_l2!='预付款')  \n");
		builder.appendSql(" union all \n");

		builder
				.appendSql(" (select (isnull(head.FProjectPriceInContract,0)-isnull(entry.flocadvance,0)) as famount from T_CAS_PaymentBill head \n");
		...
		builder.appendSql("   left outer join T_FDC_PaymentType type on type.fid=req.FPaymentType \n");
		builder.appendSql("  where req.FContractId=? and req.FHasClosed=1 and type.fname_l2!='预付款'  \n");
	...
	public void actionPrint_actionPerformed(ActionEvent e) throws Exception {
		...
		dataPvd.setIsBaseContract(isBaseContract());
    ...
    {	
		  ...
		  UIContext uiContext = new UIContext(this);
		  if(null!=prmtContractBill.getValue()){
			  uiContext.put(UIContext.ID,((ContractBillInfo)prmtContractBill.getValue()).getId().toString());
			  
			   IUIWindow
		            // 创建UI对象并显示
		            uiWindow = UIFactory.createUIFactory(UIFactoryName.MODEL).create(ContractFullInfoUI.class.getName(), uiContext, null,
		                    OprtState.VIEW);
	...
	/**
	 * 设置 "生成凭证" 状态 - R131204-0537
	 * @author RD_zhaoqin
	 * @date 2014/01/14
	 */
	protected void setActionVoucher() throws EASBizException, BOSException{
    	boolean isVouncher = this.editData.isFiVouchered();
    	if(isVouncher) {
