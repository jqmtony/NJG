...
/*		//合同对应到付款
	...
	private PaymentSplitEditUI getPaymentSplitEditUI(String paymentId) throws Exception{
		if(paymentSplitEditUI==null){
			...
			uiContext.put("Owner", new PaymentSplitListUI());
			...
			paymentSplitEditUI.getUIContext().put("Owner", new PaymentSplitListUI());
			//			paymentSplitEditUI.setOprtState(OprtState.ADDNEW);
			//			paymentSplitEditUI.onLoad();
			paymentSplitEditUI = (PaymentSplitEditUI) UIFactoryHelper.initUIObject(editName, paymentSplitEditUI.getUIContext(), null,
					OprtState.ADDNEW);
		}
