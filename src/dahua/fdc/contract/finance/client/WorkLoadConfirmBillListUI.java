...
//      确认单未审批，提示单据状态不适合。
        if(!FDCBillStateEnum.AUDITTED.
			...
			uiContext.put("contractBillId", this.getContractBillId());
...
//        }
	...
	protected FilterInfo getTreeSelectChangeFilter() {
		...
		//by tim_gao 过滤信息增加合同类型是否工程量确认的
		filter.getFilterItems().add(new FilterItemInfo("contractType.isWorkLoadConfirm", Boolean.TRUE));
	...
	protected boolean displayBillByContract(KDTSelectEvent e, EntityViewInfo view) throws BOSException {
		...
		for(Iterator it = coll.iterator();it.hasNext();){
			...
			if(info.getContractBill()!=null ){
				if(info.getContractBill().getPartB() != null)
					row.getCell(COL_PARTB).setValue(info.getContractBill().getPartB().getName());
			}
		...
		IRow footRow = FDCTableHelper.generateFootRow(tblWorkLoad);
...
//		checkWorkLoadParam();
	...
	private String getContractBillId(){
		this.checkSelected();
		int selectedRowIndex = getBillListTable().getSelectManager().getActiveRowIndex();
		return ((WorkLoadConfirmBillInfo) getBillListTable().getRow(selectedRowIndex).getUserObject()).getContractBill().getId().toString();
	}	
	...
	protected IQueryExecutor getQueryExecutor(IMetaDataPK queryPK, EntityViewInfo viewInfo) {
		...
		newfilter.getFilterItems().add(new FilterItemInfo("contractType.isWorkLoadConfirm", Boolean.TRUE));
		try {
			viewInfo.getFilter().mergeFilter(newfilter, "and");
	...
	/**
	 * 生成凭证时，取工程量确认单的id - R131204-0537
	 * @author RD_zhaoqin
	 * @date 2014/01/14
	 */
	public void getBillIdList(List idList, List entriesList) {
		int mode=0;
        List blockList=tblWorkLoad.getSelectManager().getBlocks();
        if (blockList!=null&&blockList.size()==1)
           mode=((IBlock)tblWorkLoad.getSelectManager().getBlocks().get(0)).getMode();
        ...
        if (mode==KDTSelectManager.TABLE_SELECT) { //表选择
            ...
            ArrayList blocks = tblWorkLoad.getSelectManager().getBlocks();
            ...
            while (iter.hasNext()) {
                ...
                for (int rowIndex = top; rowIndex <= bottom; rowIndex++) {
                    ICell cell = tblWorkLoad.getRow(rowIndex).getCell(getKeyFieldName());
                    //记录选中的分录ID
                    if(tblWorkLoad.getRow(rowIndex).getCell(this.getEntriesPKName()) != null
                            && tblWorkLoad.getRow(rowIndex).getCell(this.
                                    getEntriesPKName()).getValue() != null) {
                        entriesList.add(tblWorkLoad.getRow(rowIndex).getCell(this.
