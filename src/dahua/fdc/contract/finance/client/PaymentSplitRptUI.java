...
public class PaymentSplitRptUI extends AbstractPaymentSplitRptUI {
	...
	protected void fillTable() throws Exception {
		...
		try {
			...
			// 如果选择显示级次添加过滤
			FilterItemInfo filterItem = new FilterItemInfo("isSplit", new Integer(1));
			if (isShowSplitAcct) {
				acctFilter.getFilterItems().add(filterItem);
	...
	protected void fillNode(DefaultMutableTreeNode node, KDTable table) throws BOSException, SQLException, EASBizException {
		...
		CostAccountCollection cion = FDCAimcostClientHelper.getCostAccountCollection(costAcct,
				curProject.getId().toString(), acctChidrenMap);
		
		Map ChidrenValue = null;
		String costId = null;
...
//			row.getStyleAttributes().setBackground(FDCTableHelper.daySubTotalColor);
			...
			for (int i = 0; i < periodList.size(); i++) {
				...
				if (this.isShowSplitAcct && !costAcct.isIsLeaf()) {
					...
					BigDecimal ChidrenPayAmt = null;
					// 得到子集集合
					ChidrenPayAmt = FDCHelper.ZERO;
					for (int j = 0, l = cion.size(); j < l; j++) {
						costId = cion.get(j).getId().toString();
						ChidrenValue = (Map) splits.get(costId + period);
						if (ChidrenValue != null) {
							if (value == null) {
								value = new HashMap();
							}
							Set keySet = ChidrenValue.keySet();
							// 拆分数据
							for (Iterator iter = keySet.iterator(); iter.hasNext();) {
								String key = (String) iter.next();
								BigDecimal payAmt = (BigDecimal) ChidrenValue.get(key);
								payAmt = FDCHelper.add(payAmt, (BigDecimal) value.get(key));
								value.put(key, payAmt);
							}
						}
					}
				...
				acctAmt = loadRowData(table, pays, cons, txts, level, acctAmt, period, periodAmt,
						value);
			}
		 ...
		 * @see						
		 */
	private BigDecimal loadRowData(KDTable table, Map pays, Map cons, Map txts, int level,
			BigDecimal acctAmt, String period, BigDecimal periodAmt, Map value) throws BOSException {
		if (value != null) {
...
//		super.getLocateNames();
	...
	private int getPrecision(CompanyOrgUnitInfo currentCompany,CurrencyInfo currency,Date bookedDate) throws BOSException{
		...
		/**
	 * @description 显示可拆分级次事件
	 * @author 陈伟
	 * @createDate 2011-8-30
	 * @param e
	 * @throws Exception
	 * @version EAS7.0
	 * @see com.kingdee.eas.fdc.finance.client.AbstractPaymentSplitRptUI#actionSplit_actionPerformed(java.awt.event.ActionEvent)
	 */
