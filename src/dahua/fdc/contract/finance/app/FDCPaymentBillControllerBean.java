...
public class FDCPaymentBillControllerBean extends AbstractFDCPaymentBillControllerBean
{
	...
	protected HashMap _getIsRespiteByPaymentBillIds(Context ctx, HashSet idSet) throws BOSException, EASBizException {
		...
		view.getSelector().add("isRespite");
		FDCPaymentBillCollection col = FDCPaymentBillFactory.getLocalInstance(ctx).getFDCPaymentBillCollection(view);
		FDCPaymentBillInfo info = null;
		if (col != null && col.size() > 0) {
	...
	protected void _updateInvoiceAmt(Context ctx, IObjectValue fdcPayment, PaymentBillInfo paymentBill) throws BOSException, EASBizException {
		...
		String contractBillId = paymentBill.getContractBillId();
		if (contractBillId != null) {
			if ("3D9A5388".equals(BOSUuid.read(contractBillId).getType().toString())) {
				...
				/*fdcPaymentBill = FDCPaymentBillFactory.getLocalInstance(ctx).getFDCPaymentBillInfo(
						new ObjectUuidPK(fdcPaymentBill.getId().toString()), selector);*/
				BigDecimal invoiceAmt = fdcPaymentBill.getInvoiceAmt();
				BigDecimal invoiceOriAmt = fdcPaymentBill.getAllInvoiceAmt();
				
				// 拼接SQL采用appendSql和appendParam方法来处理，在后台FDCSQL处理器中会对
				//BigDecimal做setBigDecimal处理 added Adward_huang 2012-08-14
				FDCSQLBuilder builder = new FDCSQLBuilder(ctx);
				builder.appendSql("update T_CON_ContractWithoutText set FInvoiceAmt=");
				builder.appendParam(invoiceAmt);
				builder.appendSql(",FAllInvoiceAmt = ");
				builder.appendParam(invoiceOriAmt);
