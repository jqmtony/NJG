...
public abstract class AbstractPaymentSplitControllerBean extends FDCSplitBillControllerBean implements PaymentSplitController
{
    ...
    public void autoSplit(Context ctx, String paymentId) throws BOSException, EASBizException
    {
        try {
            ServiceContext svcCtx = createServiceContext(new MetaDataPK("c05f7fd1-c772-4f24-963a-c2e0b95c574f"), new Object[]{ctx, paymentId});
            invokeServiceBefore(svcCtx);
            _autoSplit(ctx, paymentId);
            invokeServiceAfter(svcCtx);
        } catch (BOSException ex) {
            this.setRollbackOnly();
            throw ex;
    ...
    protected abstract void _autoSplit(Context ctx, String paymentId) throws BOSException, EASBizException;
