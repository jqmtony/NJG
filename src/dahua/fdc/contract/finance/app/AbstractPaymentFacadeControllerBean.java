...
public abstract class AbstractPaymentFacadeControllerBean extends AbstractBizControllerBean implements PaymentFacadeController
{
    ...
    public ContractTypeInfo getContractType(Context ctx, String contractId) throws BOSException, EASBizException
    {
        try {
            ServiceContext svcCtx = createServiceContext(new MetaDataPK("bf22a9fb-0f43-4cad-9017-fe5620281150"), new Object[]{ctx, contractId});
            invokeServiceBefore(svcCtx);
            if(!svcCtx.invokeBreak()) {
            ContractTypeInfo retValue = (ContractTypeInfo)_getContractType(ctx, contractId);
            svcCtx.setMethodReturnValue(retValue);
            }
            invokeServiceAfter(svcCtx);
    ...
    protected abstract IObjectValue _getContractType(Context ctx, String contractId) throws BOSException, EASBizException;
