...
public class PaymentBillImport extends AbstractDataTransmission {
	...
	public CoreBaseInfo transmit(Hashtable hsData, Context ctx) throws TaskExternalException {
		...
		String ChangeMoney = FDCTransmissionHelper.getFieldValue(hsData, "ChangeMoney");//变更签证确认金额
		...
		FDCTransmissionHelper.valueFormat("变更签证确认金额", ChangeMoney, "Double", false, 80);
...
//				FDCTransmissionHelper.isThrow("非审批状态的时候，不能填写审核日期！");
		...
		try {
			...
			//项目名称   合同名称       最新造价        变更签证确认      金额合同内工程款_本期发生（原币）都由合同编号带出    取得合同信息
...
//			info.setAddProjectAmt(FDCTransmissionHelper.strToBigDecimal(FProjectPriceInContract));//工程付款情况表本期发生原币合同内工程款
			...
			// }
			//			
			// // "变更签证确认金额", ChangeMoney 不为空 代表覆盖数据
...
//			info.setFdcPayReqID(prbinfo.getId().toString());// 付款申请单
			...
			this.updateFDCPaymentBill(ctx, prbinfo, info, invoiceDate, InvoiceNumber, invoiceAmt);
		} catch (BOSException e) {
			e.printStackTrace();
	...
	/**
	 * 描述：修改【房地产付款单中间表】里的发票信息相关数据<br/>
	 * BT701390:房地产付款单导入成功，发票信息在模板里面已填写，但导入后里面发票信息为空。
	 * @param ctx
	 * @param prbinfo
	 * @param paymentBill
	 * @param invoiceDate
	 ...
	 */
	private void updateFDCPaymentBill(Context ctx, PayRequestBillInfo prbinfo, PaymentBillInfo paymentBill, String invoiceDate,
			String InvoiceNumber,
			String invoiceAmt) throws BOSException, EASBizException {
		
		FDCPaymentBillInfo info = null;
		EntityViewInfo view = new EntityViewInfo();
		FilterInfo filter = new FilterInfo();
		filter.getFilterItems().add(new FilterItemInfo("paymentBill", paymentBill.getId().toString(), CompareType.EQUALS));
		view.setFilter(filter);
		//根据财务付款单ID查出【房地产付款单中间表】里的记录
		FDCPaymentBillCollection fdcPaymentBillCollection = FDCPaymentBillFactory.getLocalInstance(ctx).getFDCPaymentBillCollection(view);
		//如果存在就重新设置发票号等信息
		if (fdcPaymentBillCollection != null && fdcPaymentBillCollection.size() > 0) {
			info = (FDCPaymentBillInfo) fdcPaymentBillCollection.get(0);

			info.setInvoiceDate(FDCTransmissionHelper.strToDate(invoiceDate));
			info.setInvoiceNumber(InvoiceNumber);

			info.setInvoiceAmt(FDCTransmissionHelper.strToBigDecimal(invoiceAmt));
			info.setAllInvoiceAmt(info.getInvoiceAmt());

			info.setInvoiceOriAmt(info.getInvoiceAmt());
			info.setAllInvoiceOriAmt(info.getInvoiceAmt());

    		info.setPaymentBill(paymentBill);

			FDCPaymentBillFactory.getLocalInstance(ctx).update(new ObjectUuidPK(String.valueOf(info.getId())), info);
		}
	}
