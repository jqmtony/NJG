...
import com.kingdee.eas.fdc.finance.client.PayPlanTemplateUI;
...
public class PTEEditUI extends AbstractPTEEditUI implements REAutoCompletionDataSetter, REAutoCompletionDataFilter
{
	...
	protected PayPlanTemplateUI payPlanTemplateUI;
...
//		isParent = (Boolean) this.getUIContext().get("isParent");
		...
		if (this.oprtState.equals(OprtState.VIEW)) {
			...
			isEditable(true);
			ctrButtonEnable(!isParent.booleanValue());
...
//		kDTabbedPane2.remove(kdplPayPlan);
	...
	protected void initPayPlan() throws UIException {
		...
		ProgrammingTemplateEntireInfo pInfo = (ProgrammingTemplateEntireInfo) getUIContext().get("rowObject");
		uiContext.put("programming", pInfo);
		if (pInfo.getId() != null) {
			uiContext.put("programmingId", pInfo.getId().toString());
		}

		CoreUIObject uiObj = (CoreUIObject) UIFactoryHelper
				.initUIObject(PayPlanTemplateUI.class.getName(), uiContext, null, getOprtState());

		uiObj.setBounds(0, 0, kDTabbedPane2.getWidth(), kDTabbedPane2.getHeight() - 20);
		payPlanTemplateUI = (PayPlanTemplateUI) uiObj;
	...
	public void onShow() throws Exception {
		...
		kDTabbedPane1.remove(kdcEconomy);
	}
	...
	public void fromByisrepeat() {
		int rowIndex = kdtCostEntries.getRowCount();
		List list = new ArrayList();
		for (int i = rowIndex - 1; i >= 0; i--) {
			CostAccountInfo costaccount = null;
			if (kdtCostEntries.getCell(i, COSTACCOUNT).getValue() instanceof CostAccountInfo) {
				costaccount = (CostAccountInfo) kdtCostEntries.getCell(i, COSTACCOUNT).getValue();
			}
			String ids = costaccount.getId().toString();
			list.add(ids);
		}
	...
	public void actionSubmit_actionPerformed(ActionEvent e) throws Exception {
		...
		payPlanTemplateUI.actionSubmit_actionPerformed(e);
	...
	private boolean verifyIsModify() {
		if (oprtState.equals(OprtState.EDIT) || oprtState.equals(OprtState.ADDNEW)) {
			...
			// 主要范围
			if (FDCHelper.isEmpty(oldPteInfo.getScope())) {
				if (!FDCHelper.isEmpty(txtScope.getText())) {
					if (!"可录入“工作内容”、“招标指导”、“合同执行常见问题”等备注信息".equals(txtScope.getText())) {
						return true;
					}
				}
			}
	...
	private void preparePTEData() throws Exception {
		if (pteInfo.getHead() != null) {
			...
			String headName = index != -1 ? longName.substring(0, index) : longName;
		...
		if (pteInfo.getScope() == null || pteInfo.getScope().trim().length() == 0) {
			txtScope.setCustomForegroundColor(Color.GRAY);
			this.txtScope.setText(DEFAULT_WORK_CONTENT);// 主要范围
		} else {
		...
		processAttachments(pteInfo.getId().toString());
	...
	protected void kdtCostEntries_tableClicked(KDTMouseEvent e) throws Exception {
		if (e.getRowIndex() > -1 && !oprtState.equals(OprtState.VIEW) && e.getType() != KDTStyleConstants.HEAD_ROW) {

			//不是父节点的情况下放开删除按钮 让其可以删除
	...
	protected void kdtCostEntries_editStopped(KDTEditEvent e) throws Exception {
		...
		if (colIndex == kdtCostEntries.getColumnIndex(COSTACCOUNT)) {
			CostAccountInfo[] newValue = null;
			Object obj = kdtCostEntries.getCell(rowIndex, COSTACCOUNT).getValue();// 成本科目
			if (obj instanceof CostAccountInfo) {
				newValue = new CostAccountInfo[1];
				newValue[0] = (CostAccountInfo) kdtCostEntries.getCell(rowIndex, COSTACCOUNT).getValue();// 成本科目;
			} else {
				newValue = (CostAccountInfo[]) kdtCostEntries.getCell(rowIndex, COSTACCOUNT).getValue();// 成本科目
			}
			// 1.1
			if (newValue == null) {
				// 1.1.1
				...
				for (int i = 0; i < newValue.length; i++) {
					CostAccountInfo newCostAccountInfo = (CostAccountInfo) newValue[i];
					if (i == 0) {
						row = kdtCostEntries.getRow(e.getRowIndex());
					} else {
						row = kdtCostEntries.addRow();
					}
					kdtCostEntries.getCell(row.getRowIndex(), COSTACCOUNT_NUMBER).setValue(newCostAccountInfo.getLongNumber());
					kdtCostEntries.getCell(row.getRowIndex(), COSTACCOUNT).setValue(newCostAccountInfo);

					BigDecimal assignScale = FDCHelper.ONE_HUNDRED;
					BigDecimal costAccountScale = getCostAccountScale(newCostAccountInfo, false);
					assignScale = assignScale.subtract(costAccountScale);
					kdtCostEntries.getCell(row.getRowIndex(), ASSIGNSCALE).setValue(assignScale);// 计算当前行待分配比例
					precessCostAccountLongNumber(newCostAccountInfo, row.getRowIndex());
					//	isCostAccountDup(newCostAccountInfo, row.getRowIndex());
				}
			}
	...
	private void isCostAccountDup(CostAccountInfo currentInfo, int rowIndex) {
		...
		if (!FDCHelper.isEmpty(currentInfo.getLongNumber())) {
			...
			for (int i = 0; i < rowCount; i++) {
				...
				String str = currentInfo.getId().toString() + currentInfo.getName().toString();
				String str2 = forInfo.getId().toString() + forInfo.getName().toString();
			...
			if (flag) {
				...
				if (!forPteInfo.getId().toString().equals(pteInfo.getId().toString())) {
					...
					for (int k = 0; k < forCostCollection.size(); k++) {
						PTECostInfo pteCostInfo = forCostCollection.get(k); // 成本构成
						if (pteCostInfo.getCostAccount() != null) {
							String forLongNumber = pteCostInfo.getCostAccount() != null ? pteCostInfo.getCostAccount().getLongNumber()
									: null;
							if (currentLongNumber.equals(forLongNumber) && !FDCHelper.isEmpty(pteCostInfo.getContractScale())) {
								scale = scale.add(pteCostInfo.getContractScale());
							}
	...
	private void setSmallButton() {
		...
		if (OprtState.VIEW.equals(getOprtState())) {
			...
			setButtionEnable(!isParent.booleanValue());
		}
	}
	...
	protected void actionRemoveLine_cost_actionPerformed(ActionEvent e) throws Exception {
		...
		if (FDCMsgBox.OK == FDCMsgBox.showConfirm2("是否确认删除数据？")) {
			int selectSize = this.kdtCostEntries.getSelectManager().size();
			for (int i = selectSize - 1; i >= 0; i--) {
				KDTSelectBlock selectBlock = this.kdtCostEntries.getSelectManager().get(i);
				for (int j = selectBlock.getEndRow(); j >= selectBlock.getBeginRow(); j--) {
					kdtCostEntries.removeRow(j);
				}
			}
		}
	}
	...
	protected void initListener() {
		super.initListener();
		txtScope.addFocusListener(new FocusListener() {
			public void focusLost(FocusEvent e) {
				String text = txtScope.getText();
				if (text == null || text.trim().length() == 0) {
					txtScope.setText(DEFAULT_WORK_CONTENT);
					txtScope.setCustomForegroundColor(Color.GRAY);
				}
			}
	...
	private void processAttachments(String boID) {
		...
		try {
			...
			kdpConstract.setBounds(kdpConstract.getX(), kdpConstract.getY(), kdpConstract.getWidth(), kdpConstract.getHeight()+changY);
