...
public class ProgrammingContractPromptBox extends KDCommonPromptDialog  {
	protected IUIWindow programmingContractDialog = null;
	Map ctx = new HashMap();
	private IUIObject uiObj;
	
	public ProgrammingContractPromptBox(IUIObject uiObj) {
		this.uiObj = uiObj;
	}
	...
	public void show() {
		...
		ctx.put("projectId", uiObj.getUIContext().get("projectId"));
		try  {
	        uiFactory = UIFactory.createUIFactory(UIFactoryName.MODEL);
			programmingContractDialog = uiFactory.create(ProgrammingContractF7UI.class.getName(), ctx, null, OprtState.VIEW,
