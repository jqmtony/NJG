...
//				}
				if (hasData) {
					aimCost = FDCHelper.add(row.getCell(colName).getValue(), aimCost);
					if(FDCHelper.toBigDecimal(aimCost).compareTo(FDCHelper.ZERO)!=0){
						row.getCell(colName).setValue(aimCost);
					}
				} else {
					if(FDCHelper.toBigDecimal(aimCost).compareTo(FDCHelper.ZERO)!=0){
						row.getCell(colName).setValue(aimCost);
