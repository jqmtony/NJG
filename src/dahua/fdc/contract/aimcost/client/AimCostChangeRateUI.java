...
//		int month = Calendar.getInstance().get(Calendar.MONTH);
	...
	public void initTable() throws BOSException {
		...
		column = tblMain.addColumn();
		column.setKey("单方目标成本");
		column = tblMain.addColumn();
		column.setKey("目标成本总额");
		column = tblMain.addColumn();
		column.setKey("单方动态成本");
		column = tblMain.addColumn();
		column.setKey("动态成本总额");
		column = tblMain.addColumn();
		column.setKey("ABS总额(动态-目标)");
		column = tblMain.addColumn();
		column.setKey("ABS单方(动态-目标)");
		column = tblMain.addColumn();
		column.setKey("目标成本相对变动率");
		column = tblMain.addColumn();
		column.setKey("目标成本绝对变动率");
		this.tblMain.getHeadRow(0).getCell("目标可售面积").setValue("目标可售面积");
		this.tblMain.getHeadRow(0).getCell("动态可售面积").setValue("动态可售面积");
		this.tblMain.getHeadRow(0).getCell("单方目标成本").setValue("单方目标成本");
		this.tblMain.getHeadRow(0).getCell("目标成本总额").setValue("目标成本总额");
		this.tblMain.getHeadRow(0).getCell("单方动态成本").setValue("单方动态成本");
		this.tblMain.getHeadRow(0).getCell("动态成本总额").setValue("动态成本总额");
		this.tblMain.getHeadRow(0).getCell("ABS总额(动态-目标)").setValue(
				"ABS总额(动态-目标)");
		this.tblMain.getHeadRow(0).getCell("ABS单方(动态-目标)").setValue(
				"ABS单方(动态-目标)");
		this.tblMain.getHeadRow(0).getCell("目标成本相对变动率").setValue("目标成本相对变动率");
		this.tblMain.getHeadRow(0).getCell("目标成本绝对变动率").setValue("目标成本绝对变动率");
	...
	private void setUnionData() {
		String[] cols = new String[] { "目标可售面积", "动态可售面积", "目标成本总额", "动态成本总额",
				"ABS总额(动态-目标)" };
		...
		for (int i = 0; i < table.getRowCount(); i++) {
			...
			if (aim.compareTo(FDCHelper.ZERO) != 0) {
				...
				row.getCell("目标成本相对变动率").setValue(rate1.toString() + "%");
				row.getCell("目标成本绝对变动率").setValue(rate2.toString() + "%");
			}
		}
	}
	...
	public void fillNode(KDTable table, DefaultKingdeeTreeNode node)
		...
		if (node.isLeaf() && node.getUserObject() instanceof CurProjectInfo) {
			...
			row.getCell("目标成本总额").setValue(aimCost);
			row.getCell("动态成本总额").setValue(dyCost);
			row.getCell("ABS总额(动态-目标)").setValue(dyCost.subtract(aimCost).abs());
	...
	private void getAllData()throws BOSException, SQLException {
		...
		if (FdcObjectCollectionUtil.isEmpty(cuAccts)) {
			MsgBox.showWarning(this, "请配置成本科目");
			SysUtil.abort();
		}
