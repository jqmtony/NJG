...
public class MeasureCollectTable {
	...
	private void fillTable() {
		...
		if (oprtState != null && !oprtState.equals(OprtState.ADDNEW) && !oprtState.equals(OprtState.EDIT)) {
			...
			for (int i = 0, rowCount = table.getRowCount(); i < rowCount; i++) {
				editor = table.getCell(i, "splitType").getEditor();
		...
		// 直接锁定
		table.getColumn("splitType").getStyleAttributes().setLocked(flag);
	}
	...
	private void fillNode(DefaultMutableTreeNode node) {
		...
		// 禁用的科目，不新增行，避免点表左边的节点树显示禁用科目
		if (!measureCostEditUI.isAll) {
...
//				allBuildArea = allBuildArea.add(FDCHelper.toBigDecimal(buildArea));
			...
			BigDecimal allBuildArea = getAllBuildArea();
				BigDecimal avg = FDCHelper.divide(allTotal, allSellArea, 2, BigDecimal.ROUND_HALF_UP);
				row.getCell("avg").setValue(avg);
				BigDecimal buildAvg = FDCHelper.divide(allTotal, allBuildArea, 2, BigDecimal.ROUND_HALF_UP);
			row.getCell("buildAvg").setValue(buildAvg);
//			if(isBuildPartLogic()){
//				//与之前所有可售计算一致，包括停车
...
//			}
	...
	public void setUnionData() {
		...
		String[] cols = new String[entrys.size() + 1];
		for (int i = 0; i < entrys.size(); i++) {
			cols[i] = "split" + i;
		}
		...
		SwingUtilities.invokeLater(new Runnable(){
			public void run() {
				try {
					...
					measureCostEditUI.handUIExceptionAndAbort(e);
				}
			}
		});
		afterSetUnionData();
		...
		for (int i = 0; i < table.getRowCount(); i++) {
			...
			if (row.getUserObject() instanceof CostAccountInfo) {
				...
				if (allTotal != null) {
					...
					if (FDCHelper.toBigDecimal(allBuildArea).compareTo(FDCHelper.ZERO) != 0) {
						BigDecimal avg = FDCHelper.divide(allTotal, allBuildArea, 2, BigDecimal.ROUND_HALF_UP);
						row.getCell("buildAvg").setValue(avg);
					}
				}
			}
			for (int j = 0; j < entrys.size(); j++) {
				...
				BigDecimal sellArea = entry.getSellArea();
				BigDecimal buildArea = entry.getBuildArea();
				BigDecimal total = FDCHelper.toBigDecimal(row.getCell("split" + j).getValue());
				if (FDCHelper.isZero(total)) {
					continue;
				}
				if (sellArea != null) {
					BigDecimal avg = FDCHelper.divide(total, sellArea, 2, BigDecimal.ROUND_HALF_UP);
					row.getCell("avg" + j).setValue(avg);
				}
				if (buildArea != null) {
					BigDecimal avg = FDCHelper.divide(total, buildArea, 2, BigDecimal.ROUND_HALF_UP);
					row.getCell("buildAvg" + j).setValue(avg);
				}
			}
		}
	}
	private void tableEditStopped(com.kingdee.bos.ctrl.kdf.table.event.KDTEditEvent e) {
		...
		if (table.getColumnKey(columnIndex).startsWith("split") 
			...
			if (obj instanceof Item && ((Item) obj).key.equals("man")) {
				row.getCell(columnIndex).setValue(FDCHelper.toBigDecimal(row.getCell(columnIndex).getValue()));
				...
				if (FDCHelper.toBigDecimal(sellArea).signum() != 0) {
					row.getCell("avg" + index).setValue(FDCHelper.divide(newValue, sellArea, 2, BigDecimal.ROUND_HALF_UP));
				}
				if (FDCHelper.toBigDecimal(buildArea).signum() != 0) {
					row.getCell("buildAvg" + index).setValue(FDCHelper.divide(newValue, buildArea, 2, BigDecimal.ROUND_HALF_UP));
				}
				setUnionData();
			}
		}
		...
		if (isManSplit) {
			...
			for (int i = 0; i < entrys.size(); i++) {
				...
				if (i == entrys.size() - 1) {
					...
					splitAmt = setSplitAmt(row, amt, splitAmt, i);
				} else {
					lastIndex = table.getColumnIndex("split" + i);
					String productId = entrys.get(i).getProduct().getId().toString();
					amt = getCustomIndex((Item) obj,productId);
					splitAmt = FDCHelper.divide(FDCHelper.toBigDecimal(amt).multiply(total), sum, 2, BigDecimal.ROUND_HALF_UP);
				...
				if (amt.compareTo(FDCHelper.ZERO) == 0)
					continue;
				BigDecimal avgAmt = FDCHelper.divide(splitAmt, amt, 2, BigDecimal.ROUND_HALF_UP);
				row.getCell("avg" + i).setValue(avgAmt);

				amt = FDCHelper.toBigDecimal(entrys.get(i).getBuildArea());
				if (amt.compareTo(FDCHelper.ZERO) == 0)
					continue;
				avgAmt = FDCHelper.divide(splitAmt, amt, 2, BigDecimal.ROUND_HALF_UP);
			...
			if (sumSellArea.signum() != 0) {
				row.getCell("avg").setValue(FDCHelper.divide(total, sumSellArea, 2, BigDecimal.ROUND_HALF_UP));
			}
			if (sumBuildArea.signum() != 0) {
				row.getCell("buildAvg").setValue(FDCHelper.divide(total, sumBuildArea, 2, BigDecimal.ROUND_HALF_UP));
			}
		}
	...
	public BigDecimal setSplitAmt(IRow row, BigDecimal amt, BigDecimal splitAmt, int i) {
		if (FDCHelper.isZero(amt) && !FDCHelper.isZero(splitAmt) && i > 0) {
			for (int m = i; m > 0; m--) {
				if (!FDCHelper.isZero(row.getCell("split" + (m - 1)).getValue())) {
					row.getCell("split" + (m - 1)).setValue(FDCHelper.add(row.getCell("split" + (m - 1)).getValue(), splitAmt));
					break;
				}
			}
			splitAmt = FDCHelper.ZERO;
			return splitAmt;
		}
		return splitAmt;
	}
	...
	//指定分摊名称
	private static String ZHIDINGFENTAN_TYPE_NAME = AimMeasureCostEditUI.Item.SPLITITEMS[0].name;

	/**
	 * 描述：检测指定分摊是否正确  BT701375
	 * @Author：jian_cao
	 * @CreateTime：2012-8-10
	 */
	public void checkSpecifiedShare() {
		
		int rows = this.table.getRowCount();
		for (int i = 0; i < rows; i++) {
			IRow row = this.table.getRow(i);
			//获取分配方式列
			ICell cell = row.getCell("splitType");
			//如果是分配方式是指定分摊就检测分摊是否正确
			if (cell.getValue() != null && ZHIDINGFENTAN_TYPE_NAME.equals(cell.getValue().toString())) {
				//分配方式列的下标
				int columnIndex = cell.getColumnIndex();
				//合计
				Object oTotal = row.getCell(--columnIndex).getValue();
				if (null != oTotal) {
					BigDecimal total = FDCHelper.toBigDecimal(oTotal, 2);
					//合计大于0
					if (total.compareTo(FDCNumberHelper.ZERO) > 0) {
						int entrySize = this.entrys.size();
						//循环总成本项，拿总计金额减掉每项的金额。
						for (int j = 0; j < entrySize; j++) {
							Object oValue = row.getCell(--columnIndex).getValue();
							if (null != oValue) {
								total = total.subtract(FDCHelper.toBigDecimal(oValue, 2));
							}
						}
						//如果不等于0就表示分摊金额不正确
						if (total.compareTo(FDCNumberHelper.ZERO) != 0) {
							MsgBox.showWarning("指定分摊不等于合计，不能提交或保存！");
							SysUtil.abort();
						}
					} 
				}
