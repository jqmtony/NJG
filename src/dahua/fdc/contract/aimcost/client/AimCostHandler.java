...
//			this.aimCost.setRecenseDate(DateTimeUtils.truncateDate(new Date()));
			...
			vie.setFilter(filter);// 一失足成千古恨
			filter.getFilterItems().add(new FilterItemInfo("isEnabled", Boolean.TRUE));
			vie.getSorter().add(new SorterItemInfo("number"));
			MeasureStageCollection measureStageCollection = MeasureStageFactory.getRemoteInstance().getMeasureStageCollection(vie);
			if (measureStageCollection == null || measureStageCollection.size() == 0) {
				FDCMsgBox.showWarning("没有启用测算阶段!");
				SysUtil.abort();
			}
			aimCost.setMeasureStage(measureStageCollection.get(0));
			
		}
		if (aimCost != null && aimCost.getOrgOrProId() != null)
			if (aimCost.getMeasureStage() != null && aimCost.getMeasureStage().getId() != null) {
				acctStageMap = AccountStageHelper.initAccoutStageMap(null, aimCost.getOrgOrProId(), aimCost.getMeasureStage().getId()
						.toString());
			} else {
				acctStageMap = AccountStageHelper.initAccoutStageMap(null, aimCost.getOrgOrProId(), null);
			}
	}
...
//			fillNode(table, (DefaultMutableTreeNode) root.getChildAt(i));
			...
			child = AccountStageHelper.initChild(acctStageMap,child);
			child = AccountStageHelper.initIsLeaf(child);
	...
	public void storeRow(IRow row) {
		...
		info.setChangeReason((String) row.getCell("changeReason").getValue());
...
/*		AimCostCollection aimCostCollection = AimCostFactory
		...
		this.aimCost.setIsLastVersion(true);
		this.aimCost.setGenByAdjust(null);
//		this.aimCost.getCostEntry().clear();
		handleAimCostAccredit(aimCost);
		//处理修订
