...
public class PlanIndexTable {
	...
	public PlanIndexTable(PlanIndexInfo planIndexInfo,AimMeasureCostEditUI measureCostEditUI) throws EASBizException, BOSException{
		...
		costInfo = (MeasureCostInfo) measureCostEditUI.getEditData();
...
//						if(getPlanIndexEntryInfo(product.getId().toString())!=null){
							...
							if(measureCostEditU.getplTables().getSelectedIndex()!=1){
...
//			}
	...
	*/
			...
			/*
			 *  start 常旭加的奥园的新需求： Added by Owen_wen 2013-5-8  
			 *  容积率 改为： 计容积率面积/总占地面积，
			 *  建筑密度 改为： 建筑物占地面积/总占地面积
			 */
			BigDecimal allContainArea = FDCHelper.toBigDecimal(totalContainAreaCell.getValue());
			if (allContainArea.signum() == 0) {
				buildDensityCell.setValue(null);
				cubageRateCell.setValue(null);
			}else{
				cubageRateCell.setValue(FDCHelper.divide(cubageRateAreaCell.getValue(), allContainArea, 2, BigDecimal.ROUND_HALF_UP));
				buildDensityCell.setValue(FDCHelper.divide(buildContailAreaCell.getValue(), allContainArea, 4, BigDecimal.ROUND_HALF_UP));
			}
			// end 常旭加的奥园的新需求：容积率 改为： 计容积率面积/总占地面积， Added by Owen_wen 2013-5-8 
...
//				sumMap.put("hu", FDCNumberHelper.add(sumMap.get("hu"), table.getCell(i, huColummnIndex).getValue()));
			...
			for(int i=houseSubIndex+1;i<businessSubIndex;i++){
				...
				/* modified by zhaoqin for R131030-0178 on 2013/12/11 start */
				sumMap.put("elevators", FDCNumberHelper.add(sumMap.get("elevators"), table.getCell(i, elevatorsIndex).getValue()));
				sumMap.put("floors", FDCNumberHelper.add(sumMap.get("floors"), table.getCell(i, floorsIndex).getValue()));
				sumMap.put("floorHeight", FDCNumberHelper.add(sumMap.get("floorHeight"), table.getCell(i, floorHeightIndex).getValue()));
				/* modified by zhaoqin for R131030-0178 on 2013/12/11 end*/
			}
...
//					filter.getFilterItems().add(new FilterItemInfo("forCostApportion",Boolean.TRUE));
					...
					measureCostEditU.handUIExceptionAndAbort(e1);
				} catch (EASBizException e2) {
					measureCostEditU.handUIExceptionAndAbort(e2);
				}
				view.setFilter(filter);
...
//		tblDetail_desc_TextField.setName("tblDetail_desc_TextField");
		...
		constrTable = new KDTable(4,1,0);
		 ...
		 this.constrTable.addKDTEditListener(new com.kingdee.bos.ctrl.kdf.table.event.KDTEditAdapter() {
	            public void editStopped(com.kingdee.bos.ctrl.kdf.table.event.KDTEditEvent e) {
	            	for (int i = 0; i < constrTable.getRowCount(); i++) {
	        			KDTableHelper.autoFitRowHeight(constrTable, i);
	        		}
	            }
	        });
...
//		constrTable.getColumn(1).setEditor(tblDetail_desc_CellEditor);
//		constrTable.getColumn(1).getStyleAttributes().setWrapText(true);
		FDCTableHelper.autoFitRowHeight(constrTable, "indexName1", 80);
		constrTable.getColumn(2).setKey("indexName2");
		constrTable.getColumn(2).setEditor(tblDetail_desc_CellEditor);
		constrTable.getColumn(2).getStyleAttributes().setWrapText(true);
		constrTable.getColumn(3).setKey("indexName3");
		constrTable.getColumn(3).setEditor(tblDetail_desc_CellEditor);
		constrTable.getColumn(3).getStyleAttributes().setWrapText(true);
		...
		for(Iterator iter2=info.getConstrEntrys().iterator();iter2.hasNext();){
			...
			/* modified by zhaoqin for R20150206-0232 on 2015/03/04 start */
			//添加产品列
			if(headRow.getCell(productId) == null) {
				IColumn column = constrTable.addColumn();
    			column.setWidth(150);
    			column.setKey(productId);
    			autoFitRowHeight(constrTable, productId, 1200);
    			headRow.getCell(productId).setValue(indexProduct.getName());
    			headRow.getCell(productId).setUserObject(indexProduct);
    			rowIndex = 0;
			}
			IRow rowdata = constrTable.getRow(rowIndex);
			if(null == rowdata) {
				rowdata = constrTable.addRow();
				rowdata.getCell("id").setValue(indexEntry.getId().toString());
				rowdata.getCell("indexName1").setValue(indexEntry.getIndexName1());
				rowdata.getCell("indexName2").setValue(indexEntry.getIndexName2());
				rowdata.getCell("indexName3").setValue(indexEntry.getIndexName3());
			}
			rowdata.getCell(productId).setValue(indexEntry.getDesc());
			rowdata.getCell(productId).setUserObject(indexEntry);
        	rowIndex++;
        	
        	if(true)continue;
        	/* modified by zhaoqin for R20150206-0232 on 2015/03/04 end */
		...
		for(int j=4;j<constrTable.getColumnCount();j++){
			...
			for(int i=0;i<constrTable.getRowCount();i++){
				...
				if(entry==null){
					...
					/* modified by zhaoqin for R20150206-0232 on 2015/03/04 */
					entry.setId(BOSUuid.create(entry.getBOSType()));
				}
				...
				entry.setSeq(0);	/* modified by zhaoqin for R20150206-0232 on 2015/03/04 start */
...
//        kdc.setRequired(false);
	...
	/*
	 * KDMultiLangArea控件存在问题，设置在列上后，单元格之间互相影响，先用KDTextArea。
	 */
	public static void autoFitRowHeight(KDTable table, String cloumnKey, int maxLength){
		// 设置 cloumnKey 列的样式
		KDTextArea textArea = new KDTextArea();
		textArea.setMaxLength(maxLength);
		textArea.setAutoscrolls(true);
		textArea.setEditable(true);
		
		
		KDTDefaultCellEditor cellEditor = new KDTDefaultCellEditor(textArea);
		table.getColumn(cloumnKey).getStyleAttributes().setWrapText(true);
		table.getColumn(cloumnKey).setEditor(cellEditor);
...
//		KDScrollPane textArea = new KDTextAreaCtrl();
