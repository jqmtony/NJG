 ...
 * output class name
 */
public class TempProductAimCostUI extends AbstractProductAimCostUI {
...
//		if(true) return true;
	...
	protected void treeMain_valueChanged(javax.swing.event.TreeSelectionEvent e)
		...
		this.verifySaved();
		this.productTypeGetter = new AimProductTypeGetter(selectObjId,ProjectStageEnum.DYNCOST);
		resetProductCol();
		setApporAction();
		fillTable();
//		int acctNameIndex=tblMain.getColumn("acctName").getColumnIndex()+1;
...
//		Color lockColor = new Color(0xF0AAD9);
	 ...
	 */
	public TempProductAimCostUI() throws Exception {
		super();
	}
		...
		for (int i = 0; i < this.tblMain.getRowCount(); i++) {
			...
			if (row.getUserObject() != null) {
				...
				if (isEdited) {
					...
					FilterInfo filter = new FilterInfo();
					filter.getFilterItems().add(
							new FilterItemInfo("costEntryId", entryId));
					AimCostProductSplitEntryFactory.getRemoteInstance().delete(
							filter);
					AimCostProductSplitEntryCollection rowSetting = this
							.getRowSetting(row);
					// String apporId = (String) ((Map) row.getCell(
					// "apportionType").getUserObject()).get("newId");
					...
					for (int j = 0; j < rowSetting.size(); j++) {
						...
						// if (apporId != null) {
						// ApportionTypeInfo apporInfo = new
						// ApportionTypeInfo();
						// apporInfo.setId(BOSUuid.read(apporId));
						// info.setApportionType(apporInfo);
						// }
						info.setSplitAmount((BigDecimal) calData.get(info
								.getProduct().getId().toString()));
						if (info.getApportionType() != null) {
							AimCostProductSplitEntryFactory.getRemoteInstance()
									.addnew(info);
						}
					}
