 ...
 * 描述：各产品类型目标成本表
 * 
 * 修改：非明细改造参考ProductDynamicCostUI
 */
		...
		for (int i = 0; i < table.getRowCount(); i++) {
			...
			if (row.getUserObject() == null) {
				...
				for (int j = i + 1; j < table.getRowCount(); j++) {
					...
					}else if (rowAfter.getCell("isEnterDB").getUserObject() != null) {
						//成本科目多级改造
						rowList.add(rowAfter);
					}
				}
	...
	public void fillTableNotLeaf() throws Exception{
		...
		if(costAccounts!=null){
			for(Iterator it=costAccounts.iterator();it.hasNext();){
				...
				row.getCell("isEnterDB").setUserObject(costAccountInfo);
	...
	private void fetchAndFill(String selectObjId) throws EASBizException,
		...
		dialog.setLongTimeTask(new ILongTimeTask() {
			public Object exec() throws Exception {
				...
				DefaultKingdeeTreeNode node = (DefaultKingdeeTreeNode) treeMain
						.getLastSelectedPathComponent();
				acctStageMap=AccountStageHelper.initAccoutStageMap(null,selectObjId, null);
				if (node.isLeaf()
...
//		find bugs EXP 类问题：局部变量定义，但是没有使用到
//		Color lockColor = new Color(0xF0AAD9);
	...
	private static final Logger logger = CoreUIObject.getLogger(AIMProductAimCostUI.class);
...
//		leafPrjIds.clear();
//		leafPrjIds.add("+4Tw2k00SZOiXFZ0KgiQEvnl6Ss=");
