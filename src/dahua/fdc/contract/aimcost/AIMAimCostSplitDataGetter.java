...
public class AIMAimCostSplitDataGetter extends CostDataGetter implements Serializable {
	...
	private static Logger logger = Logger.getLogger(AIMAimCostSplitDataGetter.class);
	...
	/*
	 * 新增接口在目标成本分摊处用 @param objectId @param productTypeGetter
	 */
	...
	private void init(String objectId, AimProductTypeGetter productTypeGetter) throws BOSException, SQLException, EASBizException {
	...
	private Map getAimProductData(CostEntryInfo entry, AIMAimCostProductSplitEntryCollection splits) throws BOSException {
		...
		for (int j = 0; j < splits.size(); j++) {
			...
			amount = FDCHelper.subtract(amount, firstSplit.getDirectAmount());
		}
		// 根据分摊类型拆分
		if (splits.size() > 1) {
			...
			for (int j = 0; j < splits.size(); j++) {
				...
				BigDecimal appValue = FDCHelper.ZERO;
				if (ApportionTypeInfo.appointType.equals(apportionType.getId().toString())) {
					appValue = splits.get(j).getAppointAmount() == null ? FDCHelper.ZERO : splits.get(j).getAppointAmount();
				} else {
					appValue = this.productTypeGetter.getProductApprotion(productSplitId, apportionType.getId().toString());
				}
	...
	private void initAimCostData(String objectId) throws BOSException, SQLException, EASBizException {
		...
		if (ctx != null) {
			/**
			 * 此处在后台事物调用中，有可能出现财务组织为空的情况，后台事物的上下文是通过模拟登陆获取的
			 * FDC_PARAM_AIMCOSTAUDIT参数是集团控制的，故修改此参数值的获取
			 */
			System.out.println("                            testaaa" + ctx.toString());
			FDCSQLBuilder builder = new FDCSQLBuilder(ctx);
			builder
					.appendSql("select fvalue_l2 from T_BAS_ParamItem where fkeyid =(select fid from T_BAS_Param where fnumber ='FDC5001006_AIMCOSTAUDIT')");
			IRowSet rowSet = builder.executeQuery();
			while (rowSet.next()) {
				if ("true".equals(rowSet.getString(1))) {
