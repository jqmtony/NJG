...
public class IncomeAccountControllerBean extends AbstractIncomeAccountControllerBean
{
    private static Logger logger =
        Logger.getLogger("com.kingdee.eas.fdc.basedata.app.IncomeAccountControllerBean");
    
    protected void _update(Context ctx, IObjectPK pk, IObjectValue model) throws BOSException, EASBizException {
		trimBlank(model);
		IncomeAccountInfo newCai = (IncomeAccountInfo) model;
		//分配下去的需要跟着修改,下级节点编码不能出现断层
		if (newCai.isIsSource()) {
			//新的长编码  。收入科目不需要 类型、是否成本科目。
			String newLn = newCai.getLongNumber();
			String newName = newCai.getName();
			//String newType = newCai.getType() == null ? "" : newCai.getType().getValue();
			//Integer iIncomeAccount = newCai.isIsIncomeAccount() ? Integer.valueOf("1") : Integer.valueOf("0");
			
			//未保存之前,获取其原来的长编码、类型、是否成本科目		
			IncomeAccountInfo  oldCai = IncomeAccountFactory.getLocalInstance(ctx).getIncomeAccountInfo(pk);
			...
			if (oldCai.getCurProject() != null) {
				sql = "select fid from t_fdc_IncomeAccount where FCurProject=? and FLongNumber like ?";
				params = new Object[]{oldCai.getCurProject().getId().toString(),oldLn + "!%"};
			}else if (oldCai.getFullOrgUnit() != null) {
				sql = "select fid from t_fdc_IncomeAccount where FFullOrgUnit=? and FLongNumber like ?";
			...
			params = null;
			//sql =  "update T_FDC_IncomeAccount set FLongNumber=?,FNumber=?,FName_l2=?,FType=?,FIsIncomeAccount=? where fsrcIncomeAccountid=?";	
			//params = new Object[]{newLn,newCai.getNumber(),newCai.getName(),newType,iIncomeAccount,oldCai.getId().toString()};
			sql =  "update T_FDC_IncomeAccount set FLongNumber=?,FNumber=?,FName_l2=? where fsrcIncomeAccountid=?";
			params = new Object[]{newLn,newCai.getNumber(),newCai.getName(),oldCai.getId().toString()};
			DbUtil.execute(ctx,sql,params);
			
			FDCSQLBuilder builder = new FDCSQLBuilder();
			if(set.size()>0){
				//修改当前组织或项目内所有下级科目的长编码、类型、是否成本科目 属性
				//builder.appendSql("update T_FDC_IncomeAccount set FLongNumber=REPLACE(FLongNumber,?,?),FType=?,FIsIncomeAccount=? where ");
				builder.appendSql("update T_FDC_IncomeAccount set FLongNumber=REPLACE(FLongNumber,?,?) where ");
				builder.addParam(oldLn + "!");
				builder.addParam(newLn + "!");
				//builder.addParam(newType);
				//builder.addParam(iIncomeAccount);
				...
				//builder.addParam(newType);
				//builder.addParam(iIncomeAccount);
				builder.appendParam("fsrcIncomeAccountid", set.toArray());
				builder.executeUpdate(ctx);	
			}
			...
			if (!newName.equals(oldName)) {
				...
				builder.clear();
				builder.appendSql("select FID,FLongNumber,FFullOrgUnit,FCurproject from T_FDC_IncomeAccount where ");
				builder.appendParam("fid",set.toArray());
				...
				try{
					while (rs.next()) {
						...
						sql = "update T_FDC_IncomeAccount set FDisplayName_L2=? where FID=? OR fsrcIncomeAccountid=?";
	...
	private String getNewDisplayName(Context ctx, String longNumber,String orgUnit,String curProject) throws BOSException {
		...
		FDCSQLBuilder builder = new FDCSQLBuilder();
		builder.appendSql("select FName_L2 from T_FDC_IncomeAccount where ");
		if(orgUnit == null){
	...
	protected IObjectPK _addnew(Context ctx, IObjectValue model) throws BOSException, EASBizException {
		trimBlank(model);
		IncomeAccountInfo IncomeAccountInfo = (IncomeAccountInfo) model;
/*		if(IncomeAccountInfo.getParent() != null) {
			_isReferenced(ctx, new ObjectUuidPK(IncomeAccountInfo.getParent().getId()));
		}*/
		
		// 集团分配科目后，使用了一段时间，如果要新增科目，保存时需要检查下级公司、工程项目是否已建了相同的科目（科目编码相同）。
		if ((IncomeAccountInfo.getFullOrgUnit() != null)) {//挂组织节点
			// &&((OrgConstants.DEF_CU_ID).equals(IncomeAccountInfo.getFullOrgUnit().getId().toString()))){
			// 当前要新增的科目是挂在集团上的
			// IIncomeAccount iIncomeAccount =
			// IncomeAccountFactory.getLocalInstance(ctx);
			if(OrgConstants.DEF_CU_ID.equals(IncomeAccountInfo.getFullOrgUnit().getId().toString())){			
				if (IncomeAccountInfo.getParent() != null) {// 是新增子节点
					String longNumber = IncomeAccountInfo.getParent().getLongNumber() + "!" + IncomeAccountInfo.getNumber();
					EntityViewInfo evi = new EntityViewInfo();
					FilterInfo filter = new FilterInfo();
					filter.getFilterItems().add(new FilterItemInfo("longNumber", longNumber));
					evi.setFilter(filter);
					// IncomeAccountInfo IncomeAccount
					// =iIncomeAccount.getIncomeAccountInfo("select where longNumber =
					// '" +longNumber+ "'");
					IncomeAccountCollection cac = getIncomeAccountCollection(ctx, evi);
					if (cac.size() != 0) {
						throw new com.kingdee.eas.fdc.basedata.FDCBasedataException(FDCBasedataException.INCOMEACCOUNT_ADD_SUBHAVE);
					}
					
					IncomeAccountInfo.setIsEnabled(IncomeAccountInfo.getParent().isIsEnabled());
				}
			}
		}
		/*
		String sql = "update t_fdc_IncomeAccount set FFlag = 1 where flongnumber like '5001!08!03%'";
		DbUtil.execute(ctx,sql);
		
		String sqlOR = "update t_fdc_IncomeAccount set FFlag = 1 where flongnumber like '4301!408!03%'";
		DbUtil.execute(ctx,sqlOR);
		*/
		IObjectPK pk = super._addnew(ctx, IncomeAccountInfo);
		//新增时同时也要新增对应关系
		/*
	...
	protected boolean _enable(Context ctx, IObjectPK pk) throws BOSException, EASBizException {
		...
		IncomeAccountInfo = getIncomeAccountInfo(ctx, pk);
		
		
		if (IncomeAccountInfo.getParent() != null) {
			IObjectPK parentPK = new ObjectStringPK(IncomeAccountInfo.getParent().getId().toString());
			IncomeAccountInfo parentIncomeAccountInfo = getIncomeAccountInfo(ctx, parentPK);
			if (!parentIncomeAccountInfo.isIsEnabled()) {
				// 如果上级被禁用,给出提示并返回
				throw new com.kingdee.eas.fdc.basedata.FDCBasedataException(FDCBasedataException.INCOMEACCOUNT_PARENT_DISENABLE);
			} else {
...
//			4301.401.01.01是由上级分配下来的科目，由上级禁用掉该科目，下级启用后上级组织的该科目没有自动启用
			if(IncomeAccountInfo.getSrcIncomeAccountId() != null) {
				DbUtil.execute(ctx, "update T_FDC_IncomeAccount set  FIsEnable = 1 where fid = ?", new Object[]{IncomeAccountInfo.getSrcIncomeAccountId()});
			}
		}
		return flag;
	}
	...
	protected boolean _disable(Context ctx, IObjectPK pk) throws BOSException, EASBizException {	
		...
		IncomeAccountInfo IncomeAccountInfo = this.getIncomeAccountInfo(ctx, pk);

		/* hcq temp
		 
    	if((IncomeAccountInfo.getFullOrgUnit()!=null)&&(OrgConstants.DEF_CU_ID.equals(IncomeAccountInfo.getFullOrgUnit().getId().toString()))){
    		...
    		checkExistsSql.append("select A.FIncomeAccountID AS FID FROM T_AIM_CostEntry A 		\r\n");
    		checkExistsSql.append("INNER JOIN T_FDC_IncomeAccount B ON A.FIncomeAccountID=B.FID 		\r\n");
    		...
    		checkExistsSql.append("INNER JOIN T_FDC_IncomeAccount B ON A.FAccountID=B.FID 		\r\n");
    		checkExistsSql.append("where B.FFullOrgUnit <>? and B.FLongNumber like ? 		\r\n");
    		checkExistsSql.append("UNION 		\r\n");
    		checkExistsSql.append("select A.FIncomeAccountID AS FID from T_AIM_COSTSPLITENTRY A 		\r\n");
    		checkExistsSql.append("INNER JOIN T_FDC_IncomeAccount B ON A.FIncomeAccountID=B.FID 		\r\n");
    		checkExistsSql.append("where B.FFullOrgUnit <>? and B.FLongNumber like ? 		\r\n");
    		checkExistsSql.append("UNION 		\r\n");
    		checkExistsSql.append("select A.FIncomeAccountID AS FID from T_AIM_MEASUREENTRY A 		\r\n");
    		checkExistsSql.append("INNER JOIN T_FDC_IncomeAccount B ON A.FIncomeAccountID=B.FID 		\r\n");
    		checkExistsSql.append("where B.FFullOrgUnit <>? and B.FLongNumber like ? 		\r\n");
    	...
    	* 
		...
		if (this.checkIsUsed(ctx, pk)) {//本节电下判断引用
			throw new com.kingdee.eas.fdc.basedata.FDCBasedataException(FDCBasedataException.DISENABLE_CANNOT_USED2);
		}
	...
	private boolean checkIsOnlyOneEnabled(Context ctx, IObjectPK PK) throws BOSException, EASBizException {
		IncomeAccountInfo cai = this.getIncomeAccountInfo(ctx, PK).getParent();
		if (cai != null) {
			EntityViewInfo evi = new EntityViewInfo();
			FilterInfo filter = new FilterInfo();
			...
			evi.setFilter(filter);
			if (this.getIncomeAccountCollection(ctx, evi).size() == 1) {
				return true;
			}
		}
		return false;
	...
	private boolean checkIsUsed(Context ctx, IObjectPK PK) throws EASBizException, BOSException {
		...
		EntityViewInfo evi = new EntityViewInfo();
		FilterInfo filter = new FilterInfo();
		filter.getFilterItems().add(new FilterItemInfo("parent.id", IncomeAccountInfo.getId().toString()));
		evi.setFilter(filter);
		IncomeAccountCollection IncomeAccountCollection = this.getIncomeAccountCollection(ctx, evi);
		// 如果有下级,先判断自己,再判断下级
		...
		if (IncomeAccountCollection.size() > 0) {
			...
			if ((CostEntryFactory.getLocalInstance(ctx).exists("select IncomeAccount where IncomeAccount.id ='" + PK.toString() + "'"))
					|| (DynamicCostFactory.getLocalInstance(ctx).exists("select IncomeAccount where account.id ='" + PK.toString() + "'"))){
//					|| (ContractCostSplitEntryFactory.getLocalInstance(ctx).exists("select IncomeAccount where IncomeAccount.id ='" + PK.toString() + "'"))) {
				return true;
			} else {
				// 判断下级
				for (int i = 0; i < IncomeAccountCollection.size(); i++) {
					if ((CostEntryFactory.getLocalInstance(ctx).exists("select IncomeAccount where IncomeAccount.id ='" + IncomeAccountCollection.get(i).getId().toString() + "'"))
							|| (DynamicCostFactory.getLocalInstance(ctx).exists("select IncomeAccount where account.id ='" + IncomeAccountCollection.get(i).getId().toString() + "'"))){
//							|| (ContractCostSplitEntryFactory.getLocalInstance(ctx).exists("select IncomeAccount where IncomeAccount.id ='" + IncomeAccountCollection.get(i).getId().toString() + "'"))) {
						return true;
					}else{
						checkIsUsed(ctx, new ObjectStringPK(IncomeAccountCollection.get(i).getId().toString()));
					}
				}
			}
		} else {
...
//					|| (ContractCostSplitEntryFactory.getLocalInstance(ctx).exists("select IncomeAccount where IncomeAccount.id ='" + PK.toString() + "'"))) {
	...
	protected boolean changeUseStatus(Context ctx, IObjectPK PK, boolean flag) throws EASBizException, BOSException {
		// IIncomeAccount iIncomeAccount = IncomeAccountFactory.getLocalInstance(ctx);
		IncomeAccountInfo IncomeAccountInfo = getIncomeAccountInfo(ctx, PK);
		if((!flag)&&(IncomeAccountInfo.getFullOrgUnit()!=null)&&(OrgConstants.DEF_CU_ID.equals(IncomeAccountInfo.getFullOrgUnit().getId().toString()))){
			//如果是集团下的禁用操作,前面已经校验了,此处通过了校验,直接sql处理候艳提要求录bug,裴红说要这样处理的20061124
			String longNumber = IncomeAccountInfo.getLongNumber();
//    		IncomeAccountCollection cac = this.getIncomeAccountCollection(ctx,"select longNumber where longNumber like '" + longNumber + "%'");//fullOrgUnit.id =  '" + OrgConstants.DEF_CU_ID + "' and
//    	    if(cac.size()!=0){
	    		String sql = "update T_FDC_IncomeAccount set  FIsEnable = 0  where FLongNumber like '" + longNumber + "%'";	    		
//	    	    Object[] params ;
...
//    	    }
		...
		if((flag)&&(IncomeAccountInfo.getFullOrgUnit()!=null)&&(OrgConstants.DEF_CU_ID.equals(IncomeAccountInfo.getFullOrgUnit().getId().toString()))){
			...
			String longNumber = IncomeAccountInfo.getLongNumber();
			String sql = "update T_FDC_IncomeAccount set  FIsEnable = 1  where FLongNumber like '" + longNumber + "%'";
			DbUtil.execute(ctx,sql);
		}
		else{
			EntityViewInfo evi = new EntityViewInfo();
			...
			filter.getFilterItems().add(new FilterItemInfo("parent.id", IncomeAccountInfo.getId().toString()));
			evi.setFilter(filter);
			IncomeAccountCollection IncomeAccountCollection = getIncomeAccountCollection(ctx, evi);
			// 如果有下级,要同时启用/禁用下级
			if (IncomeAccountCollection.size() > 0) {
				// 先启用/禁用自己
				String sql = "update T_FDC_IncomeAccount set  FIsEnable = ?  where fid = ? ";	    		
				Object[] params = new Object[]{Boolean.valueOf(flag),IncomeAccountInfo.getId().toString()};
    	    	DbUtil.execute(ctx,sql,params);
//				IncomeAccountInfo.setIsEnabled(flag);
//				_update(ctx, PK, IncomeAccountInfo);
				// 再启用/禁用下级
				IncomeAccountInfo childContractTypeInfo;
				IObjectPK childPK;
				for (int i = 0; i < IncomeAccountCollection.size(); i++) {
					if (Boolean.valueOf(IncomeAccountCollection.get(i).isIsEnabled()) != Boolean.valueOf(flag)) {
						childContractTypeInfo = IncomeAccountCollection.get(i);
...
//				_update(ctx, PK, IncomeAccountInfo);
	...
	protected void _checkNumberDup(Context ctx, IObjectValue model) throws BOSException, EASBizException {
		...
		if (treeModel.innerGetParent() == null) {
			...
			if (((IncomeAccountInfo) treeModel).getCurProject() != null) {
				filter.getFilterItems().add(new FilterItemInfo("curproject.id", ((IncomeAccountInfo) treeModel).getCurProject().getId().toString(), CompareType.EQUALS));
			} else if (((IncomeAccountInfo) treeModel).getFullOrgUnit() != null) {
				filter.getFilterItems().add(new FilterItemInfo("fullorgunit.id", ((IncomeAccountInfo) treeModel).getFullOrgUnit().getId().toString(), CompareType.EQUALS));
			}
	...
	protected void checkLNForTree(Context ctx, TreeBaseInfo treeBaseInfo) throws BOSException, EASBizException, TreeBaseException {
		...
		if (((IncomeAccountInfo) treeBaseInfo).getCurProject() != null) {
			lNfilter.getFilterItems().add(new FilterItemInfo("curproject.id", ((IncomeAccountInfo) treeBaseInfo).getCurProject().getId().toString(), CompareType.EQUALS));
		} else if (((IncomeAccountInfo) treeBaseInfo).getFullOrgUnit() != null) {
			lNfilter.getFilterItems().add(new FilterItemInfo("fullorgunit.id", ((IncomeAccountInfo) treeBaseInfo).getFullOrgUnit().getId().toString(), CompareType.EQUALS));
		}
	...
	protected void _delete(Context ctx, IObjectPK pk) throws BOSException, EASBizException {
		IncomeAccountInfo oldModel = this.getIncomeAccountInfo(ctx, pk);
	...
	protected void _importDatas(Context ctx, IObjectCollection cac, BOSUuid addressId) throws BOSException, EASBizException {
		...
		IncomeAccountCollection IncomeAccountCollection2 = getIncomeAccountCollection(ctx, view);
		Map acMap = new HashMap();
		for (Iterator iter = IncomeAccountCollection2.iterator(); iter.hasNext();) {
			IncomeAccountInfo element = (IncomeAccountInfo) iter.next();
			acMap.put(element.getLongNumber(), element);
		}
		
		IncomeAccountCollection IncomeAccountCollection = (IncomeAccountCollection) cac;
		IncomeAccountInfo cai;
		SelectorItemCollection seletors = new SelectorItemCollection();
		seletors.add("*");
		seletors.add("parent.isAssigned");
		for (int i = 0; i < IncomeAccountCollection.size(); i++) {
			cai = getIncomeAccountInfo(ctx, new ObjectUuidPK(IncomeAccountCollection.get(i).getId().toString()), seletors);
			
			if(cai.getParent() != null) { 
				String longNumber = cai.getLongNumber();
				String parentLN = longNumber.substring(0, longNumber.lastIndexOf("!"));
				IncomeAccountInfo ca = (IncomeAccountInfo)acMap.get(parentLN);
				if(ca!=null){
					if(ca.getId() != null && ca.isIsLeaf()) {
						_isReferenced(ctx, new ObjectStringPK(ca.getId().toString()));
					}
				}else{
					IncomeAccountCollection coll = getIncomeAccountCollection(ctx, "select id,isleaf where longNumber = '" + cai.getLongNumber().substring(0, cai.getLongNumber().lastIndexOf('!')) + "' and curProject.id = '"
							+ addressId.toString() + "'");
					ca = coll.get(0);
				}
				...
				if(ca.isIsLeaf()){
					String sql = "update T_FDC_IncomeAccount set  FIsLeaf = ?  where fid = ? ";	    		
			...
			cai.setSrcIncomeAccountId(null);
	...
	protected ArrayList _importTemplateDatas(Context ctx, IObjectCollection cac) throws BOSException, EASBizException {
		...
		if (IncomeAccountCollection.size() != 0) {
			for (int i = 0; i < cac.size(); i++) {
				cai = getIncomeAccountInfo(ctx, new ObjectUuidPK(IncomeAccountCollection.get(i).getId().toString()));
				...
				if (cai.getParent() != null) {
					...
					String sql = "select FID from t_fdc_IncomeAccount where flongNumber = ?  and ffullOrgUnit = ? ";
					Object[] params = new Object[] { ln, OrgConstants.DEF_CU_ID };
					RowSet rs = DbUtil.executeQuery(ctx, sql, params);
					try {
						if (rs.next()) {
							IncomeAccountInfo parent = new IncomeAccountInfo();
	...
	private IObjectValue trimBlank(IObjectValue model){
		IncomeAccountInfo theModel = (IncomeAccountInfo) model;
