...
//		this.menuView.setVisible(false);
	...
	private void fillRowByContractExeData(IRow row, ContractExecuteData data) {
		...
		//合同未结算，并且非工程量确认，累计完工工程量等于合同内工程款
		...
		//add by zkyan BT714009 现在：未付款原本金额=最新造价原币-实际付款原币，未付款本币金额=最新造价本币-合同内工程款，这里：未付款原币金额应该=最新造价原币-合同内工程款原币。
...
//		row.getCell("payRealDate").setValue(child.getRealPayDate());
	...
	private void fillTable(List datas) {
		if (datas != null && !datas.isEmpty()) {
			...
			BigDecimal completePrjAmt = FDCConstants.ZERO;//累加完工工程量
			BigDecimal adjustAmount = FDCConstants.ZERO;//调整金额
