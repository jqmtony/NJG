...
//		filter.getFilterItems().add(new FilterItemInfo("CU.id",cuId));
		...
		/*
		 * modified by zhaojie for BT868216 on 2015-01-30
		 */
		filter.getFilterItems().add(new FilterItemInfo("isCFreeze", false));	//是否本级禁用
		//filter.getFilterItems().add(new FilterItemInfo("isGFreeze", false));	//是否上级禁用
		
		
		/* modified by zhaoqin for R20150401-0260 on 2015/04/16 */
		filter.getFilterItems().add(new FilterItemInfo("isCFreeze", false));	//是否本级禁用
	...
	private void treeSelectChange() throws Exception {
		...
		if (node.getUserObject() instanceof CurProjectInfo) {
			...
			if(projectInfo.getFullOrgUnit().getPartFI()==null){//检查是否已经从数据库内取过值了
				projectInfo = CurProjectFactory.getRemoteInstance().getCurProjectInfo(new ObjectUuidPK(projectInfo.getId()),
						getCurProjectSelector());
