...
public class RichCustomWriteOffControllerBean extends AbstractRichCustomWriteOffControllerBean
{
    private static Logger logger =
        Logger.getLogger("com.kingdee.eas.custom.richinf.app.RichCustomWriteOffControllerBean");
    
    @Override
    protected void _aboutHxAndFanHx(Context ctx, IObjectCollection fpColl,
	...
	private void doHandHX(Context ctx, IObjectCollection fpColl,
			IObjectCollection richExamColl, IObjectValue ov) throws BOSException {
    	...
    	String richCompayid = ((RichCustomWriteOffInfo)ov).getId().toString();
	...
	private void doAutoHX(Context ctx, IObjectCollection fpColl,
			IObjectCollection richExamColl, IObjectValue ov) throws BOSException {
		
	}

	private void doFanHX(Context ctx, IObjectCollection fpColl,
			IObjectCollection richExamColl, IObjectValue ov) throws BOSException {
		SelectorItemCollection sic = new SelectorItemCollection();
    	sic.add("yhxAmount");
    	//初始化本地接口
    	...
    	OtherBillCollection collFP = (OtherBillCollection)fpColl;
    	RichExamedCollection collRE = (RichExamedCollection)richExamColl;
    	String richCompay = ((RichCustomWriteOffInfo)ov).getId().toString();
		String customerId = null;
		String fpid = null;
		String djid = null;
		BigDecimal yhxAmount = null;
