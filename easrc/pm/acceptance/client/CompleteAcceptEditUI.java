 ...
 * output class name
 */
public class CompleteAcceptEditUI extends AbstractCompleteAcceptEditUI {
	private static final Logger logger = CoreUIObject
			.getLogger(CompleteAcceptEditUI.class);

	/**
	 * output class constructor
	 */
	public CompleteAcceptEditUI() throws Exception {
		super();
	}

	@Override
	public void onLoad() throws Exception {
		setPreferredSize(new Dimension(1013, 400));
		contDescription.setVisible(false);
		contBizStatus.setVisible(false);

		prmtprojectName.setRequired(true);
		pkBizDate.setRequired(true);
        this.kdtE1.getColumn(2).getStyleAttributes().setLocked(false);
		super.onLoad();
		
		FaCatPromptBox facatBox = new FaCatPromptBox();
		facatBox.setACompanyOrgUnitInfo(SysContext.getSysContext().getCurrentFIUnit());
		
		KDBizPromptBox kdtE1_type_PromptBox = new KDBizPromptBox();
        ...
        kdtE1_type_PromptBox.setSelector(facatBox);
		...
		kdtE1_management_PromptBox.setSelector(f7);
        ...
        kdtE1_takeOverDep_PromptBox.setSelector(f7);
	}
	...
	public void actionAudit_actionPerformed(ActionEvent e) throws Exception {
		...
		CompleteAcceptE1Collection completeAcceptColl = editData.getE1();
    	for(int i = 0; i < completeAcceptColl.size(); i++) {
    		CompleteAcceptE1Info entryInfo = completeAcceptColl.get(i);
    		int number = Integer.valueOf(entryInfo.getQuantity().toPlainString());
    		for(int j = 0; j < number; j++)
				try {
					com.kingdee.eas.port.pm.acceptance.utils.createCard.codeToFaCard(entryInfo);
	...
	protected com.kingdee.eas.framework.ICoreBase getBizInterface()
			throws Exception {
		return com.kingdee.eas.port.pm.acceptance.CompleteAcceptFactory
				.getRemoteInstance();
	}
