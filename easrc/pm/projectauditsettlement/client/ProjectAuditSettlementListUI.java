 ...
 * output class name
 */
public class ProjectAuditSettlementListUI extends AbstractProjectAuditSettlementListUI
{
    private static final Logger logger = CoreUIObject.getLogger(ProjectAuditSettlementListUI.class);
    
    /**
     * output class constructor
     */
    public ProjectAuditSettlementListUI() throws Exception
    {
        super();
    }
    ...
    protected IQueryExecutor getQueryExecutor(IMetaDataPK arg0,EntityViewInfo entityViewInfo) {
    	...
    	if(treeNode!=null)
    	{
    		if(treeNode !=null && treeNode.getUserObject() instanceof OrgStructureInfo){
				...
				filterInfo.getFilterItems().add(new FilterItemInfo("projectName.Company.longnumber", orgInfo.getLongNumber()+"%", CompareType.LIKE));

    		} else if(treeNode.getUserObject() instanceof ProjectInfo) {
    			ProjectInfo project = (ProjectInfo) treeNode.getUserObject();
    			filterInfo.getFilterItems().add(new FilterItemInfo("projectName.longNumber", project.getLongNumber() + "%", CompareType.LIKE));
    		} else {
