...
import com.kingdee.eas.port.pm.utils.TreeSelectDialog;
	...
	protected void getOtherFilter(FilterInfo filter, FDCCustomerParams para) {
		...
		if (para.isNotNull(IS_SHOW_ALL) && !para.getBoolean(IS_SHOW_ALL) && para.isNotNull(DATE_TYPE)) {
			...
			if (getBeginQueryDate(para) != null) {
				filter.getFilterItems().add(new FilterItemInfo("bookedDate", getBeginQueryDate(para), CompareType.GREATER_EQUALS));
			}
			if (getEndQueryDate(para) != null) {
				filter.getFilterItems().add(new FilterItemInfo("bookedDate", FDCHelper.getNextDay(getEndQueryDate(para)), CompareType.LESS));
			}
		}
