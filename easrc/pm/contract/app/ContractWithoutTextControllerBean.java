...
public class ContractWithoutTextControllerBean extends
	...
	private static Logger logger = Logger.getLogger("com.kingdee.eas.port.pm.contract.app.ContractWithoutTextControllerBean");
	// 付款申请单编码
...
//		ContractExecInfosFactory.getLocalInstance(ctx).updateContract(ContractExecInfosInfo.EXECINFO_AUDIT, billId.toString());
	...
	void subBudgetAmount(Context ctx, IObjectPK pk)throws BOSException,EASBizException{
		...
		for (int i = 0; i <info.getBudgetEntry().size(); i++) {
			ContractWithoutTextBudgetEntryInfo entry = info.getBudgetEntry().get(i);
...
//		paramMap.put("isCost", Boolean.FALSE);
	...
	private void checkBillForSubmit(Context ctx, IObjectValue model)
		...
		String comId = null;
		if (contractBill.getCurProject().getCompany() == null) {
			SelectorItemCollection sic = new SelectorItemCollection();
			sic.add("company.id");
			ProjectInfo curProject = ProjectFactory.getLocalInstance(ctx).getProjectInfo(new ObjectUuidPK(contractBill.getCurProject().getId().toString()), sic);

			comId = curProject.getCompany().getId().toString();
		} else {
...
//		}
	...
	private void checkBillForAudit(Context ctx, BOSUuid billId,
		...
		// 检查功能是否已经结束初始化
		String comId = model.getCurProject().getCompany().getId()
				.toString();
