 ...
 * output class name
 */
public class JudgesExamineListUI extends AbstractJudgesExamineListUI
{
    private static final Logger logger = CoreUIObject.getLogger(JudgesExamineListUI.class);
    
    /**
     * output class constructor
     */
    public JudgesExamineListUI() throws Exception
    {
        super();
    }
    ...
    protected IQueryExecutor getQueryExecutor(IMetaDataPK arg0,
    	...
    	if(treeNode!=null)
    	{
    		if(treeNode !=null && treeNode.getUserObject() instanceof OrgStructureInfo){
				...
				filterInfo.getFilterItems().add(new FilterItemInfo("reportName.proName.Company.longnumber", orgInfo.getLongNumber()+"%", CompareType.LIKE));

    		} else if(treeNode.getUserObject() instanceof ProjectInfo) {
    			ProjectInfo project = (ProjectInfo) treeNode.getUserObject();
    			filterInfo.getFilterItems().add(new FilterItemInfo("reportName.proName.longNumber", project.getLongNumber() + "%", CompareType.LIKE));
    		} else {
