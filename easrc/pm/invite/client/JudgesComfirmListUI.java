 ...
 * output class name
 */
public class JudgesComfirmListUI extends AbstractJudgesComfirmListUI
{
    private static final Logger logger = CoreUIObject.getLogger(JudgesComfirmListUI.class);
    
    /**
     * output class constructor
     */
    public JudgesComfirmListUI() throws Exception
    {
        super();
    }
    ...
    protected IQueryExecutor getQueryExecutor(IMetaDataPK arg0,EntityViewInfo entityViewInfo) {
    	...
    	if(treeNode!=null)
    	{
    		if(treeNode !=null && treeNode.getUserObject() instanceof OrgStructureInfo){
				...
				filterInfo.getFilterItems().add(new FilterItemInfo("planName.proName.Company.longnumber", orgInfo.getLongNumber()+"%", CompareType.LIKE));

    		} else if(treeNode.getUserObject() instanceof ProjectInfo) {
    			ProjectInfo project = (ProjectInfo) treeNode.getUserObject();
    			filterInfo.getFilterItems().add(new FilterItemInfo("planName.proName.longNumber", project.getLongNumber() + "%", CompareType.LIKE));
    		} else {
