...
import com.kingdee.eas.port.pm.invest.investplan.client.LimitedTextDocument;
...
public class PTEEditUI extends AbstractPTEEditUI implements REAutoCompletionDataSetter, REAutoCompletionDataFilter
{
    ...
    public void onShow() throws Exception {
		...
		for (int i = 0; i < pteInfo.getPteCost().size(); i++) {
			ProgrammingTemplateEntirePteCostInfo pteCostInfo = pteInfo.getPteCost().get(i);
    ...
    public void actionSubmit_actionPerformed(ActionEvent e) throws Exception {
		...
		for (int i = 0; i < pteSize; i++) {
			ProgrammingTemplateEntirePteCostCollection pteCostCollection = pteCollection.get(i).getPteCost();// 成本构成集合
			for (int j = 0; j < pteCostCollection.size(); j++) {
				ProgrammingTemplateEntirePteCostInfo pteCostInfo = pteCostCollection.get(j);// 成本构成
    	...
    	for(int i=0;i<enonomy_rowCount;i++){
    		ProgrammingTemplateEntirePteEnonomyInfo pteEnonomyInfo = new ProgrammingTemplateEntirePteEnonomyInfo();
	...
	private void addEconomyLine(KDTable table) {
		...
		for (int i = 0; i < pteInfo.getPteEnonomy().size(); i++) {
			ProgrammingTemplateEntirePteEnonomyInfo pteEnonomyInfo = pteInfo.getPteEnonomy().get(i);
			row = table.addRow();
	...
	private BigDecimal getCostAccountScale(CostAccountInfo costAccountInfo, boolean flag) {
		...
		for (int i = 0; i < pteCollection.size(); i++) { // 框架合约集合
			...
			if (flag) {
				ProgrammingTemplateEntirePteCostCollection forCostCollection = forPteInfo.getPteCost(); // 框架合约中成本构成集合
				for (int k = 0; k < forCostCollection.size(); k++) {
					ProgrammingTemplateEntirePteCostInfo pteCostInfo = forCostCollection.get(k); // 成本构成
					if (pteCostInfo.getCostAccount() != null) {
	...
	protected IObjectValue createNewDetailData(KDTable table, Object obj) {
		...
		if (obj instanceof ProgrammingTemplateEntirePteCostInfo) {
			ProgrammingTemplateEntirePteCostInfo newDetailInfo = new ProgrammingTemplateEntirePteCostInfo();
		...
		if (obj instanceof ProgrammingEntryEconomyEntryInfo) {
			ProgrammingTemplateEntirePteEnonomyInfo newDetailInfo = new ProgrammingTemplateEntirePteEnonomyInfo();
