...
public class CostAccountControllerBean extends AbstractCostAccountControllerBean {
	...
	protected void _update(Context ctx, IObjectPK pk, IObjectValue model) throws BOSException, EASBizException {
		...
		if (newCai.isIsSource()) {
			...
			params = null;
			sql =  "update T_FDC_CostAccount set FLongNumber=?,FNumber=?,FName_l2=?,FType=?,FIsCostAccount=?,FIsProgramming=? where fsrccostaccountid = ?";	
			params = new Object[]{newLn,newCai.getNumber(),newCai.getName(),newType,iCostAccount,isProgramming,oldCai.getId().toString()};
