...
//			this.createPaymentBill(payRequestBill, context);
	 ...
	 * @see
	 */
	private void createPaymentBill(PayRequestBillInfo payRequestBill, Context context) throws BOSException {
		try {
			// 付款申请单
			FilterInfo filter = new FilterInfo();
			...
			if (!(prbColl.size() > 0)) {
				throw new TaskExternalException(getResource(context, ""));
			}
...
//			paymentBill.setLstAMatlAllReqAmt(bigDecimal);
	 ...
	 * @see
	 */
	private void updateAdjustClause(PayRequestBillInfo payRequestBill, Context context) throws BOSException {
		try {
			// 付款申请单
			FilterInfo filter = new FilterInfo();
			filter.getFilterItems().add(new FilterItemInfo("number", payRequestBill.getNumber()));
