						...
						for (int rowIndex = block.getBeginRow(); rowIndex <= block
							...
							for (int colIndex = block.getBeginCol(); colIndex <= block
								...
								try {
									...
									kdtEntrys_editStopped(new KDTEditEvent(e
											.getSource(), null, FDCHelper.ZERO,
...
//			}
		...
		menuBiz.add(new AbstractHidedMenuItem("ctrl alt F11") {
			public void action_actionPerformed() {
				try {
					 ...
					 * for(;keys.hasMoreElements();){ Object e =
					 * keys.nextElement();
	...
	protected FDCNoCostSplitBillEntryCollection getCostSplitEntryCollection(
		...
		if (splitBillType.equals(CostSplitBillTypeEnum.CONTRACTSPLIT)) {
			...
			try {
				costColl = ConNoCostSplitFactory.getRemoteInstance()
						.getFDCNoCostSplitBillCollection(viewSplit);
			...
			viewSplit.setFilter(filterSplit);
			try {
				costColl = ConChangeNoCostSplitFactory.getRemoteInstance()
						.getFDCNoCostSplitBillCollection(viewSplit);
				coll = ConChangeNoCostSplitEntryFactory.getRemoteInstance()
			...
			viewSplit.setFilter(filterSplit);
			try {
				costColl = SettNoCostSplitFactory.getRemoteInstance()
						.getFDCNoCostSplitBillCollection(viewSplit);
