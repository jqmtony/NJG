...
public class EqmOverhaulControllerBean extends AbstractEqmOverhaulControllerBean
{
    ...
    protected void _audit(Context ctx, IObjectPK pk) throws BOSException,EASBizException {
    	...
    	try {
    		...
    		CoreBillBaseCollection eqcollection = EqmOverhaulFactory.getLocalInstance(ctx).getCoreBillBaseCollection(oql);
			generateDestBill(ctx, "803AE46D", "F96E9B71",eqcollection , new ObjectUuidPK(botInfo.getId()));
		} catch (Exception e) {
			e.printStackTrace();
		}
    }
    ...
    protected void _unAudit(Context ctx, IObjectPK pk) throws BOSException,EASBizException {
    	EqmOverhaulInfo aXRBillBaseInfo =  getEqmOverhaulInfo(ctx, pk);
	...
	protected void _unAudit(Context ctx, IObjectPK[] pks) throws BOSException, EASBizException {
		for(int i=0;i<pks.length;i++){
			EqmOverhaulInfo aXRBillBaseInfo =  getEqmOverhaulInfo(ctx, pks[i]);
			for (int j = 0; j < aXRBillBaseInfo.getE1().size(); j++) {
				checktranFormBill(ctx,aXRBillBaseInfo.getE1().get(j).getId().toString());
			}
		}
